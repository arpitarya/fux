...
**Latest — v0.18.0:** Navigable graph at scale — the interactive `graph.html` now **opens community-collapsed** above `graph_lod_threshold` nodes (default 2500; click a blob to drill in, **Detail** expands all) and every node is an **ego-graph** entry point (double-click / ◎ Focus / `E`) for its selectable **1–2 hop** neighbourhood, so a 20k-node graph stays legible. Adds `fux build --profile` (per-phase timing breakdown) and the opt-in `fux build --no-xref` mode (skips the loose `references` pass for very large repos). Also fixes a pre-existing determinism bug so the default `graph.json` is now **byte-identical across builds and `PYTHONHASHSEED`**. `$0`/stdlib/deterministic — no new deps, default graph content unchanged.
