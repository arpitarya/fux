...
| **2 · Node's latency** | ✅ **MEASURED 2026-09-15 (W-179)** — the column is populated beside Python's, four arms interleaved. 🔴 **The Node reader is FLAT in corpus size** (26.3 → 26.4 ms across a 10× corpus) and every bit of its growth is W-161's in-memory graph rebuild — 36.9 ms at 100 documents, 243.1 ms at 1 000, ~2.4 s at 10 000. ⚠ **Read without the split, the column says the opposite**: `B-node` 269.5 ms against Python's 231.2 ms reads as *Node is slower*, and it is 8.7× faster. **N4 has a number where it had none — and is NOT ruled**: a benchmark rules no threshold. [the run](../regression/2026-09-15-node-column/report.md) |
