...
description: "archived_weight, rerank_weight and recency_half_life_days are built, wired and multiply by one; superseded_weight was ruled closed on 2026-09-13 (W-151). Arpit ruled remeasure, then (b): build the instrument into the golden test data. Question: does ANY single global value clear 0 broken?"
status: open
lane: agent
...
- 🟠 **THE FOUR NO-OP RANKING PRIORS — one problem, one blocker.** `agent`, then `arpit` ·
  *(records: [SR-CONFIDENCE](../../records/0141_confidence.md) ·
  [SR-TUNE](../../records/0135_tuning.md) ·
  [SR-ARCHIVED-CONTENT](../../records/0134_archived-content.md) ·
  [SR-RS](../../records/0133_predictions.md))* · `filed: 2026-08-28` ·
  `ruled: 2026-09-11`
  ...
  *"Supersedes ADR-0007"* in **prose** and declares no `supersedes:` key —
  ...
  ⚠ Caught by SR-RS decision 22d hours after it was ratified; without it this
  ...
  *"Set up the test data… so that at least we can test it."* The playground route is
  void under [SR-WORK-ENVIRONMENTS](../../records/0052_WORK-environments.md); closing the knob (c) was not
  ...
  rule behind it is [SR-RS](../../records/0133_predictions.md) **decision 23**.
  **Lane: `agent` once W-136's ladder is frozen, then back to Arpit with the result.**

  **What the remeasure still needs:**

  - **A frozen pre-registration** naming its `k`, its arms and the `0 broken`
    bar *before* the first number ([SR-QUALITY](../../records/0056_WORK-quality.md)
...
## ✅ RULED 2026-09-13 — `superseded_weight` is closed, option (c)

**Arpit:** *"there is no weight that determines which document supersedes
another in the real world. So let's remove it."*

- **One of the four is decided.** The build is
  [W-151](W-151-remove-superseded-weight.md), `agent`, ratified and **not
  implemented** — Arpit is running it through Claude Code.
- **The fact survives the knob.** The `superseded` record property, the
  `supersedes:` declaration, the edge and the graph plane are untouched. What
  goes is the multiplier that pretended supersession has a magnitude.
- ⚠ **It changes no ranking.** The knob ships at `1.0`, so this is a cleanup,
  not a fix — the current decision still does not outrank the one it replaced.
- **`archived_weight`, `recency_half_life_days` and `rerank_weight` are
  unchanged and still his**, with the same three options.

## Carried from W-97, closed 2026-09-13

**W-97 was the only written home of these; they bear on the three priors still
open, so they move here rather than into the archive.**

- 🔴 **`rerank_weight` moves TWO mechanisms since W-108** — proximity reranking
  *and* the refer plane's rescore. A verdict on it owes that sentence, because
  `ask`-only arms do not fetch and therefore never exercise the second.
- 🔴 **`archived_weight` was never in W-97's sweep at all** (W-73's law), so no
  selection evidence for it exists outside
  [VERDICT-W143](../regression/2026-09-12-priors-and-tables/VERDICT-W143.md).
- 🔴 **The hand-graded veto leg has no instrument and cannot get one.** Its
  corpus was the playground, which [SR-WORK-ENVIRONMENTS](../../records/0052_WORK-environments.md)
  removed. The design called that leg load-bearing — *"a candidate with two
  green legs is not a candidate"* — so any future sweep needs a new instrument
  on golden data under a new pre-registration, not a revival of the frozen one.
- **`k1`, `b`, field weights and recency have no instrument with headroom** —
  PRE-REGISTRATION-TUNER §6 lists the generator kinds owed before a `T6+`
  document can exist.
...
query-side design. [SR-TUNE](../../records/0135_tuning.md) decision 13 carries
