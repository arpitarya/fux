...
closed: 2026-09-12
implementation: "work/IMPLEMENTATION.md — 2026-09-12, W-106"
---
...
- [x] ✅ **RUN 2026-09-12 — and the answer is ZERO.** Two-architecture arm,
      arm64 vs x86-64 under Rosetta, every version pinned identically
      (`torch 2.2.2` / `sentence-transformers 2.7.0` / `numpy 1.26.4` /
      `BAAI/bge-small-en-v1.5`): **cosine 1.000000**, **119/119** chunk and
      **124/124** query int8 vectors byte-identical, **0 of 93 312** codes
      differing, **0/124** top-5 orderings discordant.
      [The run](../regression/2026-09-12-vector-gate-crossarch/report.md).
      ⚠ **Rosetta answers *does the x86-64 BUILD differ* and only stands in for
      *does native x86-64 HARDWARE differ*** — the report says which, and a
      native x86 runner is still unmeasured.
...
- [x] ✅ `OPEN-WORK.md` row deleted, W-112 re-balled, this file archived. 2026-09-12.
...
## ✅ CLOSED 2026-09-12 — the arm ran, and it inverts the 2026-09-05 finding

**Everything this item still owed is done.** The two-architecture arm found
**nothing**: one embedder build on two architectures produces byte-identical
vectors and identical orderings, at every level measured.

🔴 **Read against 2026-09-05, that is the interesting result.** Two
**implementations** on one architecture shared **0 of 125** int8 vectors and
disagreed on **41 of 50** orderings. Two **architectures** running one
implementation disagree on **nothing**. **The variable is the implementation,
not the machine** — so W-112's determinism claim narrows from *"same clone +
same embedder build + same CPU"* to **"same embedder build"**, and that is the
one sentence this run licenses.

**What it does NOT do**, and both matter more than the zero:

- ⚠ **It does not revive the vector plane.** The retrieval half — DENSE-CHUNK's
  frozen `>= 3 fixed / 0 broken` — was untestable on 2026-09-05 because the
  playground's index was unreadable, and is untestable now for a *new* reason:
  **the golden ladder's questions carry no rank contract**, by design, because
  their key is sealed. A corpus that can carry that bar does not exist.
- ⚠ **Native x86-64 is still unmeasured.** Rosetta translates the x86-64 build
  onto arm64 hardware, and under it the x86 kernels see no AVX-512 — the exact
  mechanism that would break bit-equality on a real x86 runner. Anyone quoting
  this number for Linux/x86 CI is quoting the wrong measurement. That is a new
  item if it is wanted, not a gap in this one.

**W-112 is unchanged in status.** It was `arpit`-lane before and remains so; what
changed is that one of its open questions has an answer.
