...
- **Live tracker:** [`../work/OPEN-WORK.md`](../work/OPEN-WORK.md) — an **index** of open
  items (restructured 2026-08-12); pick work there, then read the item's
  detail file in [`work/open/`](../work/open/README.md). This file is the *spec* for
  each milestone id; the detail file is the *state*.
- **The gate is closed.** P1 was measured twice ([ADR-PRUNING-GATE](../work/adr/0002_pruning-eval-gate.md)
  INCONCLUSIVE → [ADR-PRUNING-RERUN](../work/adr/0003_pruning-criterion-rerun.md) **FAIL**),
...
- **Format of record:** [`../work/compare/index-format.compare.md`](../work/compare/index-format.compare.md)
...
- **Handoffs:** every milestone ships as handoff + prompt with the model
  named. Specs now live in the item's detail file under `work/open/`; the
  retired pairs are in [`archive/handoff/`](../archive/README.md).
...
*(the live handoff:
[`../archive/handoff/v0.30.0-m1-t0-slice-handoff.md`](../archive/handoff/v0.30.0-m1-t0-slice-handoff.md))*
...
cold clone; ADR-INDEX-FORMAT accepted; goldens started.
...
from this tree ([ADR-PLAYGROUND](../work/adr/0012_playground-sibling-repo.md)). Fifty
...
### M2 — the T1 accelerator ✅ **SHIPPED 2026-08-12** ([ADR-ACCELERATOR](../work/adr/0005_derived-accelerator.md))
...
common terms; differential suite green; ADR-ACCELERATOR.

**Measured** ([the run](../work/regression/2026-08-12-m2-accelerator/report.md)):
...
> middleware file — [ADR-URL-MIDDLEWARE](../work/adr/0010_url-source-consumer-middleware.md)
...
systems arrive via [`../work/proposals/mcp-adapters.md`](../work/proposals/mcp-adapters.md),
...
## Predictions, re-registered (R-series; the P-series is closed by ADR-PRUNING-GATE/0003)
...
| R2 | T0 slice answers real questions on this repo | cited answer from a cold clone, no accelerator | M1 — **3/3 PASS** 2026-08-12 ([run](../work/regression/2026-08-12-r2-close/report.md)) |
| R3 | warm `ask` ≤ 150 ms on the RFC corpus, worst-case terms included | bench | M2 — **PASS** 2026-08-12, worst-case p95 **27.2 ms** ([run](../work/regression/2026-08-12-m2-accelerator/report.md)) |
...
- **Canonical-JSON reproducibility across environments** — the design's
  ...
  normalization decided in ADR-INDEX-FORMAT, analyzer version in the header.
