---
type: ADR
name: ADR-INDEX-FORMAT
title: ADR-INDEX-FORMAT (0004) — Index format & committed store — schema, canonical rules, unicode policy frozen
...
# ADR-INDEX-FORMAT — Index format & committed store
...
> **Live successors: ADR-INGEST (ingest), ADR-INDEX-LIFECYCLE (storage), ADR-RECORD (schema)** in
> [`docs/adr/`](../../docs/adr/README.md). See
> [`../README.md`](../README.md) §Archive is not evidence.


- **Name:** `ADR-INDEX-FORMAT` — cite this everywhere; never cite the number
- **Status:** superseded by ADR-INGEST (ingest), ADR-INDEX-LIFECYCLE (storage), ADR-RECORD (schema)
- **Owns:** `src/fux/store/`, `src/fux/ingest/`
- **Laws:** L1, L2, L3 — see [ADR-LAWS](../../docs/adr/0001_laws.md); never restated here
- **Date:** 2026-08-10
...
[`../../work/compare/index-format.compare.md`](../../work/compare/index-format.compare.md) §5 named the
...
| `mode` | `"extracted"` (the only ingest mode this milestone; `enriched` is M8) | ADR-INGEST-MODES |
...
> ([SETUP-PLAYGROUND](../setup/fux-playground.md)). The evidence this ADR
...
- **R2 — 2 of 3 PASS, 1 blocked on a pre-existing gap:**
  ...
  2. `"what format is the committed index"` → initially missed the top 5
     (`work/compare/index-format.compare.md` ranked #9, behind
     ...
     **Re-measured 2026-08-12: #2**, behind `README.md`, which gained a
     `.fux/` layout table in the interim (ADR-FUX-DIR). Still a PASS — the
  ...
  3. `"supersession penalty safe interval"` → the frozen citation target,
     ...
     introduced by this build: the archive README of the day (since removed)
     ...
     `work/regression/README.md` ranks it #1 for this query, and states
     ...
     **[Resolved 2026-08-10 — Arpit's ruling: everything archived lives in the root `archive/`; there is no `archive/`. The v0.26 doc set now sits at `archive/v0.26-docs/`, the nested double-archive is flattened, and the flagged discrepancy is closed.]**
     ...
     **R2 is 3/3 PASS.** R1 re-asserted (double-ingest byte-identical); the
     ...
     Full run: [`../../work/regression/2026-08-12-r2-close/report.md`](../../work/regression/2026-08-12-r2-close/report.md).
- **Playground walkthrough** (`examples/playground/`): all three
  `PLAYGROUND.md` questions answer with on-topic citations from the real
  index; the retry-budget superseded pair (P1-GATE/ADR-ACCELERATOR) both surface
...
- [`archive/v0.30.0-m1-t0-slice-handoff.md`](../../archive/handoff/v0.30.0-m1-t0-slice-handoff.md)
