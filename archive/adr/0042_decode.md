...
- 🔴 **NOT backward compatible, and no migration was written** — Arpit's call,
  ...
  `<name>doc.py` files in `.fux/decoders/`. They still declare the same
  `EXTENSIONS`, and because `_consumer_decoders` overlays the sorted glob onto
  the built-ins, **`csvdoc` is applied after `csv` and wins**: that repo
  silently keeps running its old decoders, without any fix landed since.
  **Deleting them is the upgrade step, and it is manual.**
...
- **A decoded document can be cited for the first time.** `refer/__init__.py`
  ...
  `extract.py` and nothing else. See [ADR-REFER](0030_refer-plane.md)
  decision 23.
