...
type: ADR
name: ADR-FUX-DIR
title: ADR-FUX-DIR (0011) — the .fux directory — declared layout, committed vs derived
description: Declares every child of .fux/ as committed (index, sources, middleware) or derived (runtime, cache), with a self-describing README, a narrow .gitignore that never uses `*`, CACHEDIR.TAG on derived dirs, and two `fux doctor` assertions. Relocates ADR-URL-MIDDLEWARE's URL list to .fux/sources/urls and its middleware to .fux/middleware/cdp.py, with tunables moving to an opaque [sources.url.config] table.
status: superseded
timestamp: 2026-08-11T00:00:00Z
---

# ADR-FUX-DIR — the `.fux` directory — declared layout, committed vs derived
...
> **Live successors: ADR-DOTFUX (layout), ADR-CONFIG (config)** in
> [`docs/adr/`](../../docs/adr/README.md). See
> [`../README.md`](../README.md) §Archive is not evidence.


- **Name:** `ADR-FUX-DIR` — cite this everywhere; never cite the number
- **Status:** superseded by ADR-DOTFUX (layout), ADR-CONFIG (config)
- **Owns:** `src/fux/store/fuxdir.py`, `src/fux/config.py`, `src/fux/doctor.py`
- **Laws:** L1, L3 — see [ADR-LAWS](../../docs/adr/0001_laws.md); never restated here
  ratifies by flipping this line)
- **Date:** 2026-08-11
- **Feature:** the `.fux/` directory as a declared layout, plus the relocation
  of ADR-URL-MIDDLEWARE's URL source into it.
...
2. **ADR-URL-MIDDLEWARE's placements did not scale.** The URL list was an inline TOML
...
| `index/` | committed | the wire-format index (ADR-INDEX-FORMAT) |
...
file — a hard move, since ADR-URL-MIDDLEWARE shipped hours earlier and unreleased.
...
- **Middleware at the repo root** (ADR-URL-MIDDLEWARE's original). Rejected: it puts a
...
- **`git log --follow`** does not show pre-move history for
  ...
  hours old, from the ADR-URL-MIDDLEWARE session), so there was no history to preserve.
