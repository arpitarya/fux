---
type: ADR
name: ADR-ACCELERATOR
title: ADR-ACCELERATOR (0005) — the derived T1 accelerator and the differential law
...
# ADR-ACCELERATOR — the derived T1 accelerator and the differential law
...
> **Live successors: ADR-ASK (query), ADR-T1-ACCELERATOR (build)** in
> [`docs/adr/`](../../docs/adr/README.md). See
> [`../README.md`](../README.md) §Archive is not evidence.


- **Name:** `ADR-ACCELERATOR` — cite this everywhere; never cite the number
- **Status:** superseded by ADR-ASK (query), ADR-T1-ACCELERATOR (build)
- **Owns:** `src/fux/derive/`, `src/fux/query/`, `src/fux/embed/`, `tools/differential/`
  — `src/fux/cli.py` was rehomed to [ADR-CLI](../../docs/adr/0002_cli-surface.md) on 2026-08-18
...
**A second instrument agrees.** Under fusion, R2's frozen Q1 loses P1-RERUN
...
- **A source-exclusion mechanism** ([W-45](../../work/open/W-45-source-exclusion.md)) —
...
- `.fux/runtime/` is a new derived directory in every consumer's tree. `fux
  ...
  ADR-FUX-DIR, which is the ignore rule that ADR predicted would matter.
...
- [`work/adr/0004_index-format.md`](0004_index-format.md) — the committed
  format, frozen; and [ADR-FUX-DIR](0011_fux-dir-layout.md) — the derived-plane
...
- [`work/INTERVIEW.md`](../../work/INTERVIEW.md) §"what a confident successor must not
