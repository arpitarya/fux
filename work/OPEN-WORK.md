...
### [ADR-TUNE](../docs/adr/0038_tuning.md) · [ADR-CLI](../docs/adr/0002_cli-surface.md) — three things that look shipped and are not

- **W-79** · `agent` · **filed 2026-08-24 on Arpit's instruction** (*"we need to remove the dead code"*). `query/hybrid.py` is **off the live path** — `--hybrid` routes through `query/dense.py`'s gated fusion and `hybrid_ask` is reached only from `tools/differential/playground_grade.py`; its two `[fuse]` tune keys are therefore **settable, validated and unreachable**; and `explain --no-tune` parses and does nothing. ⚠ **`[dense] mode = "gated"` is NOT in this list, and an earlier reading wrongly said it was** — it did not fire at `threshold` 0.5 or 2.0 because the corpus's top lexical score is ~8.08 and the gate is `score < threshold`; at 8.0 it fires. **The code works**; it failed [its own gate](regression/2026-08-24-dense-lane-gate/VERDICT.md) on what it admits, which means keep it and keep it `off`. **The ruling owed is delete-or-wire-up**; deleting the two `[fuse]` keys is a **closed-key-set change**, so it amends ADR-TUNE rather than tidying it — [detail](open/W-79-remove-the-dead-fusion-code.md)
