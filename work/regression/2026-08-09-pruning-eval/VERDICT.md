...
type: Verdict
name: P1-GATE
title: P1-GATE — the pruning-quality gate, first run — INCONCLUSIVE
description: "M1's measured result. The pre-registered numeric condition is met on all three corpora (delta hit@5 = 0.00 pts at k=128), but prune coverage shows the treatment reached 0-2.5% of documents and retained 96-100% of postings, so the experiment cannot license the inference the threshold was written to license."
verdict: INCONCLUSIVE
prediction: P1
pre_registration: tools/pruning-eval/PRE-REGISTRATION.md
timestamp: 2026-08-09T00:00:00Z
---

# P1-GATE — the pruning-quality gate, first run: **INCONCLUSIVE**
...
> [P1-RERUN](../2026-08-09-pruning-rerun/VERDICT.md) — the re-run this document
> asked for, which found **FAIL**. **This document is unmodified in substance:**
> it is the record of a correct refusal, and the refusal is what produced the
> re-run.

- **Name:** `P1-GATE` — cite this by name
- **Verdict:** **INCONCLUSIVE** — the scaffold stays blocked; the call is Arpit's
...
- **The harness:** [`tools/pruning-eval/`](../../../tools/pruning-eval/README.md)
  — owned by [W-38](../../open/W-38-m8-deferred.md), the only live item allowed
  to touch pruning work
...
- **W-06…W-11 (M2–M7) stay blocked** on `W-05 = PASS`, which this ADR does not
  grant.
- **P1's status in [`../../work/OPEN-WORK.md`](../../work/OPEN-WORK.md) becomes `INCONCLUSIVE`**,
  not `PASS` and not `FAIL`.
- [`../../work/compare/storage-architecture.compare.md`](../../work/compare/storage-architecture.compare.md)
...
- **The evidence** —
  [`work/regression/2026-08-09-pruning-eval/`](../../work/regression/2026-08-09-pruning-eval):
  `report.md`, `results.json`, `retention.json`, `ANALYSIS.md`, and the exact
  reproduce command.
- **The prediction under test** —
  [`work/paper/the-fux-index-paper.md`](../paper/the-fux-index-paper.md) §5
