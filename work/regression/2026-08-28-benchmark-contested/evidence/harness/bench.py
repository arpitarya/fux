...
def quality(run: str, arm: str, tier: str, corpus: str | None, label: str | None,
    ...
    contested = evald / "contested.jsonl"
    for i, row in enumerate(load(contested) if contested.exists() else []):
        locs, ms = ask(arm, cwd, row["q"], TOP)
        ranks = {c: (locs.index(c) + 1 if c in locs else 0) for c in row["candidates"]}
        r_t = ranks[row["target"]]
        seen = [r for r in ranks.values() if r]
        # `target_first` is scored WITHIN the cluster: did the target outrank
        # every other candidate? Overall rank-1 would be contaminated by the
        # rest of the corpus, and the contest is between these candidates.
        #
        # A cluster with nothing visible in the top-k has no ordering to score,
        # exactly as a chain with one half missing has no inversion — scoring it
        # either way would be inventing a comparison the run did not make. It is
        # emitted as `null` so the pairing drops it rather than counting it as a
        # loss for whichever arm happened to surface less.
        target_first = None if not seen else int(bool(r_t) and r_t == min(seen))
        rows.append({
            "suite": "contested", "qid": f"cont-{row['kind']}-{i:04d}", "q": row["q"],
            "kind": row["kind"], "target": row["target"],
            "rank_target": r_t, "ranks": ranks,
            "candidates_visible": len(seen), "target_first": target_first,
            "hit@5": int(0 < r_t <= 5), "hit@10": int(r_t > 0),
            "rr": round(1.0 / r_t, 6) if r_t else 0.0, "top": locs[:5], "ms": round(ms, 2),
        })
...
def paired(rows_a: list[dict], rows_b: list[dict], key: str) -> dict:
    ...
    for qid in shared:
        ...
        if va is None or vb is None:
            # Unscorable in EITHER arm drops the pair from both. A pair scored
            # in one arm only is not a pair.
            continue
        if vb and not va:
            b += 1
...
def main() -> int:
    ...
    m.add_argument("--kind", help="restrict to one `kind` within the suite")
