...
| the records | [ADR-AGENT-POLICY](../docs/adr/0042_agent-policy.md) d11-14, veto 5a/5b/5c · [ADR-ENRICH](../docs/adr/0047_enrich.md) d10 · [ADR-CONFIG](../docs/adr/0023_config.md) d9 · [ADR-DOTFUX](../docs/adr/0012_fux-directory.md) d6/d9 |
...
claiming `blind`. That is [ADR-RS](../docs/adr/0043_predictions.md)'s rule, not
a new one.
...
|---|---|
| 🔴 **`freshness verdicts` + `doctor --json`'s `freshness` block** — the `as-ingested` share. **The veto check of two ACCEPTED records**; until this, neither veto could be run at all | [ADR-ACQUIRED](../docs/adr/0057_acquired-plane.md) · [ADR-URL-FRESHNESS](../docs/adr/0059_url-freshness.md) · `tests/test_doctor.py` |
| **`refusal rules`** — rules that load, responses each has refused (cumulative, in `urlstate.refused`), and **the rules that have never fired**. The loud case: refusals recorded with no `url:` document surviving | [ADR-REFUSAL](../docs/adr/0058_refusals.md) decision 11 |
| **`decoder bindings`** — every binding resolves, plus the fault no ingest can catch: a binding on an extension **no indexed document has** | [ADR-DECODE](../docs/adr/0049_decode.md) |
| 🔴 **`recency prior`** — whether any document carries an `mtime`. A corpus copied out of git loses **every one**, and the whole prior switches off silently | [ADR-INGEST](../docs/adr/0016_ingest.md) · `ingest/priors.py` |
| **Redaction counts on `pii rules`** — `redact()` returned them and `run()` summed them into a variable **nothing ever read** | [ADR-PII](../docs/adr/0060_pii.md) decision 15 |
...
| what landed | evidence |
|---|---|
| `rank()`'s sort key becomes `(-round(s, 9), superseded, -mtime, -priority, id)` — **Arpit's ratified order**, identical on both paths, **turning no ranking prior on** | [ADR-RANKING](../docs/adr/0021_ranking.md) · `tests/query/test_ties_and_filters.py` |
| `AskResult.tie`, in `--json` (`required: always`) and as `(tie)` in text — **computed before truncation**, so the last row of a `--top 5` is marked | [ADR-ASK](../docs/adr/0013_ask.md) · [ADR-OUTPUT](../docs/adr/0054_output-defaults.md) |
| `find --phrase / --under / --all` — post-filters that retrieve nothing; `band` describes the unfiltered ranking; a `url:` document is kept by `--phrase`, never dropped | [ADR-FIND](../docs/adr/0014_find.md) · [ADR-RERANK](../docs/adr/0048_rerank.md) · [ADR-CLI](../docs/adr/0011_cli-surface.md) |
| ⚠ **`-priority` is UNREACHABLE and the code says so** — `priority_for` *is* the weight, so different priorities already produce different scores | [ADR-TUNE](../docs/adr/0045_tuning.md) · a test that fails if it changes |
...
| what landed | evidence |
|---|---|
| `ENRICH-SKILL.md` re-aimed at **doc2query** — 5–10 questions, one per line, no summary, no title echo, currency in frontmatter; every vendored copy refreshed | [ADR-ENRICH](../docs/adr/0047_enrich.md) 15 · [ADR-AGENT-POLICY](../docs/adr/0042_agent-policy.md) |
| **the doc2query−− filter** — `--check` refuses a question that does not place its document in the **top 3** (`SELF_RETRIEVAL_K`, Arpit's value), scored with `title` **and** `ctx` zeroed | [ADR-ENRICH](../docs/adr/0047_enrich.md) 16 · `tests/enrich/test_doc2query.py` |
| `superseded_by:` in an enrichment retires its document — declared, never inferred, with `superseded_ids`' three rules | [ADR-ENRICH](../docs/adr/0047_enrich.md) 17 · [ADR-INGEST](../docs/adr/0016_ingest.md) |
| 🔴 **extraction reuse now sees the enrichment** — a new, changed or deleted `.fux/enrich/` file invalidates that document's reuse | [ADR-ENRICH](../docs/adr/0047_enrich.md) 18 · [ADR-INGEST](../docs/adr/0016_ingest.md) |
...
| what landed | evidence |
|---|---|
| `query/expand.py::Expansion` — `hashes` / `required` / `weights` as **one** object; `score_record` gains a per-term multiplier; `[ranking] expand_weight = 0.2` | [ADR-EXPAND](../docs/adr/0061_expand.md) 1-5 · [ADR-RANKING](../docs/adr/0021_ranking.md) · [ADR-TUNE](../docs/adr/0045_tuning.md) |
| 🔴 the hallucination guard — a candidate matching no **original** term is dropped in `rank()`, before the score is kept, on both paths | [ADR-EXPAND](../docs/adr/0061_expand.md) 3 · `tests/query/test_expand.py` |
| 🔴 the accelerator bound priced per term, **and `theta` filtered on the guard** — found by measurement: the arms diverged at every `expand_weight >= 0.5` at `top = 20`, including at `1.0` | [ADR-T1-ACCELERATOR](../docs/adr/0020_accelerator.md) · `tests/derive/test_expand_bound.py` (3 injections, 2 corpus shapes) |
| `query/fuse.py` — RRF `k = 60` in **rank** space; `"fused": true` in `--json`; `--band` describes the primary arm | [ADR-EXPAND](../docs/adr/0061_expand.md) 8-11 · [ADR-CONFIDENCE](../docs/adr/0052_confidence.md) · [ADR-OUTPUT](../docs/adr/0054_output-defaults.md) |
| the receipt records the expansion and `verify --rerun` replays it; `--why` marks each term `expanded` | [ADR-PROVENANCE](../docs/adr/0053_provenance.md) |
| MCP `fux_search` gains `expand`, carrying the retry rule; the shipped usage skill teaches it | [ADR-MCP](../docs/adr/0046_mcp.md) · [ADR-AGENT-POLICY](../docs/adr/0042_agent-policy.md) |
...
| what landed | evidence |
|---|---|
| `ANSWER_TOP = 3`; `cmd_answer` and `cmd_verify --rerun` retrieve it; `answer_via_refer` takes the candidate list; **one** `refer()` call, so the passage contest stays cross-document | [ADR-ANSWER](../docs/adr/0015_answer.md) 4/5/11 · [`2026-09-05-answer-top3`](regression/2026-09-05-answer-top3/report.md) |
| `_load_fetchers` — a **URL-keyed dispatcher**, so three citations behind three `.fux/sources/urls` lines each reach the fetcher their own line names; modules memoised, connected once, closed once; a load/connect failure costs its own documents only | [ADR-REFER](../docs/adr/0037_refer-plane.md) 22 · [ADR-URL-FRESHNESS](../docs/adr/0059_url-freshness.md) |
| `rescore(…, weight=)` — the reranker's `passage_boost` and its bounded multiplicative uplift, **one constant**, threaded from `[ranking] rerank_weight`. **Ships OFF** (`0.0`), byte-identical to before | [ADR-RERANK](../docs/adr/0048_rerank.md) 9 · [ADR-REFER](../docs/adr/0037_refer-plane.md) 21 |
| per-passage `id`/`loc`/`sha` in `--json` and a locator line per passage in text — fixing a mis-citation that predated the change | [ADR-ANSWER](../docs/adr/0015_answer.md) 12 · [ADR-OUTPUT](../docs/adr/0054_output-defaults.md) |
| `_freshness_of` follows the **winning** citation's document; `_declare_change_since_last_ask` reports the **cited** set, not everything fetched | [ADR-URL-FRESHNESS](../docs/adr/0059_url-freshness.md) |
...
| item | what landed | evidence |
|---|---|---|
| **W-102** | enrichment gets its own redaction pass; `_enrichment_for()` takes the ruleset and returns `(text, hits)`; `fux enrich --check` **refuses** a body with a hit, exits 1, rewrites nothing | [ADR-INGEST](../docs/adr/0016_ingest.md) 15a · [ADR-PII](../docs/adr/0060_pii.md) 1 · [ADR-ENRICH](../docs/adr/0047_enrich.md) 12 · [`2026-09-02-enrich-pii-leak`](regression/2026-09-02-enrich-pii-leak/report.md) |
| **W-103** | [ADR-LOCKS](../docs/adr/0050_locks.md) `proposed` → `accepted`; the two docstrings that still said `runner.lock` corrected; **the three veto checks re-run** and the capture re-dated | [ADR-MAINTENANCE](../docs/adr/0039_hooks.md) 8a · the capture in ADR-LOCKS |
| **W-104** | `fux enrich [TARGET]` — an exact-match positional that filters and never widens; distinct *not declared* / *not indexed* messages; `ENRICH-SKILL.md` rewritten to plan for itself, do one document, and stop to ask on a list | [ADR-CLI](../docs/adr/0011_cli-surface.md) 2a · [ADR-OUTPUT](../docs/adr/0054_output-defaults.md) · [ADR-ENRICH](../docs/adr/0047_enrich.md) 13 |
| **W-105** | `_Conn` per fetch, a page target per worker, `_lock` on launch, `_opened` closed exactly; `[sources.url.config] fetcher_max_parallel` on **both** shipped fetchers | [ADR-CDP-FETCHER](../docs/adr/0028_cdp-fetcher.md) 7/7a/7b/**7c** · [ADR-HTTP-FETCHER](../docs/adr/0029_http-fetcher.md) 7a · [ADR-FETCHER](../docs/adr/0027_fetcher.md) 9a · [`2026-09-02-cdp-parallel`](regression/2026-09-02-cdp-parallel/report.md) |
...
|---|---|---|
| 2 — refusal detection | `.fux/refusals.toml`, a declarative rules table read **once** before the first socket; checked after `_unpack`, **before** persist and decode | `src/fux/ingest/refusals.py` · `src/fux/templates/refusals.toml.txt` · [ADR-REFUSAL](../docs/adr/0058_refusals.md) |
| 3 — the acquired plane | clock-free `.fux/acquired/`, a **third** `fuxdir` category beside committed and derived; eviction by `run_seq`, never `mtime`; a URL with `fail_streak > 0` is never evicted | `src/fux/store/acquired.py` · `src/fux/store/fuxdir.py` · [ADR-ACQUIRED](../docs/adr/0057_acquired-plane.md) |
| 4 — freshness | `ttl=` on the URL line and the sixth verdict `as-ingested`, resolved as `min(policy, declared)` | `src/fux/refer/` · `src/fux/ingest/sourcelist.py` · [ADR-URL-FRESHNESS](../docs/adr/0059_url-freshness.md) |
...
1. 🔴 **Phase 2's boundary HELD instead of being amended.** The spec's `status`,
   ...
   [ADR-FETCHER](../docs/adr/0027_fetcher.md) decision 13 forbids — and whose
   ...
   **Arpit ruled CUT** (2026-09-01): the four came out, the remaining six
   ...
   string. [ADR-REFUSAL](../docs/adr/0058_refusals.md) decision 4.
...
Recorded as [ADR-CDP-FETCHER](../docs/adr/0028_cdp-fetcher.md) decision 12;
...
| reuse invalidation | `.fux/runtime/pii-digest`; editing the ruleset re-extracts everything ([ADR-INGEST](../docs/adr/0016_ingest.md) decision 15) |
| `url:` enrichment | `src/fux/enrich.py` — `_document_text`, `URL_SCOPE`; possible only because `.fux/acquired/` holds the bytes locally |
| the records | [ADR-PII](../docs/adr/0060_pii.md) · [ADR-ENRICH](../docs/adr/0047_enrich.md) decision 11 |
...
[ADR-ENRICH](../docs/adr/0047_enrich.md) decision 11 as well as the queue, so no
...
| the records | [ADR-CONFIG](../docs/adr/0023_config.md) d12 · [ADR-INGEST](../docs/adr/0016_ingest.md) d15–16 · [ADR-ENRICH](../docs/adr/0047_enrich.md) d11 · [ADR-REFER](../docs/adr/0037_refer-plane.md) d19–20 + veto 4/4a · [ADR-ASK](../docs/adr/0013_ask.md) · [ADR-OUTPUT](../docs/adr/0054_output-defaults.md) |
...
| the records | [ADR-TYPES](../docs/adr/0038_types-list.md) decision 11 · [ADR-DECODE](../docs/adr/0049_decode.md) decision 13 · [ADR-URL-LIST](../docs/adr/0026_url-list.md) decision 12 narrowed · [ADR-CLI](../docs/adr/0011_cli-surface.md) · [ADR-DOTFUX](../docs/adr/0012_fux-directory.md) |
...
| the record | [ADR-CDP-FETCHER](../docs/adr/0028_cdp-fetcher.md) decisions 5, 10–13, rewritten in place |
| the contract it sits under | [ADR-FETCHER](../docs/adr/0027_fetcher.md) — amended; it owns `src/fux/templates/` |
...
- **A missing `.fux/output.toml` falls back to engine defaults; a present but
  incomplete one still hard-errors** ([ADR-OUTPUT](../docs/adr/0054_output-defaults.md)
...
- **`ask` gained `sections`** ([ADR-OUTPUT](../docs/adr/0054_output-defaults.md)
...
- ⚠ **Found on the device, not in the mirror: the working tree changed
  ...
  touched. Fixed by cross-reference notes: [ADR-CLI](../docs/adr/0011_cli-surface.md)
  decision 11, [ADR-CONFIDENCE](../docs/adr/0052_confidence.md) decision 14 —
...
Records: [ADR-OUTPUT](../docs/adr/0054_output-defaults.md) decisions 20–21,
[ADR-DOTFUX](../docs/adr/0012_fux-directory.md) decision 6,
[ADR-ASK](../docs/adr/0013_ask.md) decision 8,
[ADR-CLI](../docs/adr/0011_cli-surface.md) decision 11,
[ADR-CONFIDENCE](../docs/adr/0052_confidence.md) decision 14. WORKLOG has the
...
✅ **[ADR-QUALITY](../docs/adr/0051_quality-contract.md) decision 12.**
...
Records: [ADR-RS](../docs/adr/0043_predictions.md) decision 15. Suite:
...
Records: [ADR-RS](../docs/adr/0043_predictions.md) decision 15,
[ADR-FETCHER](../docs/adr/0027_fetcher.md) decision 12,
[ADR-DOTFUX](../docs/adr/0012_fux-directory.md) decision 6. Suite: **2348
passed / 1 skipped.**
...
Records: [ADR-T1-ACCELERATOR](../docs/adr/0020_accelerator.md),
[ADR-ASK](../docs/adr/0013_ask.md), [ADR-OUTPUT](../docs/adr/0054_output-defaults.md),
[ADR-RS](../docs/adr/0043_predictions.md) decision 15. WORKLOG has the full trail.
...
**Evidence:** [ADR-FUXIGNORE](../docs/adr/0055_fuxignore.md) decision 11 and
11a–e; [ADR-INGEST](../docs/adr/0016_ingest.md) decisions 4 and 15, rewritten in
...
([ADR-FUXIGNORE](../docs/adr/0055_fuxignore.md) decision 4), so 274 frozen
...
**Evidence:** [ADR-INGEST](../docs/adr/0016_ingest.md) decision 15;
...
*how much it believes it* ([ADR-CONFIDENCE](../docs/adr/0052_confidence.md)).

**Evidence:** [ADR-PROVENANCE](../docs/adr/0053_provenance.md) (`proposed`);
...
[ADR-QUALITY](../docs/adr/0051_quality-contract.md) decision 11 — the record that
...
**Records:** [ADR-INGEST](../docs/adr/0016_ingest.md) decision 4 amended (it
owns `src/fux/ingest/`), [ADR-DOTFUX](../docs/adr/0012_fux-directory.md)
amended (`runtime/` gains a third derived file),
[ADR-CLI](../docs/adr/0011_cli-surface.md) annotated as describing-not-owning.
...
> the paper's **architectural** claims ([ADR-RS](../docs/adr/0043_predictions.md))
