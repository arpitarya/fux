...
[SR-LAW-0](../records/0002_LAW-0-authority.md) collapsed the sources to one, and
...
- **`acquired_max_bytes`** — named in SR-ACQUIRED's prose and in the ownership
...
is mechanised; semantics stay judgment, as SR-LAW-0 §"What is gated, and what is
not" says plainly.
"""
...
SR_TUNE = ROOT / "records" / "0135_tuning.md"
...
def test_an_undeclared_key_is_refused(tmp_path):
    """SR-CONFIG decision 14 — the W-140 row 8 defect. A typo used to parse."""
...
# -- SR-TUNE <-> tune.py's _SCHEMA ------------------------------------------


@pytest.fixture(scope="module")
def declared_tune() -> dict[str, list[str]]:
    return _declared(SR_TUNE)
...
def test_the_tune_specimen_only_names_declared_keys(declared_tune):
    """`tune.specimen()` is what `fux setup` writes, and SR-TUNE already names
    ...
    for table, body in data.items():
        ...
        for key in body:
            assert f"{table}.{key}" in declared, (
                f"tune.specimen() writes [{table}] {key}, which SR-TUNE does not declare"
            )
