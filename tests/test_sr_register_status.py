...
- **2026-08-22** — SR-ANSWER and SR-REFER both flipped `accepted` in
  `9f8366e` and only their record files were edited; the register still said
  `proposed` three days and two releases later.

The cost is not tidiness. `records/README.md` is what a session reads first,
...
SR = Path(__file__).resolve().parents[1] / "records"
REGISTER = SR / "README.md"
...
def _register_rows() -> list[tuple[int, str, str]]:
    ...
    for lineno, line in enumerate(REGISTER.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if not match:
            continue
        cells = line.split("|")
        # `| [0006](0105_answer.md) | NAME | what it decides | status | owns code? |`
...
def test_the_register_lists_every_record() -> None:
    ...
    on_disk = {p.name for p in SR.glob("[0-9][0-9][0-9][0-9]_*.md")}
