"""fux.embed.model vs the distill-exported reference (tokenizer parity etc.).

Ported from archive/v0.26/tests/test_embed.py. The `store.py` chunk-vector
cache is not ported (M1's `code` lives directly in each ledger record), so
this only covers `model.py`.
"""
...
REFERENCE = Path(__file__).parent.parent / "data" / "embed_reference.json"

pytestmark = pytest.mark.skipif(not DATA_PATH.is_file(), reason="model bundle not present")
...
def test_long_input_truncates(model):
    ...
    assert vec is not None  # no blow-up; policy recorded in archived ADR 0006
...
def test_embed_package_is_stdlib_only():
    src = Path(__file__).parent.parent.parent / "src" / "fux" / "embed"
    allowed_local = ("fux.", ".errors", ".model")
