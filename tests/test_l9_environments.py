"""L9 veto 1, made mechanical: no tool, test or script reads the sandbox.

[ADR-LAW-9](../docs/adr/0011_LAW-9-environments.md) gives each sibling
...
_SELF = "tests/test_l9_environments.py"
...
def test_no_code_under_the_four_roots_reaches_the_sandbox() -> None:
    ...
    assert not offenders, (
        "\n".join(offenders)
        + f"\n\nADR-LAW-9 veto 1: nothing under {'/ '.join(ROOTS)} reads that environment.\n"
