...
2. **A global rename walked into a title.** `SR-INGEST (0001)` became
   `SR-INGEST (SR-INGEST)` when a citation sweep matched the number inside
...
# The twelve keys, in order. `supersedes` and `ratifies` are optional and appear
...
#: `kind` selects which ADDITIONAL gate a record answers to; the ownership and
#: freshness gates apply to any record with a non-empty `owns`, whatever its kind.
#:
#: - ``law``       — wrong only when Arpit rules differently. The generated
#:                   `CLAUDE.md` block is held byte-equal to it
#:                   (`test_claude_md_laws.py`), and it outranks every other record.
#: - ``component`` — wrong when the code changes. `test_sr_freshness.py` and the
#:                   `commit-msg` hook require the owning record in the same change.
#: - ``process``   — wrong when the WAY work is done changes, with no code behind
#:                   it. Its enforcement is a test, so it owns that test.
#:
#: ⚠ **`kind` is written, not derived, and that is deliberate.** `law` is
#: derivable from the filename, but `component` and `process` are not
#: distinguishable from `owns` alone: SR-POSTINGS owns `tools/pruning-eval` and
#: is a component record; SR-WORK-QUALITY owns `tools/quality` and is a process one.
#: Identical shape, opposite kind — which is the whole argument for writing it.
KINDS = {"law", "component", "process"}
...
def test_required_keys_present_and_sane(path: Path) -> None:
    ...
    assert meta["type"] == "Standing Record", f"{path.name}: type must be Standing Record, got {meta['type']!r}"
    assert meta["kind"] in KINDS, (
        f"{path.name}: kind must be one of {sorted(KINDS)}, got {meta['kind']!r}. "
        "A record is `law` when only Arpit's ruling can make it wrong, `component` "
        "when a code change can, and `process` when the way work is done can."
    )
...
def test_keys_are_in_the_declared_order(path: Path) -> None:
    """A fixed order is what makes sixty-eight records diffable against each other."""
    meta = meta_of(path)
...
def test_the_h1_agrees_with_the_name(path: Path) -> None:
    ...
    assert m, f"{path.name}: body has no `# SR-…` heading"
