...
`records/README.md`'s ownership table is the single source of truth for who
...
component the table gives to someone else (SR-DOTFUX over-claimed `config.py`,
SR-URL-INGEST and SR-INDEX-LIFECYCLE both over-claimed a path already
...
from sr_lib import owner_of, ownership_table, register_names  # noqa: E402
from fux import frontmatter as fm  # noqa: E402


def _path_of(claim: str) -> str:
    """`src/fux/cli.py@8b8d6e4da20c` -> `src/fux/cli.py`.

    Since 2026-09-13 every claim carries the 12-hex content hash of what it
    owns, so the record and the code can be compared without reading either
    (SR-WORK-OWNERSHIP decision 13). **The hash is not part of the identity** — the
    register's ownership table names paths and nothing else, so it is stripped
    before the two are compared. A claim with no `@` is still read: this test
    fails on the wrong OWNER, and `test_sr_owns_hash.py` fails on a missing or
    stale hash. One failure, one cause.
    """
    claim = claim.strip("`").rstrip("/")
    path, sep, tail = claim.rpartition("@")
    return path if sep and len(tail) == 12 and all(c in "0123456789abcdef" for c in tail) else claim


def declared_owns() -> dict[str, list[str]]:
    """SR-NAME -> the `src/`/`tools/` paths its frontmatter claims, hashes stripped."""
    ...
    for name, path in register_names().items():
        ...
        if not isinstance(claims, list):
            continue  # test_sr_frontmatter.py's job
        result[name] = [_path_of(str(c)) for c in claims]
    return result
