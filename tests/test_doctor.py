...
def test_url_check_names_listed_urls_that_have_never_been_fetched(tmp_path):
    ...
    assert "https://a fetch=http decoder=prose" not in check.detail.split("never been fetched")[1], (
...
def test_pinned_row_names_the_urls_with_no_retained_bytes(tmp_path):
    ...
    *Disclosed, never refused* (Arpit, 2026-09-14) is about the **exit code**,
    ...
    """
    root = _pinned_repo(tmp_path, urls=("https://x.test/a fetch=http decoder=prose", "https://x.test/b fetch=http decoder=prose"))
...
def test_a_warn_row_renders_WARN_and_still_exits_zero(tmp_path, capsys):
    ...
    root = _pinned_repo(tmp_path, urls=("https://x.test/a fetch=http decoder=prose",))
