...
# touching its OWNING record. The same rule as `tests/test_sr_freshness.py`,
...
msg_file="${1:?sr-guard.sh must run as a commit-msg hook, invoked with the message file path as \$1 — see the install line at the top of this script}"
[ -f "$msg_file" ] || exit 0

# A line reading exactly "no SR affected" (or "[no-adr]") -> the rule is
# satisfied. Anchored to its own line, deliberately: a substring inside
# unrelated prose ("no SR affected the parser, only tests") must not exempt
# the commit by accident.
if grep -qiE '^[[:space:]]*(no[ -](adr|sr)[ -]affected|\[no-(adr|sr)\])[[:space:]]*$' "$msg_file"; then
  exit 0
fi
...
# SR-NAME<TAB>path, from the register's record-listing table. Every live link
# is relative to records/ (work/adr/ is empty as of this writing — the v0.30
# superseded-pending set finished migrating into records/); if a future link
...
while IFS= read -r f; do
  ...
  if [[ "$owner" == SR-* ]]; then
    while IFS=$'\t' read -r name link; do
      [ "$name" = "$owner" ] || continue
      case "$link" in
        */*) continue ;;  # not a bare records/ filename — unresolved, skip rather than guess
        *) record="records/$link" ;;
      esac
      break
    done <<<"$names"
  ...
  [ -n "$record" ] || continue  # owner does not resolve — test_sr_ownership.py's job, not this one
...
if [ -n "$hits" ]; then
  ...
  SR guard: this commit changes an SR-owned component and does not update
