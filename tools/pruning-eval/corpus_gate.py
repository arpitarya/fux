...
ADR-0017's lesson, promoted to a gate. The previous run reported a delta of
...
mistake ADR-0017 caught. This runs *before* any arm, and its output goes into
...
def main() -> int:
    ...
    print(f"\nGate: median ≥ {MEDIAN_GATE} distinct terms per document "
          f"(ADR-0017's lesson). '@6%' = terms a 6 % budget keeps for the "
          f"median document.")
