...
> pruning (ADR-0003). **Supersedes the MST keyspace + BIC wire verdicts
...
Also relied on: ADR-0003 (full postings required — pruning FAILED);
...
| **T2** | beyond | same records, `terms` → `tpack` (binary property); external-source shards only; partial clone | mmap byte-aligned segments (the old wire/runtime design, demoted to top tier) | ADR-0003 option E; archived wire-format numbers |
...
Authoritative field-by-field: [`../adr/0004-index-format.md`](../adr/0004-index-format.md)
...
- **B — MST keyspace + BIC bit-packed wire** (the prior verdict): smallest
  ...
  ADR-0003 killed. Survives as T2's internals.
...
property) · internal: [ADR-0003](../adr/0003-pruning-criterion-rerun.md)
