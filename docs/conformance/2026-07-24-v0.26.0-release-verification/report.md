...
Corpus: `/Users/arpitarya/my_programs/fux-lab/orbit/corpus` · 944 documents · **11 failure(s)** across 79 checks.
...
| surface | `fux ask` runs | ✅ PASS | exit 0 · cold 0.127s · warm 0.137s |
| surface | `fux find` runs | ✅ PASS | exit 0 · cold 0.129s · warm 0.121s |
| surface | `fux answer` runs | ✅ PASS | exit 0 · cold 0.128s · warm 0.131s |
| surface | `fux explain` runs | ✅ PASS | exit 0 · cold 0.110s · warm 0.108s |
| surface | `fux graph` runs | ✅ PASS | exit 0 · cold 0.108s · warm 0.108s |
| surface | `fux cat` runs | ✅ PASS | exit 0 · cold 0.035s · warm 0.034s |
...
| quality | fusion demoted a lexical top-5 hit (non-monotone) | ❌ FAIL | 1/6 zero-overlap pairs were in lexical's top-5 and dropped out under hybrid: How is inventory accuracy verified without shutting the ware |
...
| baseline | ingest_seconds | ✅ PASS | 0.5 → 0.49 (-2.0%, tolerance ±15%) |
| baseline | reingest_seconds | ✅ PASS | 0.32 → 0.31 (-3.1%, tolerance ±15%) |
| baseline | latency_ask_cold_seconds | ✅ PASS | 0.123 → 0.127 (+3.3%, tolerance ±15%) |
| baseline | latency_ask_warm_seconds | ✅ PASS | 0.125 → 0.137 (+9.6%, tolerance ±15%) |
| baseline | latency_find_cold_seconds | ✅ PASS | 0.118 → 0.129 (+9.3%, tolerance ±15%) |
| baseline | latency_find_warm_seconds | ✅ PASS | 0.119 → 0.121 (+1.7%, tolerance ±15%) |
| baseline | latency_answer_cold_seconds | ✅ PASS | 0.132 → 0.128 (-3.0%, tolerance ±15%) |
| baseline | latency_answer_warm_seconds | ✅ PASS | 0.127 → 0.131 (+3.1%, tolerance ±15%) |
| baseline | latency_explain_cold_seconds | ✅ PASS | 0.107 → 0.11 (+2.8%, tolerance ±15%) |
| baseline | latency_explain_warm_seconds | ✅ PASS | 0.108 → 0.108 (+0.0%, tolerance ±15%) |
| baseline | latency_graph_cold_seconds | ✅ PASS | 0.11 → 0.108 (-1.8%, tolerance ±15%) |
| baseline | latency_graph_warm_seconds | ✅ PASS | 0.127 → 0.108 (-15.0%, tolerance ±15%) |
| baseline | latency_cat_cold_seconds | ✅ PASS | 0.04 → 0.035 (-12.5%, tolerance ±15%) |
| baseline | latency_cat_warm_seconds | ✅ PASS | 0.039 → 0.034 (-12.8%, tolerance ±15%) |
...
| baseline | staleness_inversions | ❌ FAIL | 8 → 3 (-62.5%, tolerance ±15%) |
...
```json
{
  ...
  "ingest_seconds": 0.49,
  "latency_answer_cold_seconds": 0.128,
  "latency_answer_warm_seconds": 0.131,
  "latency_ask_cold_seconds": 0.127,
  "latency_ask_warm_seconds": 0.137,
  "latency_cat_cold_seconds": 0.035,
  "latency_cat_warm_seconds": 0.034,
  "latency_explain_cold_seconds": 0.11,
  "latency_explain_warm_seconds": 0.108,
  "latency_find_cold_seconds": 0.129,
  "latency_find_warm_seconds": 0.121,
  "latency_graph_cold_seconds": 0.108,
  "latency_graph_warm_seconds": 0.108,
...
2. **fusion demoted a lexical top-5 hit (non-monotone)** — 1/6 zero-overlap pairs were in lexical's top-5 and dropped out under hybrid: How is inventory accuracy verified without shutting the ware
  repro: `fux find '<question>' --json --top 5  # with and without --lexical-only`
3. **inversion — At what pick-face fill level does replenishment trig** — superseded `docs/runbooks/legacy-replenishment-10pct.md` (rank 1) outranked current `docs/guides/replenishment.md` (rank 2)
  repro: `fux find "At what pick-face fill level does replenishment trigger now?" --json --top 10`
4. **inversion — How many days does a customer have to start a return** — superseded `docs/guides/legacy-returns-30day.md` (rank 1) outranked current `docs/guides/returns.md` (rank 2)
  repro: `fux find "How many days does a customer have to start a return?" --json --top 10`
5. **inversion — What time does the final carrier manifest close?** — superseded `docs/runbooks/legacy-manifest-cutoff-1600.md` (rank 1) outranked current `docs/guides/carrier-cutoffs.md` (rank 2)
  repro: `fux find "What time does the final carrier manifest close?" --json --top 10`
6. **typed unanswerable questions decline (n=4)** — 0/4 declined cleanly via `answer`; `find` returned candidates for 4/4
  repro: `fux answer '<unanswerable q>' --json`
7. **fabrication — What is orbit's policy on paying warehouse staff in ** — answer="Dock doors were nailed one-per-carrier, so a late carrier's · 5 sources (want a clean decline)
  repro: `fux answer "What is orbit's policy on paying warehouse staff in cryptocurrency?" --json`
8. **fabrication — What is the uptime SLA percentage for the autonomous** — answer="Movable racks are carried by a fleet of AMRs to a small num · 3 sources (want a clean decline)
  repro: `fux answer "What is the uptime SLA percentage for the autonomous drone last-mile delivery fleet?" --json`
9. **fabrication — How do I configure the GraphQL subscription API for ** — answer='The endpoint operates on the dock within the inbound subsys · 5 sources (want a clean decline)
  repro: `fux answer "How do I configure the GraphQL subscription API for the customer mobile app?" --json`
10. **fabrication — Which vendor supplies the on-site staff cafeteria ca** — answer="Track order completion at the putwall, where every zone's s · 4 sources (want a clean decline)
  repro: `fux answer "Which vendor supplies the on-site staff cafeteria catering?" --json`
11. **staleness_inversions** — 8 → 3 (-62.5%, tolerance ±15%)
