# `docs/open/` — one file per open work item
...
2. When an item closes, **delete its file and its index row** in the same
   change as the work. The durable record of a closed item is its ADR plus
   the [`WORKLOG`](../WORKLOG.md) entry — not a tombstone here.
3. A file carries only what a builder needs to start: goal, definition of
   ...
   should execute it. Anything longer belongs in `PLAN.md`, an ADR, or a
   compare/proposal doc, and is linked from here.
4. `PLAN.md` remains the **spec**; these files are the **state** plus the
   execution notes that have no home in a spec.
