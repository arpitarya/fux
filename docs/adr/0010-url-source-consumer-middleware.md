...
- **Status:** proposed (built at Arpit's direct request in session, 2026-08-10;
  he ratifies by flipping this line)
- **Date:** 2026-08-10 · **amended 2026-08-11** by
  [ADR-0011](0011-fux-dir-layout.md), before ratification
...
> **Amendment (2026-08-11, ADR-0011).** Three placements decided here moved
...
1. **The adapter cap (git + HTTP + Confluence) is a decision**
   ([INTERVIEW.md](../INTERVIEW.md) non-relitigable #6; PLAN.md M4 —
   "more systems arrive via [the MCP proposal](../proposals/mcp-adapters.md),
   not code").
2. **Offline by default; stdlib-only runtime** (CLAUDE.md non-negotiables).
   `src/fux/` currently contains zero network code.
3. **ADR-0004 froze the schema** with `src: "git"` as the only exercised
...
[sources.url.config]                    # opaque to fux (ADR-0011)
cdp_port = 9222
```
...
the batch, and `configure(config)` (ADR-0011), called once after import with
the config table verbatim. Fux passes nothing else and imports nothing else —
transport is entirely the consumer's.

### Schema (extends ADR-0004; no frozen field changes shape)
...
- `title_h` is defined as `term_hash(title)` (16-hex blake2b) — decided
  here; ADR-0004 named the field but not its hash.
...
- ~~Owed: GLOSSARY entries ("url source", "middleware")~~ — paid by
  ADR-0011's change. `fux doctor` still says nothing about middleware health
