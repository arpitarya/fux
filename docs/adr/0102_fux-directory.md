...
type: ADR
name: ADR-DOTFUX
title: ADR-DOTFUX (0102) — the .fux/ directory
...
reached it still holds ([ADR-ACQUIRED](0147_acquired-plane.md)).
...
[ADR-ACQUIRED](0147_acquired-plane.md) decision 1.
...
| `sources/` | committed | `dirs` · `urls`, one entry per line, on the one grammar in [ADR-URL-LIST](0116_url-list.md). ⚠ **`types` lived here until 2026-09-11** and is `formats.toml` now (the row below) |
| `formats.toml` | committed | which files are documents and which decoder reads each extension — `include` plus `[decoders]` ([ADR-TYPES](0128_types-list.md) decisions 11–12). **Optional**: absent means the built-in default. Write-if-missing — a repo that already has the file keeps it, bindings and all; a repo with only the old `sources/types` has it **converted** by `fux setup`, never rewritten in place |
...
configuration. See [ADR-CACHEDIR-TAG](0121_cachedir-tag.md). ⚠ **For
...
| moment | writes | why |
|---|---|---|
| **`ensure_layout`**, at the head of every ingest | `.fux/README.md`, `.fux/.gitignore`, **`.fux/node/` + `.fux/fux`** — 🔴 **and NOTHING outside `.fux/`**: [ADR-NODE-SEARCH](0155_node-search.md) decision 15 wires `.fux/node` into the consumer's root manifest, and **that edit belongs to `fux setup` alone**, precisely because this row's own rule is that a no-op ingest produces a no-op diff. ✅ **Structural since 2026-09-12, not a convention**: `detect_workspace`/`wire_workspace` live in `setup.py`, and `ensure_layout(root, node_shape=...)` takes the decided shape as a keyword it never computes — an ingest keeps whatever shape is already on disk and can flip nothing | **mandatory and idempotent** — a fresh clone must be correct before a byte is written into the directory. The reader is here rather than in `setup` for the same reason: a clone with no Python has to be able to read the index the first time anyone ingests, not only after someone remembers to run `setup` |
...
existing repo through [ADR-PII](0150_pii.md) decision 17's refusal, which names
...
fresh repo** ([ADR-TYPES](0128_types-list.md) decision 10, amended). The template
...
left `sources/types` for `formats.toml` ([ADR-TYPES](0128_types-list.md) decision
...
[ADR-FETCHER](0117_fetcher.md) decisions 12–13 added two **optional** functions
...
`main` before it was caught.** [ADR-OUTPUT](0144_output-defaults.md) decision
...
([ADR-TUNE](0135_tuning.md) decision 13, [ADR-CONFIG](0113_config.md)
...
- ✅ **And `doctor` now NAMES it — fixed 2026-09-11**, in the change after the
  ...
  ([ADR-DOCTOR](0101_cli-surface.md) §`fux doctor`, which owns the row set).
...
[ADR-URL-LIST](0116_url-list.md) decision 14. Per this decision it reaches **new
...
([ADR-FETCHER](0117_fetcher.md) decision 1). One known consequence, accepted:
...
([ADR-CDP-FETCHER](0118_cdp-fetcher.md) decision 8). A decoder is stdlib-only
...
[ADR-AGENT-POLICY](0132_agent-policy.md) decisions 5 and 6 widen it: `setup`
...
since 2026-09-06 ([ADR-AGENT-POLICY](0132_agent-policy.md) decision 11). Everything else about the
...
([ADR-AGENT-POLICY](0132_agent-policy.md) decision 12). **A vendor whose only
...
([ADR-AGENT-POLICY](0132_agent-policy.md) decision 14). It is a *fourth*
...
- **Read-only, always.** A stale runner lock is named along with the command
  ...
  ([ADR-MAINTENANCE](0129_hooks.md) veto 7). A pending re-index is a
...
- **Offline, always.** The URL section reports how many `url:` records exist,
  ...
  fetches**, and it **never deletes**: [ADR-URL-INGEST](0107_url-ingest.md)
...
[ADR-MAINTENANCE](0129_hooks.md) owns. A check that *formats* another plane's
...
one people learn to skip. See [ADR-MAINTENANCE](0129_hooks.md) decision 12.
...
|---|---|
| `refusal rules` | a refusal count lived in one run's stderr; a rule matching everything empties the `url:` half of the corpus and leaves it looking like a corpus nobody ingested — [ADR-REFUSAL](0148_refusals.md) decision 11 |
| `decoder bindings` | `registry()` refuses a broken `decoder=` binding **on the next ingest**; and a binding on an extension no document has resolves perfectly and indexes nothing forever, which is deliberately not an ingest error — [ADR-DECODE](0139_decode.md) |
...
| `freshness verdicts` | the `as-ingested` share is the **veto check** of two accepted records ([ADR-ACQUIRED](0147_acquired-plane.md), [ADR-URL-FRESHNESS](0149_url-freshness.md)) and neither veto could be run at all |
| redaction counts, on `pii rules` | `redact()` returned them, `run()` summed them, and nothing read the sum — [ADR-PII](0150_pii.md) decision 15 |
...
- **Nothing in `tune.toml` outside `[index]` reaches the maintenance path.**
  ...
  change ([ADR-TUNE](0135_tuning.md) decision 13).
...
- The third category's own record — [ADR-ACQUIRED](0147_acquired-plane.md), and the two records that consume it, [ADR-REFUSAL](0148_refusals.md) and [ADR-URL-FRESHNESS](0149_url-freshness.md)
...
[ADR-FUXIGNORE](0145_fuxignore.md) ·
[ADR-URL-INGEST](0107_url-ingest.md) · [ADR-CONFIG](0113_config.md) ·
[ADR-URL-LIST](0116_url-list.md) · [ADR-FETCHER](0117_fetcher.md) ·
[ADR-CDP-FETCHER](0118_cdp-fetcher.md) ·
[ADR-CACHEDIR-TAG](0121_cachedir-tag.md) · [ADR-TYPES](0128_types-list.md) ·
[ADR-MAINTENANCE](0129_hooks.md) · [ADR-AGENT-POLICY](0132_agent-policy.md) ·
[ADR-TUNE](0135_tuning.md) · [ADR-ENRICH](0137_enrich.md) ·
[ADR-DECODE](0139_decode.md) · [ADR-ACQUIRED](0147_acquired-plane.md) ·
[ADR-REFUSAL](0148_refusals.md) · [ADR-URL-FRESHNESS](0149_url-freshness.md)
