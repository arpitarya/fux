...
type: ADR
name: ADR-INGEST
title: ADR-INGEST (0106) — how ingest works
...
([ADR-ASK](0103_ask.md) decision 9). What carry-forward saves today is the
...
[ADR-FUXIGNORE](0145_fuxignore.md) decision 11, in two delimited blocks,
...
([ADR-DOTFUX](0102_fux-directory.md)), so a fresh clone is correctly laid out
before the first byte lands.

**8. Ingest is offline.** The exceptions are the named fenced paths
([ADR-URL-INGEST](0107_url-ingest.md)); a plain run never imports the fetcher.
...
⚠ **`DEFAULT_TYPES` is unchanged by this.** [ADR-TYPES](0128_types-list.md)
...
source** ([ADR-ARCHIVED-CONTENT](0134_archived-content.md) decision 1). It reads
...
[ADR-ARCHIVED-CONTENT](0134_archived-content.md) decision 1a). Three things
...
([ADR-PII](0150_pii.md)). Redaction runs inside this file, between
...
| "run the rules over the whole file" | the frontmatter is deliberately outside the pass. It is stripped before indexing already ([ADR-ENRICH](0137_enrich.md) decision 8), so nothing in it reaches a committed term, and matching a `model:` value would refuse a file for text the index never sees |
...
body holds ([ADR-TUNE](0135_tuning.md) decision 13). Neither moves a document's
...
([ADR-ACQUIRED](0147_acquired-plane.md) decision 8, [ADR-CONFIG](0113_config.md)
...
retires nothing. [ADR-ENRICH](0137_enrich.md) decisions 17-18.

**A pinned URL is a `POLICY` skip, taken before the fetch and carried forward
after it.** `update=never` ([ADR-URL-LIST](0116_url-list.md) decision 14) filters
...
- ⚠ **A carried `url:` record is not re-extracted, so a pinned document's
  extraction is frozen too** — including its redaction. [ADR-PII](0150_pii.md)
...
- **`fux ingest --stop` and the runner takeover change nothing about what a run
  ...
  **never by reading the dirty list** — the list is advisory, and a run that
  ...
  [ADR-MAINTENANCE](0129_hooks.md) 1d and the surface is
  [ADR-CLI](0101_cli-surface.md).
...
- **`run()` takes an optional `progress`, `None` meaning silent.** Four phases
  ...
  to [ADR-CLI](0101_cli-surface.md); what binds here is that the phases report
...
- ⚠ **`fux ingest` now writes a committed file that is also its own input.**
  ...
  [ADR-FUXIGNORE](0145_fuxignore.md) decision 11.
...
- **The graph plane can no longer be handed a dangling edge by ingest**
  (decision 10). [ADR-GRAPH](0126_graph.md)'s `edges_from_records` lifts edges
...
- **A `hashed` URL record writes a second thing before it is eligible to
  ...
  rationale on [ADR-RECORD](0109_index-record.md).
...
- **Keep the already-reported set in `.fux/runtime/skipped`.** Rejected on
  ...
  [ADR-FUXIGNORE](0145_fuxignore.md) decision 11 and made loud by 11a.
...
[ADR-PII](0150_pii.md) decision 15, which owns redaction. **Two properties this
...
[ADR-DOTFUX](0102_fux-directory.md) · [ADR-ASK](0103_ask.md) ·
[ADR-URL-INGEST](0107_url-ingest.md) ·
[ADR-INDEX-LIFECYCLE](0108_index-lifecycle.md) ·
[ADR-RECORD](0109_index-record.md) · [ADR-EXTRACTED](0115_extracted-mode.md) ·
[ADR-GRAPH](0126_graph.md) · [ADR-TYPES](0128_types-list.md) ·
[ADR-MAINTENANCE](0129_hooks.md) ·
[ADR-ARCHIVED-CONTENT](0134_archived-content.md) ·
[ADR-DECODE](0139_decode.md)
