...
type: ADR
name: ADR-CONFIG
title: ADR-CONFIG (0022) — fux.toml and every property in it
...
`.fux/sources/dirs` ([ADR-DIR-LIST](0029_dir-list.md) decision 1).
...
declared `.fux/` layout ([ADR-DOTFUX](0011_fux-directory.md)). The default is
the plain-GET fetcher because a URL line carrying no `fetch=` means
`fetch=http` ([ADR-HTTP-FETCHER](0028_http-fetcher.md) decision 1).
...
`fux setup` is write-if-missing ([ADR-DOTFUX](0011_fux-directory.md)), so a
...
for ([ADR-AGENT-POLICY](0041_agent-policy.md) decision 5). ⚠ **`codex` joined
2026-09-06** ([ADR-AGENT-POLICY](0041_agent-policy.md) decision 11): the set is
...
| `[sources.url] middleware` | renamed to `fetcher`; move the file to `.fux/fetchers/` ([ADR-FETCHER](0026_fetcher.md) decision 7) |
| `[sources] dirs` | put one directory per line in `.fux/sources/dirs`; a line may carry `archived=true` ([ADR-DIR-LIST](0029_dir-list.md) decision 1) |
| `[ranking]` (whole table) | moved to `.fux/tune.toml`; run `fux setup` to write the file, move the keys across, delete the table ([ADR-TUNE](0044_tuning.md) decision 7) |
| `[dense]` (whole table) | **removed**, not relocated — the lane it configured no longer exists ([ADR-ASK](0012_ask.md) decision 9). The error states the removal, the verdict behind it, and that ranking does not move, because `mode` defaulted to `off` |
...
a source-wide *layer*, not a setting.** `keep` ([ADR-ACQUIRED](0056_acquired-plane.md)),
`ttl` ([ADR-URL-FRESHNESS](0058_url-freshness.md)) and `enrich`
([ADR-PII](0059_pii.md)) each sit between the built-in default and the URL
...
- **A third source-list path constant lives here, with no key at all.**
  ...
  nobody asked for. Decided in [ADR-TYPES](0037_types-list.md).
...
[ADR-ASK](0012_ask.md) · [ADR-FETCHER](0026_fetcher.md) ·
[ADR-HTTP-FETCHER](0028_http-fetcher.md) · [ADR-DIR-LIST](0029_dir-list.md) ·
[ADR-TYPES](0037_types-list.md) · [ADR-AGENT-POLICY](0041_agent-policy.md) ·
[ADR-ARCHIVED-CONTENT](0043_archived-content.md) · [ADR-TUNE](0044_tuning.md)
