...
type: ADR
name: ADR-FIND
title: ADR-FIND (0104) — the find verb
...
[ADR-ASK](0103_ask.md) decision 8's `§` lines are on `ask` alone: a heading in
this stream would be read as a filename. The same reasoning keeps the
`[archived]` marker off stdout ([ADR-DIR-LIST](0120_dir-list.md) decision 12) —
...
exists and is fresh ([ADR-ASK](0103_ask.md) decision 4).
...
refused for [ADR-EXPAND](0151_expand.md) decision 11's reason: it would make
`support` describe a retrieval depth the caller never asked for, where
[ADR-CONFIDENCE](0142_confidence.md) bounds it by `--top`. One consistent
...
phrase"* on the strength of not having looked — [ADR-RERANK](0138_rerank.md)
...
- The shared ranking it projects — [ADR-ASK](0103_ask.md).
...
[ADR-DIR-LIST](0120_dir-list.md) · [ADR-RERANK](0138_rerank.md) ·
[ADR-CONFIDENCE](0142_confidence.md) · [ADR-EXPAND](0151_expand.md)
