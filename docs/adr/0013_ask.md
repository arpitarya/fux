...
type: ADR
name: ADR-ASK
title: ADR-ASK (0013) — the ask verb
...
— the proximity reranker, [ADR-RERANK](0048_rerank.md) — is not a second
...
[ADR-RUNTIME-STATS](0035_runtime-stats.md)'s.
...
- **`answer` is the verb that cites lines**, because it fetched the bytes and
  cites the sha it read them at ([ADR-ANSWER](0015_answer.md)). The split is L2
...
- ⚠ **The cap of three is `ask`'s, and it is NOT the tunable** (2026-09-11).
  ...
  ([ADR-TUNE](0045_tuning.md) decision 13). **`MAX_HEADINGS` did not move and
...
- **No `--headings` flag, and `find`'s stdout is untouched.** `find` is the
  ...
  [ADR-DIR-LIST](0030_dir-list.md) decision 12 already made for the archived
...
([ADR-OUTPUT](0054_output-defaults.md) decision 21, which carries the full
...
([ADR-EXTRACTED](0025_extracted-mode.md) decision 9). A matching heading past
...
[ADR-PROVENANCE](0053_provenance.md) — `audit_record`, `derivation` and
...
[ADR-ANSWER](0015_answer.md) tells them to switch on `source`, and that `source`
...
| `--sections` / `--no-sections` | show or hide the matched `§ heading` lines — text and the `--json` `headings` field together. On by default; `[cli.ask] sections` sets it ([ADR-OUTPUT](0054_output-defaults.md) decision 21) |
...
| `--no-tune` | ignore `.fux/tune.toml` and answer on the engine's own defaults ([ADR-TUNE](0045_tuning.md) decision 11) |
| `--why` | the derivation: matched terms per document, the four gates, the cut line, and the rerank and tune deltas ([ADR-PROVENANCE](0053_provenance.md)) |

**12. `--why` is a SECOND QUERY, and never instrumentation of the first.**
[ADR-PROVENANCE](0053_provenance.md) decision 2 owns the argument; what binds
...
[ADR-CLI](0011_cli-surface.md) decision 3 carries the surface argument.
...
constants to `.fux/tune.toml` keys ([ADR-CONFIDENCE](0052_confidence.md)
decision 13, [ADR-TUNE](0045_tuning.md)), which means a bare `band: grounded`
...
- **`as-ingested` means *the source could not be reached, and the passage still
  ...
  ([ADR-REFER](0037_refer-plane.md) decision 19,
  [ADR-URL-FRESHNESS](0059_url-freshness.md)).
...
[ADR-ANSWER](0015_answer.md) decisions 11-13 carry them. **`cmd_ask`,
...
[ADR-EXPAND](0061_expand.md) carries both decisions.
...
[ADR-RANKING](0021_ranking.md)) — `true` when this result's rounded score
...
⚠ **`find` gained `--phrase`, `--under` and `--all`** ([ADR-FIND](0014_find.md)).
...
- **`rank()` carries an out-parameter, `stats_out`.** A caller may hand it a
  ...
  — which is what [ADR-CONFIDENCE](0052_confidence.md) builds its block from. It
...
- ⚠ **A multiplier applied in `rank()` alone is not enough, and this was a real
  ...
  **score** and not for the **candidate set**: the accelerator had already
  ...
  See [ADR-T1-ACCELERATOR](0020_accelerator.md) §The weighted bound. **Any
...
- **`query/refer_answer.py` and `query/headings.py` live under this record's
  ...
  [ADR-ANSWER](0015_answer.md)'s; `headings.py` is decision 8's, display-only.
...
- ⚠ **Directory-level ownership lets a change be discharged against the wrong
  ...
  [ADR-RANKING](0021_ranking.md), whose entire subject is that scorer, need
...
[ADR-FIND](0014_find.md) · [ADR-ANSWER](0015_answer.md) ·
[ADR-RECORD](0019_index-record.md) · [ADR-RANKING](0021_ranking.md) ·
[ADR-EXTRACTED](0025_extracted-mode.md) · [ADR-DIR-LIST](0030_dir-list.md) ·
[ADR-RUNTIME-STATS](0035_runtime-stats.md) ·
[ADR-T1-ACCELERATOR](0020_accelerator.md) · [ADR-TUNE](0045_tuning.md) ·
