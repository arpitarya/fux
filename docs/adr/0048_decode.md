...
type: ADR
name: ADR-DECODE
title: "ADR-DECODE (0048) — the decoder plane: bytes become Markdown in one place, and a consumer may bring a dependency fux may not"
...
[ADR-ENRICH](0046_enrich.md) decision 1's table gaining a third row:
...
[ADR-TYPES](0037_types-list.md) decision 1 for what stays out and why, **and for
...
- **There is no `.py.txt` template.** A fetcher needs one because it carries
  ...
  ([ADR-CDP-FETCHER](0027_cdp-fetcher.md) decision 9). **A decoder is
...
built-in reverses the SVG half of [ADR-TYPES](0037_types-list.md) decision 5
...
- Rendered to `.claude/skills/fux-decoder/` and `.kiro/skills/fux-decoder/`.
  **Never to an ambient surface**: it writes committed Python that changes what
  is indexed ([ADR-AGENT-POLICY](0041_agent-policy.md) decision 9a).
...
[ADR-ENRICH](0046_enrich.md) decision 4 derives scope from a declaration; a
...
that owns the grammar is [ADR-TYPES](0037_types-list.md) decision 11, and this
...
[ADR-TYPES](0037_types-list.md) decision 11a carries the table and the argument
...
- **A decoded document can be cited for the first time.** `refer/__init__.py`
  ...
  `extract.py` and nothing else. See [ADR-REFER](0036_refer-plane.md)
  decision 23.
...
- ⚠ **A consumer decoder can break L4 and no gate reaches it.** An import fence
  ...
  [ADR-ENRICH](0046_enrich.md) decision 3 owns about `model:` being a claim fux
...
- [`tests/decode/test_decode.py`](../../tests/decode/test_decode.py) — in
  ...
  `PYTHONHASHSEED`.
- [ADR-ENRICH](0046_enrich.md) decision 1 — the table this record extends, and
  the precedent that a consumer boundary is a design choice rather than a
  dependency budget; [ADR-TYPES](0037_types-list.md) — what the widened default
  admits and what it still refuses.
- The seam it changed in ingest — [ADR-INGEST](0015_ingest.md) decision 11; the
  fetchers that stopped converting —
  [ADR-HTTP-FETCHER](0028_http-fetcher.md) and
...
3. **The default type allowlist is ever derived from the LIVE registry** rather
   ...
   ([ADR-TYPES](0037_types-list.md) decision 1a).
...
**Records** — [ADR-LAWS](0001_laws.md) · [ADR-INGEST](0015_ingest.md) ·
[ADR-EXTRACTED](0024_extracted-mode.md) ·
[ADR-CDP-FETCHER](0027_cdp-fetcher.md) ·
[ADR-HTTP-FETCHER](0028_http-fetcher.md) · [ADR-TYPES](0037_types-list.md) ·
[ADR-AGENT-POLICY](0041_agent-policy.md) · [ADR-ENRICH](0046_enrich.md)
