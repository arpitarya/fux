...
type: ADR
name: ADR-OUTPUT
title: ADR-OUTPUT (0053) — output defaults are configurable, in a third file
...
[ADR-CONFIDENCE](0051_confidence.md) decision 11, `--band` off. A consumer who
...
- Fux already has two config files with two different boundaries
  ([ADR-CONFIG](0022_config.md), [ADR-TUNE](0044_tuning.md)), so the question
...
11. ⚠ **`[mcp]` is the table this record exists for.** It carries **`top`
    ...
    **The first draft of this decision carried `band` in `[mcp]` and it was
    wrong.** [ADR-CONFIDENCE](0051_confidence.md) decision 11 makes the MCP
...
20. ⚠ **A MISSING FILE FALLS BACK; A PRESENT ONE STILL RULES. Ruled by Arpit,
    ...
    ([ADR-DOTFUX](0011_fux-directory.md) decision 6), so `fux setup` writes it
...
21. **`ask` gained `sections`, and it reaches BOTH renderings. Ruled by
    ...
    *"this fux is too old"*. **`[]` still means nothing matched** — that is
    ...
    [ADR-CONFIDENCE](0051_confidence.md) decision 11's shape for `confidence`
...
([ADR-ACQUIRED](0056_acquired-plane.md), [ADR-URL-FRESHNESS](0058_url-freshness.md)).
...
([ADR-CLI](0010_cli-surface.md) decision 2a, [ADR-ENRICH](0046_enrich.md)
...
[ADR-CONFIDENCE](0051_confidence.md) decision 12's outcome. **No ordering, no
...
`doc_coverage_floor` before the block is built ([ADR-CONFIDENCE](0051_confidence.md)
...
rule [ADR-ASK](0012_ask.md) decision 11 states for this schema.
...
[ADR-EXPAND](0060_expand.md) decision 9), present only when more than one `-q`
...
- ⚠ **Measured, 2026-08-28, against a local mirror — the CLI seam's own
  ...
  not.
- [ADR-TUNE](0044_tuning.md) — the precedent this follows in structure
  (committed, optional, closed key set, no writer, an `enabled=False` escape
  hatch) and departs from in **boundary**.
- [ADR-CONFIDENCE](0051_confidence.md) decision 11 — the accepted cost this
...
- [ADR-TUNE](0044_tuning.md) · [ADR-CONFIG](0022_config.md) ·
  [ADR-CONFIDENCE](0051_confidence.md) · [ADR-MCP](0045_mcp.md) ·
  [ADR-CLI](0010_cli-surface.md) · [ADR-FUX-DIR](0011_fux-directory.md)
