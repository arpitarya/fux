...
type: ADR
name: ADR-GRAPH
title: "ADR-GRAPH (0126) — the graph lane: communities, PPR-lite, and three relational verbs"
...
[ADR-CLI](0101_cli-surface.md)'s constraint is that there is none.
...
mode* name ([ADR-EXTRACTED](0115_extracted-mode.md) decision 5).
...
- **`edges_from_records` lifts without validating, and that is safe only because
  ...
  from it. **Fixed in ingest, not here** ([ADR-INGEST](0106_ingest.md)
...
**Records** — [ADR-CLI](0101_cli-surface.md) · [ADR-ASK](0103_ask.md) ·
[ADR-INGEST](0106_ingest.md) · [ADR-EXTRACTED](0115_extracted-mode.md) ·
