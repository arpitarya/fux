...
title: ADR-INDEX-LIFECYCLE (0017) — how the index is generated and updated
...
2. **`analyzer` is untouched by construction.** It versions how text becomes
   *terms*; a display field is not a term — it never enters `terms`, never
   ...
   ([ADR-RANKING](0020_ranking.md)). A display field cannot make a corpus's
...
[ADR-RECORD](0018_index-record.md). **Nothing compared them.** Adding a display
...
[ADR-FETCHER](0026_fetcher.md), so a record-shape file put there would have been
...
5 already covers it. Full rationale on [ADR-RECORD](0018_index-record.md).
...
- **Merges land per document.** Two branches editing different documents touch
  ...
  ([ADR-MERGE-DRIVER](0039_merge-driver.md)); decision 4's write-if-different
...
- **Committed index size is measured, never gated.** A packed-size promise at
  ...
  [ADR-POSTINGS](0021_postings.md)'s designed encoding, which is unbuilt. Read
...
[ADR-POSTINGS](0021_postings.md) · [ADR-FETCHER](0026_fetcher.md) ·
[ADR-MAINTENANCE](0038_hooks.md) · [ADR-MERGE-DRIVER](0039_merge-driver.md)
