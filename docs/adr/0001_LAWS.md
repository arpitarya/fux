...
2026-09-06 — the same day decision 1 was written. Under L0 the **records** are the
source of truth and `CLAUDE.md` is a pointer; the migration is
[W-122](../../work/open/W-122-adrs-are-the-source.md), and until it lands the
normative text is still where decision 1 put it. **Read L0 before acting on
decision 1.**

⚠ **A record in the third column is RATIONALE, never the law.** It carries why
the law exists, what it has cost, how its wording has moved, and what would
reopen it. **The normative text is still `CLAUDE.md` and only `CLAUDE.md`** —
decision 1 is unchanged, and decision 3 binds these nine records exactly as it
binds every other.
...
⚠ **What the split does NOT do.** It does **not** move the normative text.
Decision 1 stands: `CLAUDE.md` §Non-negotiable constraints is the single home,
and decision 3 forbids the eight new records from restating a law exactly as it
forbids every other record. **They explain; they never state.** A session that
reads `ADR-LAW-3` and acts on its prose instead of the law has made the mistake
...
grep -rn 'FOSS-only\|OSI-approved licence' docs/adr/ | grep -v '0001_LAWS.md\|000[2-9]_l'
# expect: no output
