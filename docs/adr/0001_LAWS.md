...
status: accepted
date: 2026-08-18
feature: the naming of the laws (L0–L9), and the cross-cutting modules no single plane can own
owns: [src/fux/__init__.py, src/fux/errors.py, src/fux/schema.py, src/fux/frontmatter.py]
laws: [L1, L2, L3, L4, L5, L6, L7, L8, L9]
...
Fux has ten rules no feature may break. **Each one is stated in its own record**
— `ADR-LAW-0` … `ADR-LAW-9`, the table below — and
[`CLAUDE.md` §Non-negotiable constraints](../../CLAUDE.md) carries a **generated**
copy of all ten, held byte-equal by
...
`CLAUDE.md` §Non-negotiable constraints is a **generated** view of all ten,
...
```mermaid
flowchart TD
    N["ADR-LAWS<br/>(this record — the handles L0..L9)"]
...
```text
                    ADR-LAWS          <-- this record: the handles L0..L9
                        |
        +---------------+---------------+---------------+
        v               v               v               v
...
record**, and `CLAUDE.md` carries a generated, test-bound view of all ten. The
...
**2. This record assigns the stable handles L1–L9** (L0 since 2026-09-06, L9 since 2026-09-11) so a decision can say which
...
the only fallback. **Hoisting ten laws into one record is the shape the split of
...
# expect exactly the ten docs/adr/*_LAW-*.md records. A hit anywhere else is a
