...
status: accepted
date: 2026-08-18
feature: the naming of the laws (L0–L10), and the cross-cutting modules no single plane can own
...
Fux has eleven rules no feature may break. **Each one is stated in its own record**
— `ADR-LAW-0` … `ADR-LAW-10`, the table below — and
[`CLAUDE.md` §Non-negotiable constraints](../../CLAUDE.md) carries a **generated**
copy of all eleven, held byte-equal by
...
numbering and nothing else — read the law itself in its own `ADR-LAW-n` record.

| # | law (one-line handle — **not** the normative text) | its record |
|---|---|---|
| **L0** | ADRs are the only source of truth; the Law records outrank every other record | [ADR-LAW-0](0002_LAW-0-authority.md) |
| **L1** | `$0`, FOSS-only — OSI-approved licences, shipped packaged | [ADR-LAW-1](0003_LAW-1-zero-cost.md) |
| **L2** | Content is never durable outside its source system | [ADR-LAW-2](0004_LAW-2-content-never-durable.md) |
| **L3** | Deterministic — no model in the maintenance path | [ADR-LAW-3](0005_LAW-3-deterministic.md) |
| **L4** | Offline by default | [ADR-LAW-4](0006_LAW-4-offline-by-default.md) |
| **L5** | Hashed meta is the default for non-git sources | [ADR-LAW-5](0007_LAW-5-hashed-meta.md) |
| **L6** | Say "index", not "db" | [ADR-LAW-6](0008_LAW-6-say-index.md) |
| **L7** | Python ≥ 3.11 | [ADR-LAW-7](0009_LAW-7-python-311.md) |
| **L8** | A use record is never committed | [ADR-LAW-8](0010_LAW-8-use-record.md) |
| **L9** | Each sibling environment has one job — playground for Arpit's hands, lab for every measurement, benchmark for speed and ranked lists | [ADR-LAW-9](0011_LAW-9-environments.md) |
| **L10** | The consumer is served build output, never source — decoders and fetchers excepted | [ADR-LAW-10](0012_LAW-10-bundled-output.md) |

🔴 **Decision 1 is SUPERSEDED by [ADR-LAW-0](0002_LAW-0-authority.md)**, accepted
...
`CLAUDE.md` §Non-negotiable constraints is a **generated** view of all eleven,
...
```mermaid
flowchart TD
    N["ADR-LAWS<br/>(this record — the handles L0..L10)"]
    W["ADR-LAW-0 … ADR-LAW-9<br/>(each STATES one law)"]
...
```text
                    ADR-LAWS          <-- this record: the handles L0..L10
                        |
        +---------------+---------------+---------------+
        v               v               v               v
  ADR-LAW-0 .. ADR-LAW-9         ADR-RECORD   ADR-T1-ACCELERATOR  ADR-URL-INGEST
   (each STATES one law)              :               :               :
        |                             +......... never restates ......+
...
🔴 **SUPERSEDED by [ADR-LAW-0](0002_LAW-0-authority.md) decision 1** (Arpit,
2026-09-06; migrated 2026-09-12). **Each law is stated in its own `ADR-LAW-n`
record**, and `CLAUDE.md` carries a generated, test-bound view of all eleven. The
...
**2. This record assigns the stable handles L1–L10** (L0 since 2026-09-06, L9 since 2026-09-11, L10 since 2026-09-12) so a decision can say which
law binds it without quoting it.

**3. No record but the law's own `ADR-LAW-n` may state a law.** A record bound by
...
**4. Changing a law is a change to its `ADR-LAW-n` record, plus this table when
the handle moves, plus `python scripts/gen-laws.py --write`** — all in the same
commit, and only on Arpit's ruling named in the record (ADR-LAW-0 decision 3).
...
belongs to [ADR-FETCHER](0117_fetcher.md) — a record with nothing to say about
the record shape.
...
[ADR-FETCHER](0117_fetcher.md)'s `MAX_PARALLEL` (declared, never detected,
...
[ADR-PROVENANCE](0143_provenance.md) decision C rules that **`fux verify` never
...
prohibition already existed, in [ADR-QUALITY](0141_quality-contract.md) decision
...
[ADR-PROVENANCE](0143_provenance.md) is the record that does.
...
`ADR-LAW-3` and acting on its §2 law block is doing the right thing; before
...
[ADR-LAW-8](0010_LAW-8-use-record.md) carries the same material as its own subject.
...
[ADR-LAW-1](0003_LAW-1-zero-cost.md); this row exists so a reader of the table above
...
- **Every version bump opens this record**, because owning
  ...
  artefact. `__version__` is `2.0.0-alpha.7`.
...
- ⚠ **The veto check below cannot catch the failure this record exists to
  ...
  **narrower claim standing in for a law** — text that was true when written,
  ...
  named paths live in [ADR-CLI](0101_cli-surface.md) decision 1e, which cites
...
- L8's subject in code — [`src/fux/maintain/lastcited.py`](../../src/fux/maintain/lastcited.py),
  ...
  the law question: [ADR-QUALITY](0141_quality-contract.md) decision 11. The
  second is [`src/fux/query/provenance.py`](../../src/fux/query/provenance.py),
  owned by [ADR-PROVENANCE](0143_provenance.md), which exists because of the
...
the only fallback. **Hoisting eleven laws into one record is the shape the split of
...
is a change to ADR-LAW-0 decision 5 and needs Arpit's ruling.
...
# expect exactly the eleven docs/adr/*_LAW-*.md records. A hit anywhere else is a
...
**Records** — [ADR-CLI](0101_cli-surface.md) · [ADR-FETCHER](0117_fetcher.md) ·
[ADR-QUALITY](0141_quality-contract.md) · [ADR-PROVENANCE](0143_provenance.md) ·
