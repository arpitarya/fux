...
belongs to [ADR-FETCHER](0027_fetcher.md) — a record with nothing to say about
the record shape.
...
[ADR-FETCHER](0027_fetcher.md)'s `MAX_PARALLEL` (declared, never detected,
...
[ADR-PROVENANCE](0053_provenance.md) decision C rules that **`fux verify` never
...
prohibition already existed, in [ADR-QUALITY](0051_quality-contract.md) decision
...
[ADR-PROVENANCE](0053_provenance.md) is the record that does.
...
- ⚠ **The veto check below cannot catch the failure this record exists to
  ...
  **narrower claim standing in for a law** — text that was true when written,
  ...
  named paths live in [ADR-CLI](0011_cli-surface.md) decision 1e, which cites
...
- L8's subject in code — [`src/fux/maintain/lastcited.py`](../../src/fux/maintain/lastcited.py),
  ...
  the law question: [ADR-QUALITY](0051_quality-contract.md) decision 11. The
  second is [`src/fux/query/provenance.py`](../../src/fux/query/provenance.py),
  owned by [ADR-PROVENANCE](0053_provenance.md), which exists because of the
...
**Records** — [ADR-CLI](0011_cli-surface.md) · [ADR-FETCHER](0027_fetcher.md) ·
[ADR-QUALITY](0051_quality-contract.md) · [ADR-PROVENANCE](0053_provenance.md) ·
