...
title: "ADR-HTTP-FETCHER (0119) — the default HTTP fetcher"
...
`fetch=cdp` on that line. [ADR-FETCHER](0117_fetcher.md) decision 5 is the rule;
...
([ADR-URL-LIST](0116_url-list.md) decision 10).
...
([ADR-CONFIG](0113_config.md) decision 7a), because *what is safe for this
...
- ⚠ **The key is spelled the same in `cdp.py`, and it has to be.**
  ...
  ([ADR-FETCHER](0117_fetcher.md) decision 8) and this file's `configure()`
  ...
  [ADR-CDP-FETCHER](0118_cdp-fetcher.md) decision 4 carries the same rule from
  the other side.
...
- **Below 1 is refused, not clamped** — `_positive_int` names the key in the
  ...
  [ADR-CONFIG](0113_config.md): a silent clamp to 1 honours a number the
...
bracket ([ADR-FETCHER](0117_fetcher.md) decision 2).
...
- **Two shipped fetchers exist**, so `.fux/fetchers/` is genuinely plural and
  [ADR-FETCHER](0117_fetcher.md) decision 6 stops being anticipatory.
...
- **A chained fetcher list.** Rejected: it contradicts
  [ADR-FETCHER](0117_fetcher.md) decision 4, and it is where the adapter cap
...
- The contract it implements — [ADR-FETCHER](0117_fetcher.md); the shipped
...
- The scaffolding mechanism it reuses —
  ...
  [ADR-DOTFUX](0102_fux-directory.md) decision 6.
