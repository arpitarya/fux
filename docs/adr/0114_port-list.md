...
type: ADR
name: ADR-PORT-LIST
title: ADR-PORT-LIST (0114) — port, don't rewrite, and this is the complete list
...
| analyzer chain | ingest, query | the shared analyzer ([ADR-RANKING](0111_ranking.md)) |
| BM25F scoring math + exact-`df` discipline | the scorer | the scan, then the accelerator |
| PPR-lite + edge extraction | the graph lane | [ADR-GRAPH](0126_graph.md) |
| chunker (heading-aware) | passage re-score | [ADR-REFER](0127_refer-plane.md) |
| converters (fidelity tiers) | bytes → Markdown | [ADR-DECODE](0139_decode.md) |
| CLI verb surface | the UX contract | [ADR-CLI](0101_cli-surface.md) |
...
[ADR-REFER](0127_refer-plane.md) · [ADR-DECODE](0139_decode.md)
