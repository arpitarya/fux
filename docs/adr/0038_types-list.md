...
([ADR-DECODE](0049_decode.md)) that emits keys as headings and drops ids,
...
directions** ([ADR-FUXIGNORE](0055_fuxignore.md) decision 4). A path
...
[ADR-DIR-LIST](0030_dir-list.md) still leans on that reading for `fux add`.
...
the `Content-Type` its fetcher declared ([ADR-FETCHER](0027_fetcher.md)
...
[ADR-DECODE](0049_decode.md) decision 5. Two people with different
...
in [ADR-DECODE](0049_decode.md) applied one layer up, for the same L3 reason.
...
- **It does not replace exclusion.** An `evidence/report.md` is still prose in a
  ...
  [`.fux/.fuxignore`](0055_fuxignore.md) rather than in `!` lines here.
...
- **Content sniffing.** ⚠ **Disqualified, not merely rejected.** Deciding
  ...
  ([ADR-HTTP-FETCHER](0029_http-fetcher.md) decision 3) decides what a document
  *is*.
...
**Records** — [ADR-INGEST](0016_ingest.md) · [ADR-URL-LIST](0026_url-list.md) ·
[ADR-FETCHER](0027_fetcher.md) · [ADR-HTTP-FETCHER](0029_http-fetcher.md) ·
[ADR-DIR-LIST](0030_dir-list.md) · [ADR-DECODE](0049_decode.md) ·
