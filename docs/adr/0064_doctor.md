...
type: ADR
name: ADR-DOCTOR
title: "ADR-DOCTOR (0064) — what `fux doctor` checks, and why each row is a warning or an error"
...
([ADR-OWNERSHIP](0056_ownership.md) decision 6: *if a record's subject IS a
...
[ADR-OWNERSHIP](0056_ownership.md) veto 2 by construction.
...
| `repo root` | error | `find_root` — is this a fux repo at all | [ADR-DOTFUX](0012_fux-directory.md) |
| `fux.toml loads` | **error** | does the loader accept the config (decision 4) | [ADR-CONFIG](0023_config.md) |
| `.fux/ writable` | error | the directory can be created and written | [ADR-DOTFUX](0012_fux-directory.md) |
| `index not gitignored` | error | a `.fux/*` blanket silently eating the committed index | [ADR-DOTFUX](0012_fux-directory.md) |
| `.fux/ layout declared` | warn | undeclared entries at `.fux/`'s top level | [ADR-DOTFUX](0012_fux-directory.md) |
| `pii rules` | **error** when absent | a missing `.fux/pii.toml`; otherwise compiles every pattern offline and states the scope. ⚠ It cannot see an over-broad rule and says so — only [`tools/pii-probe/`](../../tools/pii-probe/) can | [ADR-PII](0060_pii.md) decision 17 |
| `acquired plane` | warn, **error** on gitignore | blob count, total bytes, the 80 %-of-cap warning, and the gitignore assertion | [ADR-ACQUIRED](0057_acquired-plane.md) |
| `refusal rules` | warn | how many rules load, how many responses each has refused, and **the rules that have never fired** — what a typo'd condition looks like | [ADR-REFUSAL](0058_refusals.md) decision 11 |
| `decoder bindings` | warn | the one binding fault no ingest can catch: a `[decoders]` binding on an extension **no indexed document has** | [ADR-DECODE](0049_decode.md) |
| `recency prior` | warn | whether any document carries an `mtime` — a corpus copied out of its git repository loses every one | [ADR-INGEST](0016_ingest.md) |
| `freshness verdicts` | warn | `freshness_counts` and `AS_INGESTED_VETO_SHARE` — the veto instrument, shared verbatim with ADR-ACQUIRED's identical one so the quarter has one home | [ADR-URL-FRESHNESS](0059_url-freshness.md) |
| `ranking priors` | warn | every prior that is wired, reads its input and multiplies by one — **and the count of documents it would have acted on**. It refuses to recommend a value | [ADR-ARCHIVED-CONTENT](0044_archived-content.md) · [ADR-TUNE](0045_tuning.md) |
| `output.toml present` | warn | absent means every output default is the engine's own and none can be changed | [ADR-OUTPUT](0054_output-defaults.md) decision 20 |
...
([ADR-MAINTENANCE](0039_hooks.md) decision 1c, veto 7).
...
[ADR-CLI](0011_cli-surface.md), which owns the verb surface and the promotion
rule.
...
**Records** — [ADR-LAWS](0001_LAWS.md) · [ADR-CLI](0011_cli-surface.md) ·
[ADR-DOTFUX](0012_fux-directory.md) · [ADR-INGEST](0016_ingest.md) ·
[ADR-T1-ACCELERATOR](0020_accelerator.md) · [ADR-CONFIG](0023_config.md) ·
[ADR-URL-LIST](0026_url-list.md) · [ADR-FETCHER](0027_fetcher.md) ·
[ADR-FUXIGNORE](0055_fuxignore.md) · [ADR-TYPES](0038_types-list.md) ·
[ADR-MAINTENANCE](0039_hooks.md) ·
[ADR-ARCHIVED-CONTENT](0044_archived-content.md) · [ADR-TUNE](0045_tuning.md) ·
[ADR-DECODE](0049_decode.md) · [ADR-OUTPUT](0054_output-defaults.md) ·
[ADR-OWNERSHIP](0056_ownership.md) ·
[ADR-ACQUIRED](0057_acquired-plane.md) · [ADR-REFUSAL](0058_refusals.md) ·
[ADR-URL-FRESHNESS](0059_url-freshness.md) · [ADR-PII](0060_pii.md)
