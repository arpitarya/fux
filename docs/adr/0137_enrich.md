...
type: ADR
name: ADR-ENRICH
title: "ADR-ENRICH (0137) — enrichment as an agent skill, not an API call"
...
scope docs/adr (enrich=true)
  docs/adr/0111_ranking.md      sha 3f8a1c2d9b04…   9 chunks   MISSING
...
[ADR-FETCHER](0117_fetcher.md)'s pattern applied to a second boundary:
...
[ADR-AGENT-POLICY](0132_agent-policy.md) veto 5b.
...
[ADR-PII](0150_pii.md) decision 1 covers both. The two halves are handled
...
- **`fux enrich --check` refuses and never rewrites.** The file is prose a
  ...
  ([ADR-MAINTENANCE](0129_hooks.md) veto 7), and stronger here for that reason.
  ...
  [ADR-PII](0150_pii.md) decision 7, and says to rewrite the sentence instead.
...
⚠ **[ADR-EXTRACTED](0115_extracted-mode.md) lost its counterpart record**, so
...
with [ADR-EXTRACTED](0115_extracted-mode.md); the pair was one call.
...
neither way ([ADR-QUALITY](0141_quality-contract.md) decision 2a).
...
| **retirement / supersession flags** | nothing in the bytes distinguishes a live document from a retired one | if it reorders rather than annotates, it violates the ruling [ADR-ARCHIVED-CONTENT](0134_archived-content.md) already reached |
| **richer embeddings** | fux computes no vectors at all ([ADR-ASK](0103_ask.md) decision 9) | **L1 collision** — a larger or API-served model may be *called once and pinned*, never imported into the runtime |
...
see [ADR-REFER](0127_refer-plane.md) decision 18 and
...
([ADR-PII](0150_pii.md), 2026-09-01). `enrich=` joins the URL list's attribute
...
- **`ENRICH-SKILL.md` is exempt from the policy-agreement check.** It is a
  **procedure**, not a rendering of the archived-results policy, and the
  ...
  widened ([ADR-AGENT-POLICY](0132_agent-policy.md) decision 2a).
- ⚠ **Who may *author* an enrichment, when it is being MEASURED, is not this
  ...
  **measurement-protocol** rule, living in [ADR-RS](0133_predictions.md)
...
- The contract this inherits — ADR-ENRICHED; the scope
  grammar — [ADR-DIR-LIST](0120_dir-list.md); the field it feeds —
  [ADR-RANKING](0111_ranking.md) decision 1.
- The authorship rule that governs *measuring* enrichment —
  [ADR-RS](0133_predictions.md) decisions 11–15.
...
**Records** — [ADR-LAWS](0001_LAWS.md) · [ADR-RANKING](0111_ranking.md) ·
ADR-ENRICHED · [ADR-FETCHER](0117_fetcher.md) ·
[ADR-DIR-LIST](0120_dir-list.md) · [ADR-AGENT-POLICY](0132_agent-policy.md) ·
[ADR-RS](0133_predictions.md) · [ADR-TUNE](0135_tuning.md)
