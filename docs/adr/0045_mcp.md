...
type: ADR
name: ADR-MCP
title: "ADR-MCP (0045) — the MCP server, and a tool surface deliberately smaller than the CLI"
...
- **An agent acts on a citation by opening a file at a line**, which is why the
  citation format is `path:L12-L40` ([ADR-ANSWER](0014_answer.md) decision 9).
...
- **It is the same selection `fux ask` renders, from the same function** —
  `query/headings.py::headings_for`, owned by [ADR-ASK](0012_ask.md)
...
the agent to read it first** ([ADR-CONFIDENCE](0051_confidence.md)). An agent
...
[ADR-OUTPUT](0053_output-defaults.md)'s `[mcp] top` made the engine use whatever
...
[ADR-EXPAND](0060_expand.md)). It is the same slot as the CLI's `--expand`, at
...
- ✅ **The advertised `k` default is now the RESOLVED `[mcp] top`, closing a
  ...
  [ADR-OUTPUT](0053_output-defaults.md)'s chain resolved, replacing a
...
- ⚠ **The MCP surface is NOT reached by the 2026-08-28 `output.toml`
  ...
  [ADR-CLI](0010_cli-surface.md) §Consequences). **That is luck of the code
...
- **`fux mcp` is a verb, not a flag.** It is a long-running server, not a query
  — [ADR-CLI](0010_cli-surface.md) decision 1g.
...
- Decision 9's selection, shared with the CLI —
  ...
  for heading level over line level — [ADR-ASK](0012_ask.md) decision 8.
- Decision 10's signals — [ADR-CONFIDENCE](0051_confidence.md).
- The verb's shape on the CLI — [ADR-CLI](0010_cli-surface.md) decision 1g.
...
[ADR-REFER](0036_refer-plane.md) · [ADR-CONFIDENCE](0051_confidence.md)
