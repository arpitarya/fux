...
type: ADR
name: ADR-RANKING
title: ADR-RANKING (0021) — how documents are scored and ordered
...
([ADR-ENRICH](0047_enrich.md)); folding it into `body` would make it
...
([ADR-TUNE](0045_tuning.md)). An unconfigured corpus scores byte-identically to
...
([ADR-RERANK](0048_rerank.md)) — and two limits keep it from being a hole:
...
(W-109, [ADR-EXPAND](0061_expand.md) decision 2): a `term_weights` map applied
...
- **Ranking changes are expensive by design.** Any change to the parameters, the
  ...
  ([ADR-T1-ACCELERATOR](0020_accelerator.md) §The weighted bound), which is what
...
- ⚠ **`superseded_weight` is a shipped knob whose only measured non-neutral
  ...
  queries is below [ADR-RS](0043_predictions.md)'s resolution floor. The
...
[ADR-EXTRACTED](0025_extracted-mode.md) · [ADR-RS](0043_predictions.md) ·
[ADR-TUNE](0045_tuning.md) · [ADR-ENRICH](0047_enrich.md) ·
[ADR-RERANK](0048_rerank.md)
