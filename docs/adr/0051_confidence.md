...
type: ADR
name: ADR-CONFIDENCE
title: ADR-CONFIDENCE (0051) — how much the index believes its own answer
...
[ADR-QUALITY](0050_quality-contract.md) decision 6 has **already fixed what it
...
**[ADR-QUALITY](0050_quality-contract.md) named the same failure from the
...
5. **`answerable` is a refusal, not the bottom of a scale.** An agent handed
   ...
   [ADR-QUALITY](0050_quality-contract.md) decision 5 supplies the measured
...
6. ⚠ **`SEPARATION_FLOOR` is a PROXY, and fux does not get to pick its own
   abstention threshold.** [ADR-QUALITY](0050_quality-contract.md) decision 6
...
11. ⚠ **`--band` gates the CLI; MCP is always on. RULED by Arpit, 2026-08-27,
    ...
    - ✅ **The documentation-only mitigation is retired.**
      [ADR-OUTPUT](0053_output-defaults.md) lets a repo commit `band = true` in
...
14. **`_as_dict` and `cmd_ask` in this file gained the `sections` gate — the
    decision is [ADR-OUTPUT](0053_output-defaults.md) decision 21, noted here
...
[ADR-EXPAND](0060_expand.md) decision 10.
...
exactly the break [ADR-T1-ACCELERATOR](0019_accelerator.md) forbids. Counting
...
- **Choosing this record's own abstention threshold.** Rejected on contact with
  [ADR-QUALITY](0050_quality-contract.md) decision 6, which had frozen `t` the
...
- **Calibrating the cutoff against the 50-query golden set now.** Fastest route
  ...
  [ADR-RS](0042_predictions.md)'s run-classification rule: choosing the number
...
- **A cross-encoder or model-scored confidence.** Refused for the reason
  [ADR-RERANK](0047_rerank.md) refused it — not cost, but cross-machine
...
- [ADR-QUALITY](0050_quality-contract.md) decisions 5 and 6 — the frozen
...
[ADR-ANSWER](0014_answer.md) · [ADR-T1-ACCELERATOR](0019_accelerator.md) ·
[ADR-REFER](0036_refer-plane.md) · [ADR-RS](0042_predictions.md) ·
[ADR-TUNE](0044_tuning.md) · [ADR-MCP](0045_mcp.md) ·
[ADR-RERANK](0047_rerank.md) · [ADR-QUALITY](0050_quality-contract.md)
