...
type: ADR
name: ADR-TYPES
title: "ADR-TYPES (0037) — which files are documents: a built-in allowlist, overridable by .fux/sources/types and by .fuxignore"
...
([ADR-DECODE](0048_decode.md)) that emits keys as headings and drops ids,
...
already runs is broken — but [ADR-FUXIGNORE](0054_fuxignore.md) decision 5
...
directions** ([ADR-FUXIGNORE](0054_fuxignore.md) decision 4). A path
...
[ADR-DIR-LIST](0029_dir-list.md) still leans on that reading for `fux add`.
...
the `Content-Type` its fetcher declared ([ADR-FETCHER](0026_fetcher.md)
...
[ADR-DECODE](0048_decode.md) decision 5. Two people with different
...
in [ADR-DECODE](0048_decode.md) applied one layer up, for the same L3 reason.
...
one narrowing of [ADR-URL-LIST](0025_url-list.md) decision 12 this change makes:
...
- **It does not replace exclusion.** An `evidence/report.md` is still prose in a
  ...
  [`.fux/.fuxignore`](0054_fuxignore.md) rather than in `!` lines here.
...
- **A `[sources] types` TOML array.** Rejected: the shape
  [ADR-DIR-LIST](0029_dir-list.md) had just moved away from.
...
- **Content sniffing.** ⚠ **Disqualified, not merely rejected.** Deciding
  ...
  ([ADR-HTTP-FETCHER](0028_http-fetcher.md) decision 3) decides what a document
  *is*.
...
- The code: [`src/fux/ingest/gitdir.py`](../../src/fux/ingest/gitdir.py)
  ...
  registry the default unions with — [ADR-DECODE](0048_decode.md).
...
**Records** — [ADR-INGEST](0015_ingest.md) · [ADR-URL-LIST](0025_url-list.md) ·
[ADR-FETCHER](0026_fetcher.md) · [ADR-HTTP-FETCHER](0028_http-fetcher.md) ·
[ADR-DIR-LIST](0029_dir-list.md) · [ADR-DECODE](0048_decode.md) ·
