...
type: ADR
name: ADR-URL-INGEST
title: ADR-URL-INGEST (0016) — URL ingestion through a consumer-owned fetcher
...
**1. The fetch contract itself is [ADR-FETCHER](0026_fetcher.md)'s, and is not
...
(L4, [ADR-CLI](0010_cli-surface.md) decision 1d).
...
design. It is [ADR-INGEST](0015_ingest.md) decision 9, and it is what
...
[ADR-URL-LIST](0025_url-list.md)'s and are not restated here.
...
- **That is what the daemon is for** ([ADR-MAINTENANCE](0038_hooks.md) decision
...
- **Hashed results are unreadable by design** — `fux ask` prints a hash where a
  ...
  ([ADR-RECORD](0018_index-record.md)). That is the mode working, and it is a
...
[ADR-INGEST](0015_ingest.md) · [ADR-RECORD](0018_index-record.md) ·
