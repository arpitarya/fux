...
title: ADR-T1-ACCELERATOR (0019) — the derived T1 accelerator
...
([ADR-RANKING](0020_ranking.md)). The differential law then reduces to "the
...
fails ([ADR-INDEX-LIFECYCLE](0017_index-lifecycle.md)).
...
(W-109, [ADR-EXPAND](0060_expand.md) decisions 6-7). `--expand` gives
...
- **The differential law now covers the confidence block too.** `accel.ask`
  ...
  ([ADR-CONFIDENCE](0051_confidence.md) decision 9).
...
- **`stats.json` stores RAW `total_flen`, not a pre-weighted total.** A stored
  ...
  is [ADR-RUNTIME-STATS](0034_runtime-stats.md).
...
- **`fux build` is a two-lane build, and the second lane is not this
  record's.** The graph plane ([ADR-GRAPH](0035_graph.md)) is written by the
...
- **A corpus with hashed URL records once had no accelerator at all** and paid
  ...
  record's invariant ([ADR-RECORD](0018_index-record.md) rule 2); the
...
[ADR-ASK](0012_ask.md) · [ADR-INDEX-LIFECYCLE](0017_index-lifecycle.md) ·
[ADR-RECORD](0018_index-record.md) · [ADR-RANKING](0020_ranking.md) ·
