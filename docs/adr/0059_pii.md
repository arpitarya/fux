...
type: ADR
name: ADR-PII
title: "ADR-PII (0059) — redact what gets committed, leave alone what stays local"
...
The second force is [ADR-ACQUIRED](0056_acquired-plane.md), which put fetched
...
1. **Redaction applies to `.fux/index/` and to nothing else.** The acquired
   ...
   *sensitive vs not*.
   ...
   Closed by W-102; see [ADR-ENRICH](0046_enrich.md) decision 12 for the fix and
   ...
   *plane*, the code redacts a *phase*, and anything that reaches the plane
   ...
   backslashed twin. Fixed in [ADR-ENRICH](0046_enrich.md) decision 14, noted
   here because this is the surface that prints it.

2. **`.fux/acquired/` is never redacted, and this is load-bearing rather than
   an omission.** [ADR-URL-FRESHNESS](0058_url-freshness.md) decision 6
...
5. **There is no built-in floor, unlike [ADR-REFUSAL](0057_refusals.md).**
...
13. **`fux enrich` covers `url:` documents, via `enrich=` on the URL list.**
    ...
    ⚠ **This was impossible before [ADR-ACQUIRED](0056_acquired-plane.md).**
...
- **Refuse the document on a PII hit, like a refusal.** Rejected: it loses a
  ...
  ([ADR-ENRICH](0046_enrich.md) decision 12), because a document is a fact fux
...
- [ADR-REFUSAL](0057_refusals.md) — the record whose floor this one deliberately does not have
- [ADR-ACQUIRED](0056_acquired-plane.md) — the raw plane decision 2 protects, and what makes decision 13 possible
...
[ADR-URL-LIST](0025_url-list.md) · [ADR-RECORD](0018_index-record.md) ·
[ADR-ENRICH](0046_enrich.md) · [ADR-ACQUIRED](0056_acquired-plane.md) ·
