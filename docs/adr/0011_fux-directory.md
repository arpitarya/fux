...
type: ADR
name: ADR-DOTFUX
title: ADR-DOTFUX (0011) — the .fux/ directory
...
reached it still holds ([ADR-ACQUIRED](0056_acquired-plane.md)).
...
[ADR-ACQUIRED](0056_acquired-plane.md) decision 1.
...
| `sources/` | committed | `dirs` · `urls` · `types`, one entry per line, on the one grammar in [ADR-URL-LIST](0025_url-list.md). ⚠ Since 2026-09-01 `types` is also the **decoder map**: `fux setup` writes `decoder=<module>` on every line a built-in reads, so which decoder ran is a committed fact rather than a property of the machine's `decoders/` ([ADR-TYPES](0037_types-list.md) decision 11). Still write-if-missing — a repo that already has the file keeps it, bindings and all |
| `fetchers/` | committed | consumer code — decision 4 |
| `decoders/` | committed | consumer code — decision 5 |
| `enrich/` | committed | pinned enrichment text, one file per **source content sha**, plus `queue.tsv`. It cannot be re-derived: a model wrote it, in an agent, once, and [ADR-ENRICH](0046_enrich.md) decision 1 refuses to call one. Committed also means **every clone has identical coverage**, so L3 holds with a wider input rather than a weaker property. Keying by source sha means editing a document orphans its enrichment automatically — staleness is structural rather than a check someone has to remember |
...
| `.fuxignore` | committed | what is **not** indexed, in `.gitignore`'s grammar — the one home for exclusion, read before the source lists and outranking them in both directions ([ADR-FUXIGNORE](0054_fuxignore.md)). Committed for the same reason `tune.toml` is: a corpus that differed by clone is the surprise this split removes. Written header-only by `fux setup`, and never rewritten |
| `README.md` · `.gitignore` | committed | generated, write-if-missing |
| `refusals.toml` | committed | what a **refusal** looks like in this organisation — the sign-in walls, paywalls and viewer shells a server returns instead of the document ([ADR-REFUSAL](0057_refusals.md)). Consumer-owned and additive; the engine ships no vendor knowledge, and the always-on magic-byte floor is not configurable from it. Committed because *"what does a login page look like here"* is a team fact, exactly like `.fuxignore` |
...
configuration. See [ADR-CACHEDIR-TAG](0030_cachedir-tag.md). ⚠ **For
...
fresh repo** ([ADR-TYPES](0037_types-list.md) decision 10, amended). The template
...
[ADR-FETCHER](0026_fetcher.md) decisions 12–13 added two **optional** functions
...
`main` before it was caught.** [ADR-OUTPUT](0053_output-defaults.md) decision
...
([ADR-FETCHER](0026_fetcher.md) decision 1). One known consequence, accepted:
...
([ADR-CDP-FETCHER](0027_cdp-fetcher.md) decision 8). A decoder is stdlib-only
...
[ADR-AGENT-POLICY](0041_agent-policy.md) decisions 5 and 6 widen it: `setup`
...
since 2026-09-06 ([ADR-AGENT-POLICY](0041_agent-policy.md) decision 11). Everything else about the
...
([ADR-AGENT-POLICY](0041_agent-policy.md) decision 12). **A vendor whose only
...
([ADR-AGENT-POLICY](0041_agent-policy.md) decision 14). It is a *fourth*
...
- **Read-only, always.** A stale runner lock is named along with the command
  ...
  ([ADR-MAINTENANCE](0038_hooks.md) veto 7). A pending re-index is a
...
- **Offline, always.** The URL section reports how many `url:` records exist,
  ...
  fetches**, and it **never deletes**: [ADR-URL-INGEST](0016_url-ingest.md)
...
[ADR-MAINTENANCE](0038_hooks.md) owns. A check that *formats* another plane's
...
one people learn to skip. See [ADR-MAINTENANCE](0038_hooks.md) decision 12.
...
|---|---|
| `refusal rules` | a refusal count lived in one run's stderr; a rule matching everything empties the `url:` half of the corpus and leaves it looking like a corpus nobody ingested — [ADR-REFUSAL](0057_refusals.md) decision 11 |
| `decoder bindings` | `registry()` refuses a broken `decoder=` binding **on the next ingest**; and a binding on an extension no document has resolves perfectly and indexes nothing forever, which is deliberately not an ingest error — [ADR-DECODE](0048_decode.md) |
...
| `freshness verdicts` | the `as-ingested` share is the **veto check** of two accepted records ([ADR-ACQUIRED](0056_acquired-plane.md), [ADR-URL-FRESHNESS](0058_url-freshness.md)) and neither veto could be run at all |
| redaction counts, on `pii rules` | `redact()` returned them, `run()` summed them, and nothing read the sum — [ADR-PII](0059_pii.md) decision 15 |
...
- The third category's own record — [ADR-ACQUIRED](0056_acquired-plane.md), and the two records that consume it, [ADR-REFUSAL](0057_refusals.md) and [ADR-URL-FRESHNESS](0058_url-freshness.md)
...
**Records** — [ADR-LAWS](0001_laws.md) · [ADR-CLI](0010_cli-surface.md) ·
[ADR-FUXIGNORE](0054_fuxignore.md) ·
[ADR-URL-INGEST](0016_url-ingest.md) · [ADR-CONFIG](0022_config.md) ·
[ADR-URL-LIST](0025_url-list.md) · [ADR-FETCHER](0026_fetcher.md) ·
[ADR-CDP-FETCHER](0027_cdp-fetcher.md) ·
[ADR-CACHEDIR-TAG](0030_cachedir-tag.md) · [ADR-TYPES](0037_types-list.md) ·
[ADR-MAINTENANCE](0038_hooks.md) · [ADR-AGENT-POLICY](0041_agent-policy.md) ·
[ADR-TUNE](0044_tuning.md) · [ADR-ENRICH](0046_enrich.md) ·
[ADR-DECODE](0048_decode.md) · [ADR-ACQUIRED](0056_acquired-plane.md) ·
[ADR-REFUSAL](0057_refusals.md) · [ADR-URL-FRESHNESS](0058_url-freshness.md)
