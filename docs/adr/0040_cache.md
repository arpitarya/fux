...
type: ADR
name: ADR-CACHE
title: "ADR-CACHE (0040) — two caches, two different proofs: ARC decides what a fetch returned, the TTL store decides whether to fetch at all"
...
index recorded, and quotes it ([ADR-REFER](0036_refer-plane.md) decisions 1–5).
...
downstream is [ADR-REFER](0036_refer-plane.md) decision 4's *knob that lies*
...
([ADR-RUNTIME-STAMP](0033_runtime-stamp.md)): derived, per-machine,
...
*no* ([ADR-REFER](0036_refer-plane.md) decision 4, and
...
already-tagged `runtime/` ([ADR-CACHEDIR-TAG](0030_cachedir-tag.md)), so backup
...
- The parent record — [ADR-REFER](0036_refer-plane.md), decisions 4, 6 and 7
...
- The carve-out precedent this follows —
  [ADR-MERGE-DRIVER](0039_merge-driver.md), which takes `mergedriver.py` out of
...
[ADR-RUNTIME-STAMP](0033_runtime-stamp.md) · [ADR-REFER](0036_refer-plane.md)
