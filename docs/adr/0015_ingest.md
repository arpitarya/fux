...
type: ADR
name: ADR-INGEST
title: ADR-INGEST (0015) — how ingest works
...
([ADR-ASK](0012_ask.md) decision 9). What carry-forward saves today is the
...
[ADR-FUXIGNORE](0054_fuxignore.md) decision 11, in two delimited blocks,
...
([ADR-DOTFUX](0011_fux-directory.md)), so a fresh clone is correctly laid out
before the first byte lands.

**8. Ingest is offline.** The exceptions are the named fenced paths
([ADR-URL-INGEST](0016_url-ingest.md)); a plain run never imports the fetcher.
...
⚠ **`DEFAULT_TYPES` is unchanged by this.** [ADR-TYPES](0037_types-list.md)
...
source** ([ADR-ARCHIVED-CONTENT](0043_archived-content.md) decision 1). It reads
...
([ADR-PII](0059_pii.md)). Redaction runs inside this file, between
...
| "run the rules over the whole file" | the frontmatter is deliberately outside the pass. It is stripped before indexing already ([ADR-ENRICH](0046_enrich.md) decision 8), so nothing in it reaches a committed term, and matching a `model:` value would refuse a file for text the index never sees |
...
([ADR-ACQUIRED](0056_acquired-plane.md) decision 8, [ADR-CONFIG](0022_config.md)
...
retires nothing. [ADR-ENRICH](0046_enrich.md) decisions 17-18.
...
- **`fux ingest --stop` and the runner takeover change nothing about what a run
  ...
  **never by reading the dirty list** — the list is advisory, and a run that
  ...
  [ADR-MAINTENANCE](0038_hooks.md) 1d and the surface is
  [ADR-CLI](0010_cli-surface.md).
...
- **`run()` takes an optional `progress`, `None` meaning silent.** Four phases
  ...
  to [ADR-CLI](0010_cli-surface.md); what binds here is that the phases report
...
- ⚠ **`fux ingest` now writes a committed file that is also its own input.**
  ...
  [ADR-FUXIGNORE](0054_fuxignore.md) decision 11.
...
- **The graph plane can no longer be handed a dangling edge by ingest**
  (decision 10). [ADR-GRAPH](0035_graph.md)'s `edges_from_records` lifts edges
...
- **A `hashed` URL record writes a second thing before it is eligible to
  ...
  rationale on [ADR-RECORD](0018_index-record.md).
...
- **Keep the already-reported set in `.fux/runtime/skipped`.** Rejected on
  ...
  [ADR-FUXIGNORE](0054_fuxignore.md) decision 11 and made loud by 11a.
...
[ADR-PII](0059_pii.md) decision 15, which owns redaction. **Two properties this
...
[ADR-DOTFUX](0011_fux-directory.md) · [ADR-ASK](0012_ask.md) ·
[ADR-URL-INGEST](0016_url-ingest.md) ·
[ADR-INDEX-LIFECYCLE](0017_index-lifecycle.md) ·
[ADR-RECORD](0018_index-record.md) · [ADR-EXTRACTED](0024_extracted-mode.md) ·
[ADR-GRAPH](0035_graph.md) · [ADR-TYPES](0037_types-list.md) ·
[ADR-MAINTENANCE](0038_hooks.md) ·
[ADR-ARCHIVED-CONTENT](0043_archived-content.md) ·
[ADR-DECODE](0048_decode.md)
