...
`.fux/sources/dirs` ([ADR-DIR-LIST](0030_dir-list.md) decision 1).
...
declared `.fux/` layout ([ADR-DOTFUX](0012_fux-directory.md)). The default is
the plain-GET fetcher because a URL line carrying no `fetch=` means
`fetch=http` ([ADR-HTTP-FETCHER](0029_http-fetcher.md) decision 1).
...
`fux setup` is write-if-missing ([ADR-DOTFUX](0012_fux-directory.md)), so a
...
for ([ADR-AGENT-POLICY](0042_agent-policy.md) decision 5). ⚠ **`codex` joined
2026-09-06** ([ADR-AGENT-POLICY](0042_agent-policy.md) decision 11): the set is
...
a source-wide *layer*, not a setting.** `keep` ([ADR-ACQUIRED](0057_acquired-plane.md)),
`ttl` ([ADR-URL-FRESHNESS](0059_url-freshness.md)) and `enrich`
([ADR-PII](0060_pii.md)) each sit between the built-in default and the URL
...
[ADR-ASK](0013_ask.md) · [ADR-FETCHER](0027_fetcher.md) ·
[ADR-HTTP-FETCHER](0029_http-fetcher.md) · [ADR-DIR-LIST](0030_dir-list.md) ·
[ADR-TYPES](0038_types-list.md) · [ADR-AGENT-POLICY](0042_agent-policy.md) ·
