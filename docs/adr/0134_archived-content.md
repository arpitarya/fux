...
title: "ADR-ARCHIVED-CONTENT (0134) — what a document declared archived does, once it is indexed"
...
> [ADR-DIR-LIST](0120_dir-list.md)** — two different questions that once shared
> one document.

## §1 — For humans

A directory (or, through [ADR-URL-LIST](0116_url-list.md)'s shared grammar, a
...
```console
$ fux ask "what is the ingest cache" --top 5
3.4417             How ingest works today        (docs/adr/0106_ingest.md)
...
2.1188             Delta ingest and reuse        (docs/adr/0106_ingest.md)
...
[ADR-DIR-LIST](0120_dir-list.md) decides the file, its grammar, and that the
...
[ADR-RECORD](0109_index-record.md)'s reason: **a record read years later states
...
- **It is applied to CARRIED records, not only freshly fetched ones**
  ([ADR-INGEST](0106_ingest.md)). A retired page is precisely the one that has
...
- ⚠ **Fetchers never see it.** It is resolved from the committed list and does
  not cross the fetch contract ([ADR-FETCHER](0117_fetcher.md)), which is what
...
`ask` show the same fact, because [ADR-FIND](0104_find.md) makes `find` a
...
- **It keys off the declaration, never a path.**
  [ADR-DIR-LIST](0120_dir-list.md) decision 4 stands: a path prefix is not a
...
- **It states what archived *is*, not what the reader should *do* about it**,
  ...
  **A single sentence cannot instruct all three, and the list is not closed.**
  ...
  the consumer's policy, not the engine's** — [ADR-AGENT-POLICY](0132_agent-policy.md).
...
- **stdout stability applies.** `--json` is a contract and the surface captures
  ...
  [ADR-CLI](0101_cli-surface.md)'s call, taken there.
...
| **`superseded_by:` in an enrichment** ([ADR-ENRICH](0137_enrich.md) decision 17) | ✅ exact, **and never touches the original** — the answer for a document you cannot edit |
...
8. 6.4341 [live] docs/adr/0101_cli-surface.md          <-- the actual answer
```
...
- [ADR-DIR-LIST](0120_dir-list.md) — the file and grammar this record's
  behaviour attaches to.
- [ADR-RECORD](0109_index-record.md) — the record schema `archived: true` joins;
  [ADR-FIND](0104_find.md) — why `find` shows the same fact as `ask`;
  [ADR-CLI](0101_cli-surface.md) — stdout stability, which puts the disclaimer
  on stderr.
- [ADR-REFER](0127_refer-plane.md) — the `current`/`stale`/`unverified`
  precedent for stating a fact and refusing to interpret it, behind decision 7;
  [ADR-AGENT-POLICY](0132_agent-policy.md) — where the interpretation lives
  instead.
...
[ADR-FIND](0104_find.md) · [ADR-RECORD](0109_index-record.md) ·
[ADR-URL-LIST](0116_url-list.md) · [ADR-DIR-LIST](0120_dir-list.md) ·
[ADR-REFER](0127_refer-plane.md) ·
[ADR-AGENT-POLICY](0132_agent-policy.md) · [ADR-TUNE](0135_tuning.md)
