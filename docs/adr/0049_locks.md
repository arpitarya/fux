...
type: ADR
name: ADR-LOCKS
title: "ADR-LOCKS (0049) — the one lock fux owns, and the three files beside it that are not locks"
...
[ADR-MAINTENANCE](0038_hooks.md)'s decisions 1a–1d and widened again when the
...
refused: [ADR-MAINTENANCE](0038_hooks.md) decision 1c requires the state to be
...
- **This record owns no component, so the freshness gate will not fire on it.**
  A change to locking updates [ADR-MAINTENANCE](0038_hooks.md), which owns
...
**Records** — [ADR-MAINTENANCE](0038_hooks.md) · [ADR-DOTFUX](0011_fux-directory.md) · [ADR-LAWS](0001_laws.md)
