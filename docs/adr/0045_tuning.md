...
[ADR-CONFIDENCE](0052_confidence.md) decision 13 puts `separation_floor` and
...
|---|---|---|
| **changes the committed index** | the phrase cap · the edge grades `10`/`8`/`6` · the token cap · the fixed shard count | format decisions; moving one costs a re-ingest and a `_format` bump. The type allowlist already has a home — [ADR-TYPES](0038_types-list.md) |
| **derived, and a speed knob not a ranking one** | the block size · the community sweep cap | changes `.fux/runtime/` only; **the differential law means results cannot move, only latency**. Belongs to [ADR-T1-ACCELERATOR](0020_accelerator.md) |
| **operational, not retrieval** | runner timeouts · the progress threshold · the fetch cache's TTL and byte cap | the cache pair is the arguable one — it is resource policy, [ADR-CACHE](0041_cache.md) owns it, and exposing it belongs in `fux.toml` beside the fetcher config |
...
with [ADR-DIR-LIST](0030_dir-list.md) decision 2b's order-independent
...
stated ([ADR-GRAPH](0036_graph.md)).
...
| `w == 0` | error, naming the `!` entry | that is **exclusion**, and [ADR-DIR-LIST](0030_dir-list.md) decision 2a already owns it. **Two ways to do one thing is the rot** |
...
consequences are [ADR-T1-ACCELERATOR](0020_accelerator.md) §The weighted bound.
...
measurement, [ADR-CONFIDENCE](0052_confidence.md) decision 12's outcome.
...
[ADR-EXPAND](0061_expand.md) decision 5), default **`0.2`** — Query2doc's 1:5
...
2026-09-05** (W-111, [ADR-RANKING](0021_ranking.md)) — and **the tie-break
...
- ⚠ **The accelerator gets slower in proportion to the spread.** Decision 12's
  ...
  [ADR-RS](0043_predictions.md) with a frozen pre-registration.
...
[ADR-DOTFUX](0012_fux-directory.md) ·
[ADR-T1-ACCELERATOR](0020_accelerator.md) · [ADR-RANKING](0021_ranking.md) ·
[ADR-CONFIG](0023_config.md) · [ADR-URL-LIST](0026_url-list.md) ·
[ADR-DIR-LIST](0030_dir-list.md) ·
[ADR-RUNTIME-STATS](0035_runtime-stats.md) · [ADR-GRAPH](0036_graph.md) ·
[ADR-REFER](0037_refer-plane.md) · [ADR-TYPES](0038_types-list.md) ·
[ADR-CACHE](0041_cache.md) · [ADR-RS](0043_predictions.md) ·
[ADR-ARCHIVED-CONTENT](0044_archived-content.md)
