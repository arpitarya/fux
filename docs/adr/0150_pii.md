...
type: ADR
name: ADR-PII
title: "ADR-PII (0150) — redact what gets committed, leave alone what stays local"
...
The second force is [ADR-ACQUIRED](0147_acquired-plane.md), which put fetched
...
1. **Redaction applies to `.fux/index/` and to nothing else.** The acquired
   ...
   *sensitive vs not*.
   ...
   Closed by W-102; see [ADR-ENRICH](0137_enrich.md) decision 12 for the fix and
   ...
   *plane*, the code redacts a *phase*, and anything that reaches the plane
   ...
   backslashed twin. Fixed in [ADR-ENRICH](0137_enrich.md) decision 14, noted
   here because this is the surface that prints it.

2. **`.fux/acquired/` is never redacted, and this is load-bearing rather than
   an omission.** [ADR-URL-FRESHNESS](0149_url-freshness.md) decision 6
...
5. **There is no built-in floor, unlike [ADR-REFUSAL](0148_refusals.md).**
...
11. **Editing `pii.toml` invalidates extraction reuse.** Reuse is keyed on the
    ...
    covers `.fux/tune.toml [index]` the same way ([ADR-INGEST](0106_ingest.md)
    decision 15b).
...
13. **`fux enrich` covers `url:` documents, via `enrich=` on the URL list.**
    ...
    ⚠ **This was impossible before [ADR-ACQUIRED](0147_acquired-plane.md).**
...
17. **`.fux/pii.toml` is required: `fux setup` writes it, and a missing file
    ...
    |---|---|
    | `fux setup` | **writes the starter**, write-if-missing, never rewrites ([ADR-DOTFUX](0102_fux-directory.md) decision 6) |
    ...
    - **`doctor` is exempt from the gate, not from the error.** The verb whose
      ...
      property [ADR-OUTPUT](0144_output-defaults.md) decision 19 protects.
    ...
    - ⚠ **The upgrade cost is real.** Every repo that predates this decision
      ...
      ([ADR-DOTFUX](0102_fux-directory.md) decision 6's *a loader refusal,
      never a rewrite*).
...
- **`update=never` makes that permanent rather than transient.**
  ([ADR-URL-LIST](0116_url-list.md) decision 14.) A failed fetch carries a
  record forward until the next run succeeds; a pinned line is filtered out
  **before** `fetch_all` on every run, `--full` included, so the record it has
  is the record it keeps.
- ⚠ **`keep=true` does not rescue it today.** The bytes are sitting in
  `.fux/acquired/`, and the ingest path does not re-extract a `url:` record from
  retained bytes — it re-extracts only what it fetched. **So the data needed to
  honour a new rule is present and unused.**
- **Recorded, not fixed.** Re-extracting `url:` records from the acquired plane
  is its own decision with its own cost (it would make every ingest re-derive
  every retained document), and inventing it inside a source-list attribute's
  change would be the wrong place to decide it. **What this record owes is that
  nobody believes a `pii.toml` edit swept a corpus when it could not.**
  ...
  does not read. **Not hard, not free, and not decided here.**
...
([ADR-AGENT-POLICY](0132_agent-policy.md) decision 15). ⚠ **Those surfaces
...
- **What they may carry:** how to run the probe, how to read a refusal, which
  ...
  ([ADR-LAWS](0001_LAWS.md) L0), and a guide that repeats one can drift from it
...
in the title field's terms on every record including a hashed-meta one.
...
- **This is not a `meta` question.** `meta = "hashed"` (L5) keeps *display text*
  out of a record; it has never touched `loc`, and for a `url:` document it
  could not — the address is the document.
- **`cli.py`'s PII gate is untouched by the 2026-09-11 rendering-flag change**
  (ADR-OUTPUT): `json_explicit` reaches one verb, `hooks`, which writes no
  index and reads no document.
- ⚠ **A pinned URL added after this lands is still redacted on its one fetch**,
  and never again — [ADR-URL-LIST](0116_url-list.md) decision 14's exemption
...
surface, and [ADR-CLI](0101_cli-surface.md) veto 1 refuses a second way to do
...
([ADR-CONFIG](0113_config.md) decisions 13–14). Nothing about this record's
...
and the code landed two commits later — see ADR-CONFIG after decision 15.
...
that file — [L10](0012_LAW-10-bundled-output.md) made `.fux/node/fux.mjs` and
...
instrument is aimed at"* is ADR-NODE-SEARCH's lesson from three shipped
...
`_format` refusal (decision 2 of ADR-NODE-SEARCH) is what catches that, not this
...
- **Refuse the document on a PII hit, like a refusal.** Rejected: it loses a
  ...
  ([ADR-ENRICH](0137_enrich.md) decision 12), because a document is a fact fux
...
- [ADR-REFUSAL](0148_refusals.md) — the record whose floor this one deliberately does not have
- [ADR-ACQUIRED](0147_acquired-plane.md) — the raw plane decision 2 protects, and what makes decision 13 possible
...
[ADR-URL-LIST](0116_url-list.md) · [ADR-RECORD](0109_index-record.md) ·
[ADR-ENRICH](0137_enrich.md) · [ADR-ACQUIRED](0147_acquired-plane.md) ·
