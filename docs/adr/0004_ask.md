...
**Since 2026-08-26 a result also names the sections that match** (decision 10).
Captured from this repository, not typed:
...
5.0242  [archived] W-48 — three output-contract inconsistencies in the query verbs  (archive/open/W-48-query-output-contract.md)
        § Definition of done
5.0091  [archived] W-46 — `ask --hybrid` crashes on a source install  (archive/open/W-46-hybrid-missing-model-crash.md)
        § Definition of done
...
exactly as right when it is wrong. Decision 10 carries the argument.
...
> [W-82 §5.3](../../work/open/W-82-the-consolidated-build.md)'s finding firing
...
**10. `ask` cites at HEADING level, and never at line level**
([W-84](../../archive/open/W-84-heading-level-ask.md), 2026-08-26). Each
result carries the document's committed headings that match
the query — best first, at most three — rendered as indented `§` lines in text
and as an always-present `"headings"` array in `--json`.

**The refusal is the decision; the feature is what is left after it.** Arpit
asked whether `ask` should cite lines. It may not, and the reason is
...
- **A line range on `ask` could only be computed at ingest, so it would
  ...
  defect class as a configured `max_age_seconds` or a `cached` verdict
  reported as `current`: a field that answers confidently about something it
  no longer knows.
...
- **`answer` is the verb that cites lines**, because it fetched the bytes and
  cites the sha it read them at ([ADR-ANSWER](0006_answer.md)). The split is
...
`_resolve_title` occupies under P5 — so whichever generator answered, both
resolve identically and there is **no seam for the differential law to break
through**. Nothing in `query/headings.py` computes or adjusts a score, and
nothing in it may. Re-verified on this repo: `diff` of the two paths is
`IDENTICAL` with headings present.
...
- **`"headings"` is always present, `[]` when nothing matches.** An absent key
  is a trap, not a signal (W-48) — a caller cannot otherwise tell *"nothing
  matched"* from *"this fux is older than the feature"*.
...
> **Amended 2026-08-26 (W-82 §3.4) — a note on `cmd_answer`, recorded here
> because of where the code lives, not because of what it is about.**
>
> `_declare_change_since_last_ask` was added to `src/fux/query/__init__.py`,
> which **this record owns at directory level**. Its subject is
> [ADR-ANSWER](0006_answer.md)'s, and the substantive amendment is there.
>
> ⚠ **This is W-82 §5.3's governance gap in miniature, and it is deliberate to
> leave it visible.** `tests/test_adr_freshness.py` demands the *owning* record
> for a changed component; ownership is **directory-level**; so a change whose
> subject is one record can be discharged by touching another. That is exactly
> how sixteen records went stale during W-76 while the check stayed green.
> Both records are amended here rather than only the one the check would have
> accepted.
>
> **Nothing about BM25F, the scan, `rank()` or the differential law changes.**
> The added function writes a line to **stderr** and touches gitignored runtime
> state; it cannot reach a score, an ordering, or a byte of stdout.

- **W-84 added a second display-only resolution beside P5's, 2026-08-26.**
  `query/headings.py` is new under this directory and the directory-level claim
  covers it. It reads **one committed record** — the same one `_resolve_title`
  already reads, which is why `_title_from` was split out so the shard is read
  once rather than twice — and never the working tree (that is
  [ADR-RERANK](0041_rerank.md)'s carve-out, and the distinction is why it is a
  carve-out). `_as_dict` takes the query string now, because the selection is a
  function of it. **The `--json` default payload gained a key**, which is a
  change from `--explain`'s precedent of hiding additions behind a flag; the
  reason the two differ is that `"path"` is *diagnostic* and optional to a
  caller, while a citation an agent cannot act on is the defect being fixed.
  Both paths produce `"headings"` from the same record, so byte-identity across
  the differential law is preserved — it is a function of a list the two
  generators already agree on.

> **Amended 2026-08-26 — the `--json` output has a declared shape, and it is
> the only PUBLIC one fux has.**
>
> `query/output.schema.json` declares `ask_result`, `answer_citation`,
> `answer_payload` and `passage`. **`fux answer --json` is validated against it
> before it is printed**, so fux cannot emit JSON that violates its own
> contract.
>
> ⚠ **Two promises this record and ADR-ANSWER already made were checked by
> nothing.** This record tells callers to branch on the `archived` **boolean**
> rather than on the prose note; ADR-ANSWER tells them to switch on `source`,
> and W-48 established that `source` must be present even on the no-match
> branch because *an absent key is a trap, not a signal*. Both were prose. **A
> key quietly renamed here breaks a consumer silently, at their end, with no
> error at ours** — which is the one failure mode a declaration cannot be
> talked out of.
>
> **A contract violation raises rather than degrading**, which is the opposite
> of how the reading paths behave and deliberately so: `coerce` exists because a
> file on disk may have been written by an older version or a killed process,
> and neither applies to a dict this process built three lines ago. **A
> violation here is a bug in fux, not in the world.**
>
> **The schema records the `ask` / `answer` split as law rather than accident:**
> `ask` and `find` are document-level (`docs/mesh.md`), `answer` is span-level
> (`docs/mesh.md:L10-L13`). A line range can only be computed by chunking
> *fetched* bytes, the index holds statistics rather than text, and `ask` is
> offline by default — so giving `ask` line numbers would mean making it fetch.
> **Nothing about BM25F, the scan, `rank()` or the differential law changes.**

### Alternatives considered

- **Line ranges on `ask` (W-84, 2026-08-26).** Rejected on two independent
  grounds, either of which is sufficient. **It would have to lie:** the range
  could only be computed at ingest, so one edit makes it wrong while it still
  looks right. **And it would cost the product's central promise:** line
  numbers need term positions — a positional index, conventionally 2–4× the
  postings (Zobel & Moffat 2006 §5) — against an index whose pitch is that it
  fits in git. The value is also thinnest where the cost is highest: for
  `file:` sources the document is already in the tree and a line number saves
  one `grep`, while `url:` sources, where it would help, are the ones most
  likely to have changed since ingest. `answer` cites lines because it fetches.
...
- Decision 10's selection and its refusal —
  ...
  [ADR-EXTRACTED](0016_extracted-mode.md); the tests —
  [`tests/query/test_headings.py`](../../tests/query/test_headings.py).
...
# 4. decision 10's headings are still display-only (W-84)
...
[ADR-TUNE](0038_tuning.md) · [ADR-EXTRACTED](0016_extracted-mode.md) ·
[ADR-DIR-LIST](0022_dir-list.md) · [ADR-RERANK](0041_rerank.md) ·
[ADR-MCP](0039_mcp.md)
...
- Zobel & Moffat, *Inverted Files for Text Search Engines* (ACM Computing
  Surveys 38(2), 2006) — §5 on positional postings, the cost decision 10
  declines to pay
