...
[ADR-EXPAND](0054_expand.md) carries both decisions.
...
[ADR-RANKING](0012_ranking.md)) — `true` when this result's rounded score
...
⚠ **`find` gained `--phrase`, `--under` and `--all`** ([ADR-FIND](0005_find.md)).
