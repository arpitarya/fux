...
> after Phase 7. `fuse.py` itself is not deleted — it has no consumer left in
> `src/`, but it is the archived engine's ported RRF math
> (`tests/query/test_fuse.py`), kept for what it pins rather than for a caller.
