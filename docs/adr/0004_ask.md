...
capture it is; the flag is on all three read verbs, and on `explain`, `graph`
and `path`. It is one flag rather than one per tune table because the question
it answers is *"is it me or the config?"* — [ADR-CLI](0002_cli-surface.md)
