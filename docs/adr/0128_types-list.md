...
type: ADR
name: ADR-TYPES
title: "ADR-TYPES (0128) — which files are documents: a built-in allowlist, overridable by .fux/formats.toml and by .fuxignore"
...
([ADR-DECODE](0139_decode.md)) that emits keys as headings and drops ids,
...
subtraction at all.** [ADR-FUXIGNORE](0145_fuxignore.md) decision 5 made
...
directions** ([ADR-FUXIGNORE](0145_fuxignore.md) decision 4). A path
...
[ADR-DIR-LIST](0120_dir-list.md) still leans on that reading for `fux add`.
...
the `Content-Type` its fetcher declared ([ADR-FETCHER](0117_fetcher.md)
...
[ADR-DECODE](0139_decode.md) decision 5. Two people with different
...
in [ADR-DECODE](0139_decode.md) applied one layer up, for the same L3 reason.
...
[ADR-URL-LIST](0116_url-list.md) decision 12; `.fux/formats.toml` refuses an empty
...
([ADR-URL-LIST](0116_url-list.md) decisions 1-2), and a file of its own is not a
...
**The reader is lenient, the writer is strict** — [ADR-URL-LIST](0116_url-list.md)
...
**`[sources] types_file` in `fux.toml` is refused by name** ([ADR-CONFIG](0113_config.md)).
...
- **It does not replace exclusion.** An `evidence/report.md` is still prose in a
  ...
  [`.fux/.fuxignore`](0145_fuxignore.md) rather than in `!` lines here.
...
- **A `[sources] types` TOML array.** Rejected: the shape
  [ADR-DIR-LIST](0120_dir-list.md) had just moved away from. ⚠ **Reversed in
...
- **Content sniffing.** ⚠ **Disqualified, not merely rejected.** Deciding
  ...
  ([ADR-HTTP-FETCHER](0119_http-fetcher.md) decision 3) decides what a document
  *is*.
...
- The code: [`src/fux/ingest/gitdir.py`](../../src/fux/ingest/gitdir.py)
  ...
  decoder registry the default unions with — [ADR-DECODE](0139_decode.md).
...
**Records** — [ADR-INGEST](0106_ingest.md) · [ADR-URL-LIST](0116_url-list.md) ·
[ADR-FETCHER](0117_fetcher.md) · [ADR-HTTP-FETCHER](0119_http-fetcher.md) ·
[ADR-DIR-LIST](0120_dir-list.md) · [ADR-DECODE](0139_decode.md) ·
