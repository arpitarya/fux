...
type: ADR
name: ADR-EXTRACTED
title: "ADR-EXTRACTED (0115) — the extracted ingest mode"
...
counterpart is [ADR-ENRICH](0137_enrich.md), which absorbed ADR-ENRICHED on
...
the `enriched` mode ([ADR-ENRICH](0137_enrich.md)) is built.
...
- **`flen` rather than a weighted length**, because a weighted length computed
  ...
  ([ADR-TUNE](0135_tuning.md) decision 6). Raw counts are facts; the weighting
...
([ADR-ASK](0103_ask.md) decision 9); that awkwardness went with it.
...
Markdown by contract ([ADR-DECODE](0139_decode.md) decision 2). Only an
...
[ADR-DECODE](0139_decode.md) decision 14 for why one grammar and what the two
...
- ⚠ **Post-hoc and single-corpus**, and it is not a ranking claim: no query
  ...
  ~0.24 % of the index.
- **Why tune.toml** — [ADR-TUNE](0135_tuning.md) decision 13. **Why a changed
  cap reaches unchanged documents** — [ADR-INGEST](0106_ingest.md)'s
  `[index]` digest.
...
- ⚠ **`fux enrich` does not build the enriched mode, and the difference is
  ...
  the `enriched` mode ([ADR-ENRICH](0137_enrich.md)) remains unbuilt.
...
[ADR-INGEST](0106_ingest.md) · [ADR-ENRICH](0137_enrich.md) ·
[ADR-TUNE](0135_tuning.md) · [ADR-DECODE](0139_decode.md)
