...
type: ADR
name: ADR-QUALITY
title: "ADR-QUALITY (0141) — what \"good\" means: the four-gate funnel, the declared mix, and the published cost of an error"
...
[ADR-RS](0133_predictions.md) governs *how* a claim is frozen and is silent on
...
2a. **A BAR ON `recall@k` NAMES ITS `k`, OR IT IS VOID.** `recall@k` is not one
   ...
   [ADR-RS](0133_predictions.md)'s moving threshold wearing an omission instead
   of an edit.
...
3. **`nDCG` and `MRR` are diagnostics, not the headline**, and the demotion is
   ...
   **reranker follows retrieval** ([ADR-RERANK](0138_rerank.md)), so the
...
12. **The RANK CONTRACT and the RELEVANCE SET are two fields, not one.**
    ...
    **Four rules, and each closes a specific way this can go wrong:**
    ...
    the system it judges ([ADR-RS](0133_predictions.md) decision 13). The
...
[ADR-RS](0133_predictions.md) decision 19 — never a difference of aggregates.
...
- [ADR-RS](0133_predictions.md) — the freezing discipline this record supplies a
  quantity for
...
**Records** — [ADR-RS](0133_predictions.md) · [ADR-RERANK](0138_rerank.md)
