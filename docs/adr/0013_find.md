...
type: ADR
name: ADR-FIND
title: ADR-FIND (0013) — the find verb
...
[ADR-ASK](0012_ask.md) decision 8's `§` lines are on `ask` alone: a heading in
this stream would be read as a filename. The same reasoning keeps the
`[archived]` marker off stdout ([ADR-DIR-LIST](0029_dir-list.md) decision 12) —
...
exists and is fresh ([ADR-ASK](0012_ask.md) decision 4).
...
refused for [ADR-EXPAND](0060_expand.md) decision 11's reason: it would make
`support` describe a retrieval depth the caller never asked for, where
[ADR-CONFIDENCE](0051_confidence.md) bounds it by `--top`. One consistent
...
phrase"* on the strength of not having looked — [ADR-RERANK](0047_rerank.md)
...
- **`No confident matches.` goes to stdout, not stderr, and exits 0.** In a pipe
  that is a line of prose where a path was expected. A script consuming `find`
  should check for empty output rather than assume every line is a path — or use
  `--json`, where the empty case is `{"results": []}`.
...
- The shared ranking it projects — [ADR-ASK](0012_ask.md).
...
computed after filtering rather than before; or a real script is observed
breaking on the no-match line.
...
fux find "any query" --top 20 --band 2>&1 >/dev/null | grep '^\[band\]'
fux find "any query" --top 20 --band --under docs 2>&1 >/dev/null | grep -E '^\[band\]|^\[filter\]'

# 5. the no-match line is still the only prose on stdout
fux find "zzz nonexistent term"
...
[ADR-DIR-LIST](0029_dir-list.md) · [ADR-RERANK](0047_rerank.md) ·
[ADR-CONFIDENCE](0051_confidence.md) · [ADR-EXPAND](0060_expand.md)
