...
title: "ADR-MERGE-DRIVER (0034) — the committed index merges line by line, last-writer-wins on `(ver, sha)`, and refuses rather than guesses"
...
- **Status:** proposed — the design is unchanged from the day it shipped; its
...
- **Split from:** [ADR-MAINTENANCE](0033_hooks.md), which keeps the hooks, the
...
**This record was decisions 6–9 of [ADR-MAINTENANCE](0033_hooks.md) until
...
[ADR-MAINTENANCE](0033_hooks.md)'s `fux hooks`, and is not re-decided here.
...
```bash
# 1 and 2 — the four refusal cases, and the three-tier harness
