...
title: ADR-RUNTIME-STATS (0034) — stats.json, the corpus-wide numbers BM25F needs
...
two paths returning different bytes, which is [ADR-ASK](0012_ask.md)'s
...
rebuild is not a knob**, and the whole claim under [ADR-TUNE](0044_tuning.md) is
...
([ADR-T1-ACCELERATOR](0019_accelerator.md) §The weighted bound).
...
- ⚠ **This record owns no module, so no mechanical check can point at it.**
  ...
  `derive/_build.py` under [ADR-T1-ACCELERATOR](0019_accelerator.md). A change to
  this file satisfies
...
- The parent record — [ADR-T1-ACCELERATOR](0019_accelerator.md); the scorer that
  consumes the result — [ADR-RANKING](0020_ranking.md).
...
[ADR-ASK](0012_ask.md) · [ADR-RECORD](0018_index-record.md) ·
[ADR-T1-ACCELERATOR](0019_accelerator.md) · [ADR-RANKING](0020_ranking.md) ·
[ADR-RUNTIME-MANIFEST](0032_runtime-manifest.md) · [ADR-TUNE](0044_tuning.md)
