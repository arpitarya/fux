...
type: ADR
name: ADR-REFER
title: "ADR-REFER (0127) — the refer plane: fetch from the source, verify by content, assemble under a byte budget"
...
[ADR-FETCHER](0117_fetcher.md) already established: the consumer owns the
...
[ADR-FETCHER](0117_fetcher.md)'s veto fire on its own successor. A further
...
**9. The caches are [ADR-CACHE](0131_cache.md)'s** — the ARC content cache keyed
...
([ADR-RERANK](0138_rerank.md)) now scores their passages, over the same
...
**23. The citation path DECODES, and the seam is in `refer()`, not
`source.py`.** Until 2026-09-06 this plane chunked
`result.content.decode("utf-8", errors="replace")` — the raw bytes. For a
`file:` document that meant a `.docx` was split as the mojibake of a zip archive
and a `.pdf` as the mojibake of its binary: **every decoder's output reached
`extract.py` and nothing else**, so a format could rank perfectly well and be
impossible to quote.

- **`_read_local` still returns raw bytes**, and must. `content_sha` hashes them
  and the `current`/`stale`/`unverified` verdict is that hash compared against
  the index's. Decoding before hashing would silently redefine what freshness
...
range is only meaningful when the passage's lines are lines of a file on disk —
and for a decoded `.docx` the chunked text is Markdown that exists nowhere.

- `chunk(..., line_numbers=False)` zeroes the range, and `_rescore.locator`
  falls back to `path#p3`. The passage's **heading travels beside it** in the
  `Citation`, which is what `query/__init__.py` already prints.
- **Ruled by Arpit 2026-09-06**, choosing this over synthesising line numbers
  against generated Markdown: a citation that looks openable and is not is worse
  than one that admits it is an ordinal. This is decision 17's own reasoning
  applied to the case that produced it.
- ⚠ **A local `.html` loses its line range** as a consequence — the raw HTML had
  real lines and the converted Markdown does not. The trade is deliberate:
  passages that match what the index actually holds, against locators that
  pointed into markup nobody reads.
...
- **The repeated header is the ONE exception to totality.** Every *content* byte
  still lands in exactly one passage; the header row and its `|---|` separator
  appear in each band because a band whose columns have no names is a citation
  nobody can read.
- **It is also why the splitter reports source lines rather than its own line
  count.** A band holds more lines than it covers, and counting its own would run
...
- **`MAX_TABLE_BAND_BYTES = 900` instead.** A table has no narrative
  continuity — row 41 does not depend on row 40 the way a paragraph depends on
  the one above it — so the reason prose bands are large does not apply. 900
  bytes is ~11 rows: **43 passages instead of 10 on that file, overhead down
  from 58 % to 9 %**, header intact, and a locator like `owners.csv:L41-L52`
...
- 🔴 **The floor was eating structural boundaries, and that is a defect rather
  ...
  `Slide 3` carried slide 4's content; **six small `.jsonl` records emitted six
...
- **The rule is DEPTH, not size and not company.** A short section folds
  ...
  each other ([ADR-CHUNKING](0153_chunking.md) decision 1).
...
([ADR-CHUNKING](0153_chunking.md) decision 1). ⚠ **This decision shipped on
...
- **The document title is excluded from naming a passage** — see
  [ADR-CHUNKING](0153_chunking.md) decision 1a. Without that, `# deck.pptx`
...
[ADR-CHUNKING](0153_chunking.md) on 2026-09-06, which owns what a passage IS —
...
- **The display cache is a third store and is not this plane's.** It is
  ...
  citation.** Full rationale on [ADR-RECORD](0109_index-record.md).
...
- **Putting the TTL cache inside ARC.** Rejected — [ADR-CACHE](0131_cache.md)
...
**Records** — [ADR-RECORD](0109_index-record.md) ·
[ADR-FETCHER](0117_fetcher.md) · [ADR-CACHE](0131_cache.md) ·
[ADR-TUNE](0135_tuning.md)
