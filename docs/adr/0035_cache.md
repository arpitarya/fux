...
type: ADR
name: ADR-CACHE
title: "ADR-CACHE (0035) — two caches, two different proofs: ARC decides what a fetch returned, the TTL store decides whether to fetch at all"
...
- **Status:** proposed — carries [ADR-REFER](0031_refer-plane.md)'s status
...
- **Owns:** `src/fux/refer/arc.py` · `src/fux/refer/fetchcache.py` — carved out
  ...
  the same way [ADR-MERGE-DRIVER](0034_merge-driver.md) takes
...
- **Split from:** [ADR-REFER](0031_refer-plane.md), which keeps the fetch
...
index recorded, and quotes it ([ADR-REFER](0031_refer-plane.md) decisions 1–5).
...
([ADR-RUNTIME-STAMP](0028_runtime-stamp.md)): derived, per-machine,
...
already-tagged `runtime/` ([ADR-CACHEDIR-TAG](0024_cachedir-tag.md)), so backup
...
- The parent record — [ADR-REFER](0031_refer-plane.md), decisions 4, 6, 7
