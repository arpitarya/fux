...
title: "ADR-MERGE-DRIVER (0039) — the committed index merges line by line, last-writer-wins on (ver, sha), and refuses rather than guesses"
...
`sha` changes ([ADR-RECORD](0018_index-record.md)), which is what makes *higher*
...
`.gitattributes` line is [ADR-MAINTENANCE](0038_hooks.md)'s `fux hooks`. ⚠ **The
...
- `ver` and `sha`, whose relationship decision 1 rests on:
  [ADR-RECORD](0018_index-record.md).
...
[ADR-RECORD](0018_index-record.md) · [ADR-POSTINGS](0021_postings.md) ·
