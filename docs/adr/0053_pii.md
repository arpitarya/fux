...
1. **Redaction applies to `.fux/index/` and to nothing else.** The acquired
   ...
   *sensitive vs not*.
   ...
   Closed by W-102; see [ADR-ENRICH](0040_enrich.md) decision 12 for the fix and
   ...
   *plane*, the code redacts a *phase*, and anything that reaches the plane
   ...
   backslashed twin. Fixed in [ADR-ENRICH](0040_enrich.md) decision 14, noted
...
- **Refuse the document on a PII hit, like a refusal.** Rejected: it loses a
  ...
  ([ADR-ENRICH](0040_enrich.md) decision 12), because a document is a fact fux
