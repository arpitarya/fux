...
type: ADR
name: ADR-RECORD
title: ADR-RECORD (0019) — one line of the committed index, property by property
...
| `ver` | bumps strictly when *this* record's `sha` changes — never on an edge change. A version is a statement about the document, not its neighbourhood ([ADR-INGEST](0016_ingest.md)) |

**Retrieval — what ranking consumes:**

| property | purpose |
|---|---|
| `terms` | the postings: `{16-hex term hash: [tf per field]}`, in `tf_fields` order, trailing zeros omitted. See [ADR-POSTINGS](0022_postings.md) |
...
([ADR-TUNE](0045_tuning.md) decision 6) is what put the tunable back on the
...
| `archived` | true when the document was ingested under an archived-content rule. The record states the rule it was written under rather than every reader recomputing it ([ADR-ARCHIVED-CONTENT](0044_archived-content.md)); **written only when true** |
...
[ADR-TUNE](0045_tuning.md) decision 1, and putting the facts on the committed
...
| `phrases` | heading-derived phrases — what [ADR-ANSWER](0015_answer.md) returns, and what [ADR-ASK](0013_ask.md) decision 8 cites as `§` headings. **`"plain"` only** |
...
([ADR-INDEX-LIFECYCLE](0018_index-lifecycle.md) decision 11). Neither is a
...
- **Commit dense vectors.** Built and removed. Measured on this repo before
  ...
  ([ADR-ASK](0013_ask.md) decision 9). The bar at the top of §2 is what it
  failed.
