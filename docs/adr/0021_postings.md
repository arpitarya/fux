...
type: ADR
name: ADR-POSTINGS
title: ADR-POSTINGS (0021) — the postings, committed and derived
...
**Sparsity is what paid for five fields instead of two.** Trailing zeros are
never written, so a body-only posting is `[1]` rather than `[1,0,0,0,0]`.
Measured on this repo, **92.5 % of postings are body-only**, so the sparse
body-first encoding came in **−36.7 %** on tf bytes *while the field count went
from two to five*; the naive dense five-slot form would have been **+24 %**. The
field order is therefore a committed format decision, not a preference — see
[ADR-RANKING](0020_ranking.md).
...
**The full postings are permanent.** Pruning them was measured and failed — the
best selector landed 35.9 points below unpruned recall@20 at 6 % retention.
...
written because a later slot is non-zero; and a term in body, heading, title and
path at once. The same trimming applies to `flen`, which for this document is
`[1827, 141, 6, 6]` — four numbers, not five, because `ctx` is empty and an
...
alongside the two other things a reader cannot recover from the postings
themselves:
...
`store/reader.py` **refuses** a shard whose header does not match the running
build rather than mixing it in — two analyzers in one index is undetectable at
query time and corrupts every `df`, and a `[1, 2]` read under a different field
order silently swaps body for heading.
...
⚠ **That argument is structural, and does not rest on corpus size.** A posting
list is keyed by term, so editing one document rewrites every line for every
word it contains, whatever the corpus size. Scale sets the **magnitude** of the
damage, not its direction: at the 10 000-document design point, with an ordinary
Zipfian vocabulary, a common term's posting list still runs to thousands of
entries, so a one-word edit still produces a diff nobody reads and still
conflicts with an unrelated branch touching the same word. **The smallest corpus
at which the argument stops holding has never been measured**, because the
...
**1. Committed postings are inline and doc-major.** `terms` in the record maps a
**16-hex term hash** to a **five-element sparse tf list** — one slot per field
in `TF_FIELDS` order, trailing zeros omitted. One line per document is the
...
**3. Field order is pinned in the header**, not by convention. `tf_fields` makes
`[0, 1]` unambiguous to a reader that has never seen this code.

**4. Term frequencies are integers.** No floats anywhere in the committed plane
— floats are not byte-reproducible across platforms, and the byte-identical
guarantee is the whole basis of the design.
...
**The tf list is nested, not flattened into the row**, and that is the shape
that survives a field count changing: a flat row has to be re-specified every
time the arity moves, and a reader has no way to tell a two-field row from a
truncated three-field one. Nesting also lets the derived posting reuse the
committed record's tf list **verbatim**, trailing zeros already trimmed, which
is what makes the two planes checkably the same information rather than two
encodings of it.
...
contiguous read. Its two summary extrema are **per-field arrays and deliberately
unweighted** — a weighted extremum cannot be precomputed once when the field
weights are query-time tune keys. **The offset table is derived and
disposable**, so its size costs a rebuild and nothing in git, which is exactly
why the arrays could go there rather than into the committed plane. Contents,
layout and rationale in [ADR-T1-ACCELERATOR](0019_accelerator.md).
...
points below unpruned recall@20.

**9. Stopwords never become postings.** Filtering happens in the shared
analyzer, so a stopword is absent from both planes rather than filtered at query
time ([ADR-RANKING](0020_ranking.md)).
...
- ⚠ **Pruning work is forbidden outside a dedicated, signed-off item, and this
  ...
  into this record. **If pruning appears in any milestone's diff, that is a plan
  violation, not a bonus** — because a pruning change looks like a size win and
  is measured as a recall loss, so it is exactly the kind of work that gets
  waved through on the wrong metric.
- **A document edit is one line in one shard.** The reviewable-diff property the
  committed index exists for.
...
- **A term hash is only meaningful within one analyzer version.** `analyzer` in
  the header is what makes that checkable; change the analyzer and every posting
  must be rebuilt.
- **Full postings set the committed size floor**, and the only lever that would
  have lowered it is closed. Committed size is a number to watch, never a gate:
  a packed-size promise at 100 000 documents was retired by ruling and has no
  successor. The preliminary read on this repo measured **2.429×** git-pack
  compression, against today's plain-JSON placeholder rather than a designed
  encoding — see
  [the analysis](../../work/regression/2026-08-21-r7-preliminary-analysis/ANALYSIS.md)
  before treating it as evidence about the design.
...
- **A dense five-slot tf list.** Rejected on measurement: **+24 %** on tf bytes
  against the sparse body-first form's **−36.7 %**.
...
- The pruning verdict —
  [P1-RERUN](../../work/regression/2026-08-09-pruning-rerun/VERDICT.md), and the
  harness this record owns, [`tools/pruning-eval/`](../../tools/pruning-eval/).
...
- The other generated files the derived plane writes alongside `postings/`, each
  with its own record — [ADR-CACHEDIR-TAG](0030_cachedir-tag.md),
  [ADR-DOCS-TABLE](0031_docs-table.md),
  [ADR-RUNTIME-MANIFEST](0032_runtime-manifest.md),
  [ADR-RUNTIME-STAMP](0033_runtime-stamp.md),
  [ADR-RUNTIME-STATS](0034_runtime-stats.md).

### Veto condition

**Reopen this decision if** the two shapes stop agreeing, or if a phrase-query
requirement arrives.
...
# 4. committed index size — informational only, never a gate.
#    Print the number, watch it over time, and read NO pass or fail off it.
...
[ADR-INDEX-LIFECYCLE](0017_index-lifecycle.md) ·
[ADR-T1-ACCELERATOR](0019_accelerator.md) · [ADR-RANKING](0020_ranking.md) ·
[ADR-CACHEDIR-TAG](0030_cachedir-tag.md) ·
[ADR-DOCS-TABLE](0031_docs-table.md) ·
[ADR-RUNTIME-MANIFEST](0032_runtime-manifest.md) ·
[ADR-RUNTIME-STAMP](0033_runtime-stamp.md) ·
[ADR-RUNTIME-STATS](0034_runtime-stats.md)
...
- [`tools/pruning-eval/`](../../tools/pruning-eval/)
