...
title: "ADR-URL-FRESHNESS (0058) — six verdicts, and a per-URL ttl that can only narrow"
...
[ADR-ACQUIRED](0056_acquired-plane.md) put the fetched bytes on disk, which made
...
- ~~**`fux doctor` does not report the `as-ingested` share.**~~ **Closed
  ...
  [ADR-ACQUIRED](0056_acquired-plane.md)'s identical one can now be run.
...
- [ADR-ACQUIRED](0056_acquired-plane.md) — the plane the fourth verdict reads from
- [ADR-CACHE](0040_cache.md) — the TTL store `ttl=` bounds, and the argument for keeping two caches provably separate
...
[ADR-REFER](0036_refer-plane.md) · [ADR-CACHE](0040_cache.md) ·
[ADR-PROVENANCE](0052_provenance.md) · [ADR-ACQUIRED](0056_acquired-plane.md) ·
[ADR-REFUSAL](0057_refusals.md)
