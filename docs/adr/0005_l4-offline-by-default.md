...
type: ADR
name: ADR-L4
title: "ADR-L4 (0005) — L4 — offline by default"
...
# ADR-L4 — L4 — offline by default

## §1 — For humans

> **This record is the RATIONALE for law L4. It is not the law.**
...
**The handle:** *Offline by default* — the one-line form from [ADR-LAWS](0001_laws.md)'s
...
**Network code lives in the consumer's repo, not in the package.** `fux setup` writes `http.py` and `cdp.py` into `.fux/fetchers/`, where they become the consumer's own code. That is why the package can keep zero network lines while URLs still work — and it is a decision of [ADR-CDP-FETCHER](0027_cdp-fetcher.md), not of this law.
...
**This is the restatement hazard in its purest form** — text that was true when written, became the thing everyone cited, and then made a legal new case look like a law change. It is why [ADR-LAWS](0001_laws.md) forbids restating a law, and why the named paths live in [ADR-CLI](0010_cli-surface.md) rather than here.
...
flowchart LR
    ...
    R["ADR-L4<br/>(this record — rationale, history, veto)"]
...
- ⚠ **L4 does not close the use-record gap.** [L8](0009_l8-use-record.md)'s 2026-08-27 ratification dropped its transmission clause, and L4 is *offline by default* with an existing fenced path — a journal POSTed through that fence would not obviously violate it. **What holds is the code, not this law.**
...
- [ADR-CLI](0010_cli-surface.md) decision 1e — the named fenced paths, cited rather than restated
- [ADR-FETCHER](0026_fetcher.md) · [ADR-CDP-FETCHER](0027_cdp-fetcher.md) — consumer-owned network code
- [ADR-PROVENANCE](0052_provenance.md) — `fux verify` never fetches
