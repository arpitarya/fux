...
type: ADR
name: ADR-DECODE
title: "ADR-DECODE (0139) — the decoder plane: bytes become Markdown in one place, and a consumer may bring a dependency fux may not"
...
[ADR-ENRICH](0137_enrich.md) decision 1's table gaining a third row:
...
[ADR-TYPES](0128_types-list.md) decision 1 for what stays out and why, **and for
...
- **There is no `.py.txt` template.** A fetcher needs one because it carries
  ...
  ([ADR-CDP-FETCHER](0118_cdp-fetcher.md) decision 9). **A decoder is
...
built-in reverses the SVG half of [ADR-TYPES](0128_types-list.md) decision 5
...
- 🔴 **A decoder change does NOT invalidate carried extraction.** Reuse is keyed
  ...
  [ADR-INGEST](0106_ingest.md)'s reuse key, not this record's.
...
[ADR-ENRICH](0137_enrich.md) decision 4 derives scope from a declaration; a
...
that owns the grammar is [ADR-TYPES](0128_types-list.md) decision 11, and this
is what it means for dispatch.

⚠ **Where the binding lives moved on 2026-09-11** ([ADR-TYPES](0128_types-list.md)
...
[ADR-TYPES](0128_types-list.md) decision 11a carries the table and the argument
...
([ADR-CHUNKING](0153_chunking.md) decision 1). The evidence is that **`pdf`,
...
DEFINED in [ADR-CHUNKING](0153_chunking.md) and deliberately not restated here;
...
- [`tests/decode/test_decode.py`](../../tests/decode/test_decode.py) — in
  ...
  `PYTHONHASHSEED`.
- [ADR-ENRICH](0137_enrich.md) decision 1 — the table this record extends, and
  the precedent that a consumer boundary is a design choice rather than a
  dependency budget; [ADR-TYPES](0128_types-list.md) — what the widened default
  admits and what it still refuses.
- The seam it changed in ingest — [ADR-INGEST](0106_ingest.md) decision 11; the
  fetchers that stopped converting —
  [ADR-HTTP-FETCHER](0119_http-fetcher.md) and
...
3. **The default type allowlist is ever derived from the LIVE registry** rather
   ...
   ([ADR-TYPES](0128_types-list.md) decision 1a).
...
**Records** — [ADR-LAWS](0001_LAWS.md) · [ADR-INGEST](0106_ingest.md) ·
[ADR-EXTRACTED](0115_extracted-mode.md) ·
[ADR-CDP-FETCHER](0118_cdp-fetcher.md) ·
[ADR-HTTP-FETCHER](0119_http-fetcher.md) · [ADR-TYPES](0128_types-list.md) ·
[ADR-AGENT-POLICY](0132_agent-policy.md) · [ADR-ENRICH](0137_enrich.md) ·
[ADR-CHUNKING](0153_chunking.md)
