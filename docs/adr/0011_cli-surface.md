...
type: ADR
name: ADR-CLI
title: ADR-CLI (0011) — the command-line surface
...
more read verbs — see [ADR-GRAPH](0036_graph.md).
...
- **The defaults are decisions, not conveniences.** The scan is the default
  ...
  [ADR-T1-ACCELERATOR](0020_accelerator.md).
...
creates one ([ADR-DOTFUX](0012_fux-directory.md) decision 6). Everything it
...
[ADR-MAINTENANCE](0039_hooks.md)'s.
...
[ADR-MCP](0046_mcp.md) owns the JSON-RPC protocol and the tool surface;
[ADR-MAINTENANCE](0039_hooks.md) owns what the daemon does. What binds here is
...
rewrite ([ADR-DOTFUX](0012_fux-directory.md)). The human pastes; the file stays
...
| **it filters what is reported; it cannot make a document plannable** | a document no `enrich=true` line reaches stays unenrichable, and naming it here changes nothing ([ADR-ENRICH](0047_enrich.md) decision 4 — which directories are enriched is a human's declaration in `fux.toml`) |

⚠ **It is also not a gated flag, so [ADR-OUTPUT](0054_output-defaults.md)
...
[ADR-PROVENANCE](0053_provenance.md) emits a receipt from `fux answer
...
11. **`ask` gained a `--sections` / `--no-sections` pair — the decision is
    [ADR-OUTPUT](0054_output-defaults.md) decision 21, noted here only because
...
([ADR-AGENT-POLICY](0042_agent-policy.md) decision 5). Its durable form is
...
never clears it ([ADR-MAINTENANCE](0039_hooks.md) decision 1c).
...
[ADR-INGEST](0016_ingest.md) decision 4, and the summary count is untouched by
...
| `--full` | re-extract every document instead of carrying unchanged ones forward. **Bytes are unaffected** — only speed, and it is the complete term-collision check ([ADR-INGEST](0016_ingest.md) decision 1b) |
...
[ADR-MAINTENANCE](0039_hooks.md) decision 1d.
...
conjunction with no precedence ([ADR-DIR-LIST](0030_dir-list.md) /
[ADR-TYPES](0038_types-list.md)), so the line is written, the check still runs,
...
The `*` is [ADR-URL-LIST](0026_url-list.md) decision 13 made visible: the
...
([ADR-MAINTENANCE](0039_hooks.md) decision 1b), the committed index can be
...
[ADR-URL-INGEST](0017_url-ingest.md) decision 8.
...
zero. See [ADR-FETCHER](0027_fetcher.md) decision 12.

⚠ **`ask`, `find` and `answer` gained `--expand TEXT`, and `ask`/`find` gained
a repeatable `-q/--query`, on 2026-09-05** (W-109,
[ADR-EXPAND](0061_expand.md)). Both are plain valued options, so decision 10's
...
⚠ **`answer` takes `--expand` and NOT `-q`** — [ADR-ANSWER](0015_answer.md)
...
2026-09-05** (W-111, [ADR-FIND](0014_find.md)). `--phrase` and `--under` are
...
- 🔴 **`_apply_output_defaults` no longer degrades when `.fux/output.toml` is
  ...
  [ADR-OUTPUT](0054_output-defaults.md) decision 19 made the file the sole
  ...
  ([ADR-DOTFUX](0012_fux-directory.md) decision 6), so it reaches new repos
...
- **The `--json` shape is a contract, in three shapes, and they do not
  ...
  `citation={id, loc, sha, freshness}` — [ADR-ANSWER](0015_answer.md)); and the
...
- The measured basis for `--fast` being off by default —
  [ADR-T1-ACCELERATOR](0020_accelerator.md) and the
...
[ADR-ANSWER](0015_answer.md) · [ADR-INGEST](0016_ingest.md) ·
[ADR-T1-ACCELERATOR](0020_accelerator.md) · [ADR-URL-LIST](0026_url-list.md) ·
[ADR-DIR-LIST](0030_dir-list.md) · [ADR-GRAPH](0036_graph.md) ·
[ADR-TYPES](0038_types-list.md) · [ADR-MAINTENANCE](0039_hooks.md) ·
[ADR-AGENT-POLICY](0042_agent-policy.md) · [ADR-MCP](0046_mcp.md) ·
