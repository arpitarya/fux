...
type: ADR
name: ADR-CONFIG
title: ADR-CONFIG (0113) — fux.toml and every property in it
...
`.fux/sources/dirs` ([ADR-DIR-LIST](0120_dir-list.md) decision 1).
...
`fux setup` is write-if-missing ([ADR-DOTFUX](0102_fux-directory.md)), so a
...
for ([ADR-AGENT-POLICY](0132_agent-policy.md) decision 5). ⚠ **`codex` joined
2026-09-06** ([ADR-AGENT-POLICY](0132_agent-policy.md) decision 11): the set is
...
| `[sources.url] middleware` | renamed to `fetcher`; move the file to `.fux/fetchers/` ([ADR-FETCHER](0117_fetcher.md) decision 7) |
| `[sources] dirs` | put one directory per line in `.fux/sources/dirs`; a line may carry `archived=true` ([ADR-DIR-LIST](0120_dir-list.md) decision 1) |
| `[ranking]` (whole table) | moved to `.fux/tune.toml`; run `fux setup` to write the file, move the keys across, delete the table ([ADR-TUNE](0135_tuning.md) decision 7) |
| `[decode]` (whole table) | moved to `.fux/tune.toml [index]` on 2026-09-11 (Arpit), beside `max_phrases`; move `max_table_rows` across and delete the table ([ADR-TUNE](0135_tuning.md) decision 13) |
| `[dense]` (whole table) | **removed**, not relocated — the lane it configured no longer exists ([ADR-ASK](0103_ask.md) decision 9). The error states the removal, the verdict behind it, and that ranking does not move, because `mode` defaulted to `off` |
...
[ADR-TABULAR](0152_tabular.md) added it on 2026-09-06 as a **fourth** top-level
...
a source-wide *layer*, not a setting.** `keep` ([ADR-ACQUIRED](0147_acquired-plane.md)),
`ttl` ([ADR-URL-FRESHNESS](0149_url-freshness.md)) and `enrich`
([ADR-PII](0150_pii.md)) each sit between the built-in default and the URL
...
`update=` attribute ([ADR-URL-LIST](0116_url-list.md) decision 14). `"auto"` or
...
- **A third source-list path constant lives here, with no key at all.**
  ...
  nobody asked for. Decided in [ADR-TYPES](0128_types-list.md). ⚠ **It was
...
[ADR-ASK](0103_ask.md) · [ADR-FETCHER](0117_fetcher.md) ·
[ADR-HTTP-FETCHER](0119_http-fetcher.md) · [ADR-DIR-LIST](0120_dir-list.md) ·
[ADR-TYPES](0128_types-list.md) · [ADR-AGENT-POLICY](0132_agent-policy.md) ·
