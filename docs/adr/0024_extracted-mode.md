...
type: ADR
name: ADR-EXTRACTED
title: "ADR-EXTRACTED (0024) — the extracted ingest mode"
...
counterpart is [ADR-ENRICH](0046_enrich.md), which absorbed ADR-ENRICHED on
...
the `enriched` mode ([ADR-ENRICH](0046_enrich.md)) is built.
...
- **`flen` rather than a weighted length**, because a weighted length computed
  ...
  ([ADR-TUNE](0044_tuning.md) decision 6). Raw counts are facts; the weighting
...
([ADR-ASK](0012_ask.md) decision 9); that awkwardness went with it.
...
Markdown by contract ([ADR-DECODE](0048_decode.md) decision 2). Only an
...
[ADR-DECODE](0048_decode.md) decision 14 for why one grammar and what the two
...
- ⚠ **`fux enrich` does not build the enriched mode, and the difference is
  ...
  the `enriched` mode ([ADR-ENRICH](0046_enrich.md)) remains unbuilt.
...
[ADR-INGEST](0015_ingest.md) · [ADR-ENRICH](0046_enrich.md) ·
[ADR-TUNE](0044_tuning.md) · [ADR-DECODE](0048_decode.md)
