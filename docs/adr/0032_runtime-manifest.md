...
title: ADR-RUNTIME-MANIFEST (0032) — manifest.json, the accelerator's staleness fingerprint
...
([ADR-T1-ACCELERATOR](0019_accelerator.md)). Proving it — rather than merely
...
`stamp.json` ([ADR-RUNTIME-STAMP](0033_runtime-stamp.md)) is a cheap pre-filter
...
- **The `docs_fields` half of the contract** — the field set at
  ...
  (`is_fresh`).
- The parent record — [ADR-T1-ACCELERATOR](0019_accelerator.md); the table it
  pins — [ADR-DOCS-TABLE](0031_docs-table.md).
