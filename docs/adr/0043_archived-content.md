...
title: "ADR-ARCHIVED-CONTENT (0043) — what a document declared archived does, once it is indexed"
...
> [ADR-DIR-LIST](0029_dir-list.md)** — two different questions that once shared
> one document.

## §1 — For humans

A directory (or, through [ADR-URL-LIST](0025_url-list.md)'s shared grammar, a
...
```console
$ fux ask "what is the ingest cache" --top 5
3.4417             How ingest works today        (docs/adr/0015_ingest.md)
...
2.1188             Delta ingest and reuse        (docs/adr/0015_ingest.md)
...
[ADR-DIR-LIST](0029_dir-list.md) decides the file, its grammar, and that the
...
[ADR-RECORD](0018_index-record.md)'s reason: **a record read years later states
...
`ask` show the same fact, because [ADR-FIND](0013_find.md) makes `find` a
...
- **It keys off the declaration, never a path.**
  [ADR-DIR-LIST](0029_dir-list.md) decision 4 stands: a path prefix is not a
...
- **It states what archived *is*, not what the reader should *do* about it**,
  ...
  **A single sentence cannot instruct all three, and the list is not closed.**
  ...
  the consumer's policy, not the engine's** — [ADR-AGENT-POLICY](0041_agent-policy.md).
- **Fux does not take an `--intent` flag, and that is a decision.** Carrying a
  ...
  [refer plane](0036_refer-plane.md) set the precedent: it refuses to collapse
...
- **stdout stability applies.** `--json` is a contract and the surface captures
  ...
  [ADR-CLI](0010_cli-surface.md)'s call, taken there.
...
8. 6.4341 [live] docs/adr/0010_cli-surface.md          <-- the actual answer
```
...
- [ADR-DIR-LIST](0029_dir-list.md) — the file and grammar this record's
  behaviour attaches to.
- [ADR-RECORD](0018_index-record.md) — the record schema `archived: true` joins;
  [ADR-FIND](0013_find.md) — why `find` shows the same fact as `ask`;
  [ADR-CLI](0010_cli-surface.md) — stdout stability, which puts the disclaimer
  on stderr.
- [ADR-REFER](0036_refer-plane.md) — the `current`/`stale`/`unverified`
  precedent for stating a fact and refusing to interpret it, behind decision 7;
  [ADR-AGENT-POLICY](0041_agent-policy.md) — where the interpretation lives
  instead.
...
[ADR-FIND](0013_find.md) · [ADR-RECORD](0018_index-record.md) ·
[ADR-URL-LIST](0025_url-list.md) · [ADR-DIR-LIST](0029_dir-list.md) ·
[ADR-REFER](0036_refer-plane.md) ·
[ADR-AGENT-POLICY](0041_agent-policy.md) · [ADR-TUNE](0044_tuning.md)
