...
type: ADR
name: ADR-ANSWER
title: ADR-ANSWER (0014) — the answer verb
...
([ADR-REFER](0036_refer-plane.md)), never a rewrite of them.

**4. One ANSWER is forced; top-1 is not.** There is no `--top`, and a caller
wanting alternatives still wants [`ask`](0012_ask.md) — but since W-108 the
...
This follows [ADR-REFER](0036_refer-plane.md)'s own reasoning: a `file:`
...
**It needs no fifth freshness label.** [ADR-REFER](0036_refer-plane.md)
...
removed or repurposed ([ADR-ASK](0012_ask.md) decision 11's rule).
...
[ADR-EXPAND](0060_expand.md) decision 12 records the split, and `--expand` is
...
The declared tie-break ([ADR-RANKING](0020_ranking.md)) runs in `rank()`, which
...
- **`--json` is validated against `query/output.schema.json` before it is
  printed** ([ADR-ASK](0012_ask.md) decision 11), so a key cannot be quietly
...
- 🔴 **`--band` reports a real `separation` on this verb for the first time,
  ...
  ([ADR-CONFIDENCE](0051_confidence.md) decision 14); what changed is that the
  number is now true.
...
- The ranking it projects — [ADR-ASK](0012_ask.md).
