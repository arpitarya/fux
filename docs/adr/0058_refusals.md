...
type: ADR
name: ADR-REFUSAL
title: "ADR-REFUSAL (0058) — the response the server sent instead of the document"
...
1. **Refusal is checked after `_unpack` and before persist and decode.** A
   ...
   ([ADR-ACQUIRED](0057_acquired-plane.md) decision 6).
...
4. **Every condition is pure over the bytes.** The six are `content_type`,
   ...
   ⚠ **This is [ADR-FETCHER](0027_fetcher.md) decision 13 holding, and it was
...
- [ADR-FETCHER](0027_fetcher.md) decision 13 — the boundary this record holds, and whose veto check named this module's caller
...
[ADR-URL-INGEST](0017_url-ingest.md) · [ADR-URL-LIST](0026_url-list.md) ·
[ADR-FETCHER](0027_fetcher.md) · [ADR-DECODE](0049_decode.md) ·
[ADR-ACQUIRED](0057_acquired-plane.md)
