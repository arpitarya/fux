...
type: ADR
name: ADR-RECORD
title: ADR-RECORD (0109) — one line of the committed index, property by property
...
| `ver` | bumps strictly when *this* record's `sha` changes — never on an edge change. A version is a statement about the document, not its neighbourhood ([ADR-INGEST](0106_ingest.md)) |

**Retrieval — what ranking consumes:**

| property | purpose |
|---|---|
| `terms` | the postings: `{16-hex term hash: [tf per field]}`, in `tf_fields` order, trailing zeros omitted. See [ADR-POSTINGS](0112_postings.md) |
...
([ADR-TUNE](0135_tuning.md) decision 6) is what put the tunable back on the
...
| `archived` | true when the document was ingested under an archived-content rule. The record states the rule it was written under rather than every reader recomputing it ([ADR-ARCHIVED-CONTENT](0134_archived-content.md)); **written only when true** |
...
[ADR-TUNE](0135_tuning.md) decision 1, and putting the facts on the committed
...
| `phrases` | heading-derived phrases — what [ADR-ANSWER](0105_answer.md) returns, and what [ADR-ASK](0103_ask.md) decision 8 cites as `§` headings. **`"plain"` only** |
...
([ADR-INDEX-LIFECYCLE](0108_index-lifecycle.md) decision 11). Neither is a
...
- **Commit dense vectors.** Built and removed. Measured on this repo before
  ...
  ([ADR-ASK](0103_ask.md) decision 9). The bar at the top of §2 is what it
  failed.
