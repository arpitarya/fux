...
title: "ADR-CDP-FETCHER (0028) — the CDP fetcher"
...
`fetch=cdp` on its line ([ADR-URL-LIST](0026_url-list.md)), not because a
cheaper fetch failed and something retried. That is
[ADR-FETCHER](0027_fetcher.md) decision 5, and it is what keeps the same URL
...
[ADR-FETCHER](0027_fetcher.md) says fux never fetches, which is only useful if a
...
it, and is covered by [ADR-PORT-LIST](0024_port-list.md)'s discipline of porting
...
[ADR-FETCHER](0027_fetcher.md) decision 8 in use.
...
- **`connect()` / `close()` stay once per fetcher group, never once per
  worker** — [ADR-FETCHER](0027_fetcher.md) decision 9's contract, unchanged.
...
([ADR-FETCHER](0027_fetcher.md) decision 9a), which both shipped fetchers accept
...
the download.** It is [ADR-FETCHER](0027_fetcher.md)'s optional fifth entry
...
([ADR-RECORD](0019_index-record.md) rule 2), and the absence of
...
([ADR-EXTRACTED](0025_extracted-mode.md)). **Borrowing a session is not
enrichment.**
...
- **This is the slow path.** A browser round trip per URL. It is why
  [ADR-HTTP-FETCHER](0029_http-fetcher.md) is the default and this is the
...
- **Making this the default fetcher.** Rejected: it demands a browser for URLs
  ...
  [ADR-HTTP-FETCHER](0029_http-fetcher.md).
...
**Records** — [ADR-LAWS](0001_LAWS.md) · [ADR-RECORD](0019_index-record.md) ·
[ADR-PORT-LIST](0024_port-list.md) · [ADR-EXTRACTED](0025_extracted-mode.md) ·
[ADR-URL-LIST](0026_url-list.md) · [ADR-FETCHER](0027_fetcher.md) ·
[ADR-HTTP-FETCHER](0029_http-fetcher.md) · [ADR-DECODE](0049_decode.md)
