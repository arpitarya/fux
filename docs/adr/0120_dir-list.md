...
type: ADR
name: ADR-DIR-LIST
title: "ADR-DIR-LIST (0120) — the committed directory list"
...
> a disclaimer — is [ADR-ARCHIVED-CONTENT](0134_archived-content.md).
...
Same reasons as the URL list ([ADR-URL-LIST](0116_url-list.md)): one entry per
...
on 2026-09-11 — [ADR-TYPES](0128_types-list.md) decision 12). `[sources] dirs` in `fux.toml` is a
**retired key that errors with instructions**
([ADR-CONFIG](0113_config.md) decision 10).

**2. The grammar is [ADR-URL-LIST](0116_url-list.md)'s**, by reference and not
...
⚠ **Exclusion's home is [`.fux/.fuxignore`](0145_fuxignore.md), and this is the
...
([ADR-URL-LIST](0116_url-list.md) decision 8) while a repeated key is an error
...
[ADR-ENRICH](0137_enrich.md) and is the one attribute whose effect on the index
is indirect — see [ADR-URL-LIST](0116_url-list.md) §The `dirs` attribute set.
...
([ADR-FUXIGNORE](0145_fuxignore.md) decision 4). A `!` line there is a
...
explicitly ([ADR-URL-LIST](0116_url-list.md) decision 12). This file is
...
- **`fux add --types` seeds the built-in allowlist when it creates the file**
  ([ADR-TYPES](0128_types-list.md)'s *absent means the default*), because the
...
- The grammar this record reuses — [ADR-URL-LIST](0116_url-list.md), and its one
  implementation,
...
- The key it retires — [ADR-CONFIG](0113_config.md) decision 10.
- The layout it extends — [ADR-DOTFUX](0102_fux-directory.md) decision 2.
...
[ADR-RECORD](0109_index-record.md) · [ADR-CONFIG](0113_config.md) ·
[ADR-URL-LIST](0116_url-list.md) · [ADR-FETCHER](0117_fetcher.md) ·
[ADR-TYPES](0128_types-list.md) ·
[ADR-ARCHIVED-CONTENT](0134_archived-content.md) ·
[ADR-ENRICH](0137_enrich.md)
