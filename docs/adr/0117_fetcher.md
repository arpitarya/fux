...
type: ADR
name: ADR-FETCHER
title: "ADR-FETCHER (0117) — the consumer-owned fetcher"
...
The name also had to avoid a collision. [ADR-RECORD](0109_index-record.md)
...
the exact collision [ADR-EXTRACTED](0115_extracted-mode.md) exists to close.
...
[ADR-URL-INGEST](0107_url-ingest.md) owns *how URL ingestion behaves* — refresh
...
[ADR-URL-LIST](0116_url-list.md)'s `fetch=` attribute, which resolves to
`<fetchers dir>/<name>.py` — the directory being the parent of
`[sources.url] fetcher` ([ADR-CONFIG](0113_config.md) decision 5). **A fetcher
...
[ADR-DOTFUX](0102_fux-directory.md). Plural, because decision 5 presumes more
...
here rather than only in [ADR-CDP-FETCHER](0118_cdp-fetcher.md): decision 5
...
`min(declared, DEFAULT_MAX_PARALLEL)` — see [ADR-CONFIG](0113_config.md)
...
`Skipped` and the batch continues — [ADR-URL-INGEST](0107_url-ingest.md)
...
move** (W-126, [ADR-ARCHIVED-CONTENT](0134_archived-content.md) decision 1a).
...
([ADR-URL-LIST](0116_url-list.md) decision 14) is filtered **above**
...
- **"adapter"** — the tempting one, because the surrounding prose already says
  "the adapter cap". Rejected: [ADR-RECORD](0109_index-record.md) defines `src`
...
[ADR-REFUSAL](0148_refusals.md) decision 11; the storage is
[ADR-MAINTENANCE](0129_hooks.md)'s `url-state.json`.
...
- The behaviour around the contract — [ADR-URL-INGEST](0107_url-ingest.md),
  captured in
...
[ADR-URL-LIST](0116_url-list.md) · [ADR-CDP-FETCHER](0118_cdp-fetcher.md) ·
[ADR-HTTP-FETCHER](0119_http-fetcher.md) · [ADR-REFER](0127_refer-plane.md) ·
