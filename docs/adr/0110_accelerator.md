...
title: ADR-T1-ACCELERATOR (0110) — the derived T1 accelerator
...
([ADR-RANKING](0111_ranking.md)). The differential law then reduces to "the
...
fails ([ADR-INDEX-LIFECYCLE](0108_index-lifecycle.md)).
...
(W-109, [ADR-EXPAND](0151_expand.md) decisions 6-7). `--expand` gives
...
- **The differential law now covers the confidence block too.** `accel.ask`
  ...
  ([ADR-CONFIDENCE](0142_confidence.md) decision 9).
...
- **`stats.json` stores RAW `total_flen`, not a pre-weighted total.** A stored
  ...
  is [ADR-RUNTIME-STATS](0125_runtime-stats.md).
...
- **`fux build` is a two-lane build, and the second lane is not this
  record's.** The graph plane ([ADR-GRAPH](0126_graph.md)) is written by the
...
[ADR-ASK](0103_ask.md) · [ADR-INDEX-LIFECYCLE](0108_index-lifecycle.md) ·
[ADR-RECORD](0109_index-record.md) · [ADR-RANKING](0111_ranking.md) ·
