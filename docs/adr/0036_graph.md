...
type: ADR
name: ADR-GRAPH
title: "ADR-GRAPH (0036) — the graph lane: communities, PPR-lite, and three relational verbs"
...
[ADR-CLI](0011_cli-surface.md)'s constraint is that there is none.
...
mode* name ([ADR-EXTRACTED](0025_extracted-mode.md) decision 5).
...
- **`edges_from_records` lifts without validating, and that is safe only because
  ...
  from it. **Fixed in ingest, not here** ([ADR-INGEST](0016_ingest.md)
...
- **`fux graph`'s seed query is an ordinary `run_query`**, so it inherits
  ...
  does not is exactly the divergence [ADR-ASK](0013_ask.md) decision 4 exists to
...
**Records** — [ADR-CLI](0011_cli-surface.md) · [ADR-ASK](0013_ask.md) ·
[ADR-INGEST](0016_ingest.md) · [ADR-EXTRACTED](0025_extracted-mode.md) ·
