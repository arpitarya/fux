...
type: ADR
name: ADR-EXPAND
title: "ADR-EXPAND (0060) — the caller supplies the vocabulary, and fuses its own phrasings"
...
unrepresentable. This is [ADR-TUNE](0044_tuning.md) decision 6's argument for
...
is additive ([ADR-ASK](0012_ask.md) decision 11's rule) and appears only when
...
caller never asked for, and [ADR-CONFIDENCE](0051_confidence.md)'s docstring
states plainly that `support` is bounded by `--top`.

**12. `answer` takes `--expand` and does not take `-q`.**
[ADR-ANSWER](0014_answer.md) decision 4: the verb means one answer to one
...
- Cormack, Clarke & Buettcher, *Reciprocal Rank Fusion outperforms Condorcet
  ...
  https://doi.org/10.1145/1571941.1572114
- [ADR-TUNE](0044_tuning.md) decision 6 — one object, not three parameters.
- [ADR-ANSWER](0014_answer.md) decision 4 — one answer to one question.
- [ADR-CONFIDENCE](0051_confidence.md) — the band, and `support`'s bound.
- [ADR-ASK](0012_ask.md) decision 11 — the additive-key rule for `--json`.
- [ADR-PORT-LIST](0023_port-list.md) rule 1 — a revival returns with a record.
