...
|---|---|---|
| **L1** | `$0`, FOSS-only — OSI-approved licences, shipped packaged | [ADR-L1](0002_l1-zero-cost.md) |
| **L2** | Content is never durable outside its source system | [ADR-L2](0003_l2-content-never-durable.md) |
| **L3** | Deterministic — no model in the maintenance path | [ADR-L3](0004_l3-deterministic.md) |
| **L4** | Offline by default | [ADR-L4](0005_l4-offline-by-default.md) |
| **L5** | Hashed meta is the default for non-git sources | [ADR-L5](0006_l5-hashed-meta.md) |
| **L6** | Say "index", not "db" | [ADR-L6](0007_l6-say-index.md) |
| **L7** | Python ≥ 3.11 | [ADR-L7](0008_l7-python-311.md) |
| **L8** | A use record is never committed | [ADR-L8](0009_l8-use-record.md) |
...
decision 1 is unchanged, and decision 3 binds these eight records exactly as it
binds every other.
...
belongs to [ADR-FETCHER](0026_fetcher.md) — a record with nothing to say about
the record shape.
...
[ADR-FETCHER](0026_fetcher.md)'s `MAX_PARALLEL` (declared, never detected,
...
[ADR-PROVENANCE](0052_provenance.md) decision C rules that **`fux verify` never
...
prohibition already existed, in [ADR-QUALITY](0050_quality-contract.md) decision
...
[ADR-PROVENANCE](0052_provenance.md) is the record that does.
...
reads `ADR-L3` and acts on its prose instead of the law has made the mistake
...
[ADR-L8](0009_l8-use-record.md) carries the same material as its own subject.
...
[ADR-L1](0002_l1-zero-cost.md); this row exists so a reader of the table above
...
- ⚠ **The veto check below cannot catch the failure this record exists to
  ...
  **narrower claim standing in for a law** — text that was true when written,
  ...
  named paths live in [ADR-CLI](0010_cli-surface.md) decision 1e, which cites
...
- L8's subject in code — [`src/fux/maintain/lastcited.py`](../../src/fux/maintain/lastcited.py),
  ...
  the law question: [ADR-QUALITY](0050_quality-contract.md) decision 11. The
  second is [`src/fux/query/provenance.py`](../../src/fux/query/provenance.py),
  owned by [ADR-PROVENANCE](0052_provenance.md), which exists because of the
...
**Records** — [ADR-CLI](0010_cli-surface.md) · [ADR-FETCHER](0026_fetcher.md) ·
[ADR-QUALITY](0050_quality-contract.md) · [ADR-PROVENANCE](0052_provenance.md) ·
