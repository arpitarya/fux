...
type: ADR
name: ADR-CHUNKING
title: "ADR-CHUNKING (0153) — a chunk does three jobs, and one span cannot do all of them well"
...
it is `.fux/tune.toml [index] max_table_rows` ([ADR-TABULAR](0152_tabular.md),
...
- ⚠ **Ownership moved.** `src/fux/refer/_chunk.py` is carved out of
  [ADR-REFER](0127_refer-plane.md)'s directory claim, on `freshness.py`'s
...
**Records** — [ADR-DECODE](0139_decode.md) decision 19 (`CHUNK`, added and
retired the same day) · [ADR-REFER](0127_refer-plane.md) decisions 25–27 (table
banding, and the two superseded runt rules) ·
[ADR-TABULAR](0152_tabular.md) (one passage per row, and the numbers)
