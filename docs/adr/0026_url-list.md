...
type: ADR
name: ADR-URL-LIST
title: "ADR-URL-LIST (0026) — the committed URL list"
...
keeps [ADR-INGEST](0016_ingest.md)'s byte-reproducibility true when two people
...
This record exists separately from [ADR-URL-INGEST](0017_url-ingest.md) because
...
define what `fetch=` selects. **The writer is [ADR-CLI](0011_cli-surface.md)'s**
...
declared **committed** by [ADR-DOTFUX](0012_fux-directory.md). The path is
configurable ([ADR-CONFIG](0023_config.md)); the format is not.
...
([ADR-URL-INGEST](0017_url-ingest.md) decision 3). **The file never records
fetch outcomes.**

**7. A line may carry attributes after the URL**, separated by whitespace:
`<url> key=value [key=value ...]`. **One parser, shared with
`.fux/sources/dirs`** ([ADR-DIR-LIST](0030_dir-list.md) decision 2) — two
...
[ADR-RECORD](0019_index-record.md) applies to `_format`: a reader that does not
...
of a key. This is the property [ADR-RECORD](0019_index-record.md) already gives
...
`[sources.url] fetcher` ([ADR-CONFIG](0023_config.md) decision 5) — so
relocating a repo's fetchers is a one-key change and never a per-line edit.
Exactly one runs ([ADR-FETCHER](0027_fetcher.md) decision 4), and nothing
escalates from one to another ([ADR-HTTP-FETCHER](0029_http-fetcher.md)
...
([ADR-FETCHER](0027_fetcher.md) decision 8). A `settle=500` in this grammar is
...
([ADR-EXTRACTED](0025_extracted-mode.md)), and a title supplied by the list
...
|---|---|---|
| `snapshot` | commit a machine-made copy of the content, per URL | the refer/snapshot policy is per *source* today and belongs to [ADR-REFER](0037_refer-plane.md); a per-URL form is an L2 decision, not a grammar one |
...
|---|---|---|
| `archived` | no — it routes ranking | [ADR-DIR-LIST](0030_dir-list.md) |
| **`enrich`** | **yes, indirectly** — a scope's documents gain a `ctx` field | [ADR-ENRICH](0047_enrich.md) |
...
- **Two records describe one subsystem**, deliberately: this one for the format,
  [ADR-URL-INGEST](0017_url-ingest.md) for the fetch contract. The split earns
...
- **A TOML array in `fux.toml`** — the original shape, **retired with an
  erroring key** ([ADR-CONFIG](0023_config.md) decision 10). One diff hunk, one
...
- The fetch contract this record is split from —
  [ADR-URL-INGEST](0017_url-ingest.md).
...
[ADR-URL-INGEST](0017_url-ingest.md) · [ADR-RECORD](0019_index-record.md) ·
[ADR-CONFIG](0023_config.md) · [ADR-EXTRACTED](0025_extracted-mode.md) ·
[ADR-FETCHER](0027_fetcher.md) · [ADR-CDP-FETCHER](0028_cdp-fetcher.md) ·
[ADR-HTTP-FETCHER](0029_http-fetcher.md) · [ADR-DIR-LIST](0030_dir-list.md) ·
[ADR-REFER](0037_refer-plane.md) · [ADR-ENRICH](0047_enrich.md)
