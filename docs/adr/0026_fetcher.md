...
type: ADR
name: ADR-FETCHER
title: "ADR-FETCHER (0026) — the consumer-owned fetcher"
...
The name also had to avoid a collision. [ADR-RECORD](0018_index-record.md)
...
the exact collision [ADR-EXTRACTED](0024_extracted-mode.md) exists to close.
...
[ADR-URL-INGEST](0016_url-ingest.md) owns *how URL ingestion behaves* — refresh
...
[ADR-URL-LIST](0025_url-list.md)'s `fetch=` attribute, which resolves to
`<fetchers dir>/<name>.py` — the directory being the parent of
`[sources.url] fetcher` ([ADR-CONFIG](0022_config.md) decision 5). **A fetcher
...
[ADR-DOTFUX](0011_fux-directory.md). Plural, because decision 5 presumes more
...
here rather than only in [ADR-CDP-FETCHER](0027_cdp-fetcher.md): decision 5
...
`min(declared, DEFAULT_MAX_PARALLEL)` — see [ADR-CONFIG](0022_config.md)
...
`Skipped` and the batch continues — [ADR-URL-INGEST](0016_url-ingest.md)
...
- **"adapter"** — the tempting one, because the surrounding prose already says
  "the adapter cap". Rejected: [ADR-RECORD](0018_index-record.md) defines `src`
...
[ADR-REFUSAL](0057_refusals.md) decision 11; the storage is
[ADR-MAINTENANCE](0038_hooks.md)'s `url-state.json`.
...
- The behaviour around the contract — [ADR-URL-INGEST](0016_url-ingest.md),
  captured in
...
[ADR-URL-LIST](0025_url-list.md) · [ADR-CDP-FETCHER](0027_cdp-fetcher.md) ·
[ADR-HTTP-FETCHER](0028_http-fetcher.md) · [ADR-REFER](0036_refer-plane.md) ·
