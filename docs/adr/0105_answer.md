...
type: ADR
name: ADR-ANSWER
title: ADR-ANSWER (0105) — the answer verb
...
([ADR-REFER](0127_refer-plane.md)), never a rewrite of them.
...
This follows [ADR-REFER](0127_refer-plane.md)'s own reasoning: a `file:`
...
**It needs no fifth freshness label.** [ADR-REFER](0127_refer-plane.md)
...
removed or repurposed ([ADR-ASK](0103_ask.md) decision 11's rule).
...
[ADR-EXPAND](0151_expand.md) decision 12 records the split, and `--expand` is
...
The declared tie-break ([ADR-RANKING](0111_ranking.md)) runs in `rank()`, which
...
- **`--json` is validated against `query/output.schema.json` before it is
  printed** ([ADR-ASK](0103_ask.md) decision 11), so a key cannot be quietly
...
- 🔴 **`--band` reports a real `separation` on this verb for the first time,
  ...
  ([ADR-CONFIDENCE](0142_confidence.md) decision 14); what changed is that the
  number is now true.
...
- The ranking it projects — [ADR-ASK](0103_ask.md).
