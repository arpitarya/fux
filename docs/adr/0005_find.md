...
`[archived]` marker off stdout ([ADR-DIR-LIST](0022_dir-list.md) decision 12) —
...
exists and is fresh ([ADR-ASK](0004_ask.md) decision 4).
...
refused for [ADR-EXPAND](0054_expand.md) decision 11's reason: it would make
`support` describe a retrieval depth the caller never asked for, where
[ADR-CONFIDENCE](0045_confidence.md) bounds it by `--top`. One consistent
...
phrase"* on the strength of not having looked — [ADR-RERANK](0041_rerank.md)
...
[ADR-CONFIDENCE](0045_confidence.md) · [ADR-EXPAND](0054_expand.md)
