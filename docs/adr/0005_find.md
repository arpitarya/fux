...
titles, no headings, no headers. That is the pipeable contract, and it is why
[ADR-ASK](0004_ask.md) decision 8's `§` lines are on `ask` alone: a heading in
this stream would be read as a filename.
...
**4. `--top`, `--fast`, `--scan` and `--no-tune` behave exactly as on `ask`** —
scan by default, `--fast` opts into the accelerator when it exists and is fresh
([ADR-ASK](0004_ask.md) decision 4).

**5. `find` has no `--explain`.** The path that answered is not part of the
pipeable contract, and a diagnostic line in a stream of paths is a defect
rather than a courtesy.

**6. No confident match prints `No confident matches.` and exits 0** — the same
rule as `ask`. Note the consequence below: it is prose on stdout in a stream of
paths.

### What it looks like

Verbatim from
[the capture](../../work/regression/2026-08-18-query-verbs/report.md).
...
**`find` gained three precision controls on 2026-09-05 (W-111), and none of
them is a second retrieval strategy.**
...
**Each one REMOVES results the ranking already produced.** A document the
ranking did not place in `--top` cannot be filtered *into* the answer, and
raising `--top` is what widens the pool. That is stated rather than worked
around: retrieving deeper and truncating after would make `support` in the
confidence block describe a depth the caller never asked for, which
[ADR-CONFIDENCE](0045_confidence.md) forbids and
[ADR-EXPAND](0054_expand.md) decision 11 already refused for `-q`.

⚠ **`band` is computed on the UNFILTERED ranking**, and a stderr line says so
whenever a filter removed anything. Confidence is a claim about the corpus's
answer to the question, not about the subset a caller asked to see; dropping
rows cannot make fux more or less sure of what it found.

⚠ **A `url:` document is KEPT by `--phrase`, never dropped.** Offline it has no
text to test, and dropping it would report *"this page does not contain the
phrase"* on the strength of not having looked. That is
[ADR-RERANK](0041_rerank.md) decision 8's rule and the four-state freshness
vocabulary's, applied to a filter.

⚠ **`--phrase` uses the INDEX's analyzer**, so stopwords are dropped: `--phrase
"roll back"` matches *"roll the back"*. A phrase filter with its own notion of
a token would be a second analyzer disagreeing with the first.

**Still refused: fuzzy and prefix matching.** The committed plane stores term
...
- **Sending `No confident matches.` to stderr** so stdout stays pure paths.
  ...
  the pipeable contract exact. Not taken because it would make the three verbs
  behave differently from each other for the same condition; the veto below is
  what watches for that trade going the other way.
...
# 2. text mode is still bare locations — no scores, no headings, no headers
