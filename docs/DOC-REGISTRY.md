...
| [`handoff/`](handoff/) | A feature enters build (handoff + prompt pair) | 2026-08-12 | M0/M1 pair (**executed** — ADR-0004) + M1-rerun pair (executed) + two CLAUDE.md diffs (proposed) + **`v0.31.0-claude-md-layout.diff` (proposed, not applied — the `.fux/` line for the Layout section)**. The v0.31.0 `.fux`-layout pair was **executed and moved to `archive/`**. **+the v0.31.0 fux-playground pair, executed** (ADR-0012). **+the v0.32.0 open-items pair (proposed, live)** — the program for every remaining open item: Phase 0 backlog clearing, Phase 1 M2, later milestones gated on their own pairs. **Archive-law debt outstanding:** the executed M1-T0-slice and fux-playground pairs are still in this directory and belong in `../archive/` ([W-43](open/W-43-archive-law-debt.md)). |
