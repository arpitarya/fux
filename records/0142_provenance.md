...
kind: component
name: SR-PROVENANCE
title: "SR-PROVENANCE (0142) — fux does not keep an audit trail; it makes one derivable"
...
[SR-CONFIDENCE](0141_confidence.md), **how much it believes it**. It has never
...
| [SR-CONFIDENCE](0141_confidence.md) | how much of the query the corpus covers | which term, which field, which weight |
...
**4. The four gates are [SR-WORK-QUALITY](0056_WORK-quality.md)'s funnel, not a
...
[SR-RUNTIME-STAMP](0124_runtime-stamp.md) decision 2 excludes the stamp from
...
✅ **RULED 2026-09-13 by Arpit: there are TWO consent surfaces and both are
explicit.** *"I need the flag as well as output TOML configuration."*

This decision said *"only `--journal` WRITES"* and that stopped being true when
`.fux/output.toml` shipped: `[cli.answer] journal = true` sets the same
`args.journal` the flag does. **The capability stays; what was missing was the
ruling, and it is made.**

- **`fux answer --journal`** — per-invocation consent, typed by the person
  running the command.
- **`[cli.answer] journal = true`** — repository-level consent, committed. A
  line in a committed file is *more* reviewable than a flag somebody types: it
  is in git, it goes through code review, and it is readable without watching
  anybody's terminal.
- **The default is `false` on both surfaces** and `output_config.BUILT_IN`
  agrees, so nothing records by accident. Consent is still explicit — **the
  fork this decision reserved was *always-on by default*, and that is still
  refused.** An opt-in someone committed is not a default fux picked.
- ⚠ **The asymmetry is real and is handled by disclosure, not by refusal.**
  `journal` is the one key in `.fux/output.toml` that writes to disk; every
  other key there changes how an answer is *displayed*. So the key is named as
  such in [SR-OUTPUT](0143_output-defaults.md) decision 22 and on the generated
  specimen's own line — a reader of a rendering config is told, in the file,
  which key is not about rendering.
- **`.fux/tune.toml` is REFUSED as the home** — the option this decision
  originally reserved. `tune.toml` is the *ranking* file; journalling is not
  ranking, and putting it there would trade one mislabelled home for another.
- ✅ **BUILT 2026-09-13 (W-147).** Both surfaces are bound by
  `tests_e2e/test_verbs.py::test_both_journal_consent_surfaces_write_and_neither_alone_is_removable`,
  which asserts all four states — **the flag alone writes, the committed key
  alone writes, neither writes nothing, and `--no-output-config` reaches the
  key** — plus a second test that `[cli] journal` is still refused by name at
  the shared level, because it is **per-verb** and a shared opt-in would be a
  much larger consent than the one ruled. The specimen line now opens *"⚠ NOT a
  rendering key"* and names the file it appends to.
  ⚠ **One property the test had to learn, and it is worth knowing:**
  `.fux/output.toml` is a **complete declaration** — once it exists every gated
  key must be set, and a two-line snippet is refused. So the committed surface
  a consumer actually has is the specimen with one line flipped, and that is
  what is tested.
  ⚠ **A stale sentence was corrected on contact:** `query/provenance.py`'s
  module docstring said the opt-in was `[provenance] journal = true` in
  **`.fux/tune.toml`** — the very home this decision REFUSES. It said so from
...
- 🔴 **The algorithm key said `sha256` from 2026-08-27 to 2026-09-12, and the
  ...
  characters, chosen in M1 and recorded in [SR-RECORD](0109_index-record.md).
...
- **The funnel** — [SR-WORK-QUALITY](0056_WORK-quality.md) decisions on the
  four-gate attribution.
- **The record this one made writable** — [SR-LAWS](0001_LAWS.md) decision 8.
...
6. **The journal is written without `--journal` or a `[answer] journal` key.** Any always-on path is an
   SR-TUNE decision this record does not make.
...
[SR-RANKING](0111_ranking.md) · [SR-RUNTIME-STAMP](0124_runtime-stamp.md) ·
[SR-REFER](0127_refer-plane.md) · [SR-TUNE](0135_tuning.md) ·
[SR-WORK-QUALITY](0056_WORK-quality.md) ·
[SR-CONFIDENCE](0141_confidence.md) · [the register](README.md)
