...
content_sha: 23cb8862d8b51fe6a8f092154a8ab615f9501e039ce1574895322db89047171f
...
| I am authoring set A — may I write its answers? | **yes, in the chat, once** — that is the carve-out, and that session then never runs a rung |
| may I re-read set A's answers later, since Claude wrote them? | **no.** Authorship buys nothing; from the handoff on, set A is as closed as set B |
...
flowchart TD
    ...
    SA["set A — questions + answers<br/>authored by Claude from seed/"]
    SB["set B — questions + answers<br/>authored by Codex"]
...
```text
   ...
   set A (Claude authors, from seed/) --.
...
authored by Claude from `work/golden/seed/`; **set B** is authored by Codex.
...
**6. The authoring carve-out, and its exact width.** Writing set A means writing
its answers; a law that forbade that would forbid the thing Arpit asked for. So:
**one designated session** authors set A from `seed/`, hands the questions and
...
access:** *"Claude wrote it"* is not a reason to read it back, and set A's
...
author of set A and the runner of set A are the same model family, so no
separation of sessions makes a set A run `blind` under
[SR-RS](0133_predictions.md). **Set A is bought for question-authorship
comparison against set B, not for a clean delta**, and any document that states a
set A number states that label beside it.
...
reading; a ruling that set A runs may claim `blind` would amend this decision and
nothing else.
...
**11. The law does not depend on the key being good.** The set B key in use
...
| option | why not |
|---|---|
| Keep the file, keep it Codex-readable, and only add set A | the state this amendment ends. Arpit's ruling is explicit that **no agent** reaches an answer, and a second key file authored by the party that must not read it is the worst artifact to leave on disk |
...
| Forbid Claude from authoring set A at all | it forbids what Arpit asked for. The containable part is *access after authorship*, and decision 6 contains exactly that |
| Let the authoring session also run set A, since it already knows | it converts a one-turn exposure into a standing one, and the run is the artifact that gets cited. The separation costs one session and buys the only honesty available here |
...
- [SR-RS](0133_predictions.md) — `blind` versus `informed`, which is the currency
  a leak spends, and the label decision 7 pins to every set A number.
