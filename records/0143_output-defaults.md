...
kind: component
name: SR-OUTPUT
title: SR-OUTPUT (0143) — output defaults are configurable, in a third file
...
[SR-CONFIDENCE](0141_confidence.md) decision 11, `--band` off. A consumer who
...
- **Every output default is a single baked constant** — `--top 5` in
  `cli.py`, `hops 2`, `json` off, and `band` off since SR-CONFIDENCE
...
- ⚠ **SR-CONFIDENCE decision 11 accepted a cost it could only mitigate with
...
- Fux already has two config files with two different boundaries
  ([SR-CONFIG](0113_config.md), [SR-TUNE](0135_tuning.md)), so the question
...
3. ⚠ **AMENDED 2026-08-27 — THREE CATEGORIES, BECAUSE THERE ARE THREE
   ...
   - **A file in the old flat layout is NAMED, not shrugged at.** It parses
     ...
     SR-TUNE's `_LEGACY_FIELD_KEYS` precedent, same reasoning.
...
5. **The key set is closed, per verb, and a typo is loud.** Same reasoning as
   SR-TUNE decision 5: this is a file that changes the shape of every
...
11. ⚠ **`[mcp]` is the table this record exists for.** It carries **`top`
    ...
    **The first draft of this decision carried `band` in `[mcp]` and it was
    wrong.** [SR-CONFIDENCE](0141_confidence.md) decision 11 makes the MCP
...
19. ⚠ **THE FILE IS THE SOLE SOURCE OF TRUTH. Ruled by Arpit, in Cowork,
    ...
    **Why an error over the alternative this record's own §2 Context once
    named** (a silent `fux doctor` warning, SR-DOTFUX decision 6's usual
...
20a. ⚠ **Decision 20's lesson was applied by a DIFFERENT record on
    ...
    [SR-PII](0148_pii.md) decision 17 makes a missing `.fux/pii.toml` a hard
    ...
    **Why this belongs in THIS record and not only in SR-PII.** Decision 20 is
...
([SR-ACQUIRED](0145_acquired-plane.md), [SR-URL-FRESHNESS](0147_url-freshness.md)).
...
([SR-URL-LIST](0116_url-list.md) decision 14), gates no rendering, and reads
...
([SR-CLI](0101_cli-surface.md) decision 2a, [SR-ENRICH](0137_enrich.md)
...
`doc_coverage_floor` before the block is built ([SR-CONFIDENCE](0141_confidence.md)
...
[SR-EXPAND](0149_expand.md) decision 9), present only when more than one `-q`
...
- **It exists because this file reached a verb's BEHAVIOUR once.** `fux hooks`
  ...
  ([SR-MAINTENANCE](0129_hooks.md)). This record's own claim — that a
...
22. ⚠ **`journal` is the one key in this file that WRITES TO DISK, and it is
    RATIFIED — it is not a rendering default.** Every other key here changes how
    an answer is *rendered*; `[cli.answer] journal = true` changes whether a
    durable local record of the question survives the process — the same
    `args.journal` `fux answer --journal` sets.

    - ✅ **Ruled 2026-09-13 by Arpit:** *"I need the flag as well as output TOML
      configuration."* **Two consent surfaces, both explicit** — the flag for a
      single invocation, the committed key for a repository. A committed line is
    ...
    - 🔴 **This key is therefore a DECLARED EXCEPTION to decision 2's boundary
      rule, not an instance of it.** Decision 2 admits a key iff changing it
      leaves the ranked result set and its order identical — `journal` satisfies
      that and still writes bytes, which is exactly why the rule alone was never
      going to catch it. The exception is named here so the next key that writes
      has to argue for itself rather than inherit this one's pass.
    - ✅ **Disclosure is the mitigation, and it is BUILT** (2026-09-13, W-147).
      The specimen line now opens *"⚠ NOT a rendering key"* and names the file
      it appends to, instead of *"record each answer's receipt locally"* —
      which named the **place** and not the **act**, to a reader scanning a
      rendering config for things that change what they see.
      `tests_e2e/test_verbs.py::test_both_journal_consent_surfaces_write_and_neither_alone_is_removable`
      binds all four states — flag alone, committed key alone, neither, and
      `--no-output-config` — and
      `tests/test_node_config_parity.py::test_the_journal_key_is_bound_on_BOTH_runtimes_by_name`
      names the key in both runtimes so a drop fails with what it broke rather
      than with *"`answer` key set differs"*.
      ⚠ **The committed surface is a COMPLETE declaration, which the test had to
...
- ⚠ **`output.schema.json#confidence` was already conditional on argv** after
  SR-CONFIDENCE decision 11. It is now additionally conditional on a committed
...
| **an `[output]` table in the repo-root `fux.toml`** | `fux.toml` is *what is indexed*. Mixing rendering into it breaks SR-CONFIG's boundary and grows the file with the most schema surface already. |
| **flags only, no file** | leaves MCP unconfigurable in principle, and leaves SR-CONFIDENCE decision 11's cost mitigated by documentation alone. |
...
- ⚠ **Measured, 2026-08-28, against a local mirror — the CLI seam's own
  ...
  (`test_sr_ownership.py`, `test_doc_links.py`, `test_doc_registry.py`,
