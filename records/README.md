...
**`0013`–`0050` and `0068`–`0100` are reserved and EMPTY — there are no
...
| [0068](0068_WORK-test-data.md) | **SR-WORK-TESTDATA** | The test-data checklist — **fourteen items, T1…T14**, that every seed document, question set and rung is authored against. Each points to the record that owns its rule and restates nothing; the three with no other home are stated here — **anchor-only vocabulary**, **abbreviation and glossary pairs**, and **a corpus with git history**. Every prompt that creates test data links it and names the items it carries, and a new failure class becomes a new item in the change that finds it | accepted | yes |
...
| `tools/golden-score/` | SR-LAW-11 | 🔴 **the ONE program L11 decision 13 permits to read an answer key**, and the only code in this repository that may. Owned by the **law** rather than by SR-WORK-GOLDEN because the permission *is* the law — decision 13 names this path, and a program the record does not name has none. The precedent is SR-LAW-0 owning its generator/bind pairs. **Its output allow-list is the carve-out's containment**, so the freshness gate must drag the law forward whenever this file moves |
| `tools/golden-switch/` | SR-LAW-11 | 🔴 **the switch L11 decision 14 names** — `unlock` / `lock` / `retire`, plus a `state` verb that is safe for anyone. It is the first program in this repository that can turn a **law's** enforcement off, so what it may touch is narrow and asserted: exactly one file is mutated (`.claude/settings.json`), the hook FILES are deregistered and never edited, and `tests/test_golden_switch.py` fails if the code so much as reaches `.gitignore` or `.fux/sources/dirs` — *never committed* is the clause no state relaxes. ⚠ **Its environment check is a tripwire, not the rule**: `just golden-unlock` names none of the strings the Bash deny patterns match, so **no guard stops an agent running it** and the law's sentence is the whole defence |
| `tests/test_golden_score_output.py` | SR-LAW-11 | the allow-list, pinned against a **synthetic** key — it needs no real key and touches none. Owned by the record whose rule it enforces, per §The three kinds. It exists because `answerable_key`, one of the four things L11 calls an answer, sat in every scored row until writing this test found it |
...
| `tests/test_golden_hook_prose.py` | SR-WORK-GOLDEN | the gate on the **prose** question ruled by Arpit on 2026-09-21 (W-209), the fourth recorded occurrence of the class: a shell command that merely *writes a sentence* naming a key path is refused, while the same sentence in a Write/Edit `content` field is not. It pins **both** directions against the committed guards, so neither a "usability" narrowing nor a widening that would make the law undocumentable can land silently. The convention itself is stated once in [SR-WORK-GOLDEN](0066_WORK-golden.md) decision 7. It imports the Windows `bash` probe from `test_golden_key_guards.py` rather than copying it — two copies of that workaround could disagree while both looking correct |
| `tests/test_settings_never_committed_unlocked.py` | SR-WORK-GOLDEN | the gate on the one file `just golden-unlock` mutates: **HEAD's `.claude/settings.json` is always the LOCKED one**. An unlock leaves the repository one careless commit from putting *"the guards are down"* into history and every clone — the same never-fails-loudly shape L11 is written around. **Two strikes → a gate**: the class *a commit took more than the author staged* happened three times on 2026-09-22 (`a0140874`, `f425a79a`, `163313a1`). ⚠ The general case has no mechanical check, because nothing can know which staged paths the author meant; **this one does**. It reads `git show HEAD:`, never the working tree, which is *expected* to be unlocked while somebody scores |
