...
[SR-WORK-QUALITY](0056_WORK-quality.md) decision 6 has **already fixed what it
...
- `answer` states its ceiling in prose (SR-ANSWER), which an agent parsing
  `--json` never sees.
...
**[SR-WORK-QUALITY](0056_WORK-quality.md) named the same failure from the
...
5. **`answerable` is a refusal, not the bottom of a scale.** An agent handed
   ...
   [SR-WORK-QUALITY](0056_WORK-quality.md) decision 5 supplies the measured
...
11. ⚠ **`--band` gates the CLI; MCP is always on. RULED by Arpit, 2026-08-27,
    ...
    - ✅ **The documentation-only mitigation is retired.**
      [SR-OUTPUT](0143_output-defaults.md) lets a repo commit `band = true` in
...
- **Two things replace the prohibition, and both are weaker than it was.**
  ...
  2. **`--no-tune` reaches the band**, not just the ranking. The floors are
     ...
     the config?"* switch (SR-TUNE decision 11) answers for confidence too.

- **The boundary rule is satisfied trivially, and that is worth saying rather
  than assuming.** SR-TUNE decision 1 asks whether a value leaves `.fux/index/`
...
- ⚠ **This does NOT settle R10, and does not let a repo settle it either.**
  The measurement is still owed to SR-WORK-QUALITY's frozen `t`. A repo-local floor
...
- **The mechanism.** Every term occurs — `sla` and `publish` in the retention
  ...
  SR — in **four different documents**. Corpus-wide coverage cannot see that,
...
a corpus [SR-WORK-ENVIRONMENTS](0052_WORK-environments.md) retired six days later, so this is a
...
14. **`_as_dict` and `cmd_ask` in this file gained the `sections` gate — the
    decision is [SR-OUTPUT](0143_output-defaults.md) decision 21, noted here
...
[SR-EXPAND](0149_expand.md) decision 10.
...
[SR-WORK-OWNERSHIP](0054_WORK-ownership.md)'s new symbol qualifier exists for, and it is
...
exactly the break [SR-T1-ACCELERATOR](0110_accelerator.md) forbids. Counting
...
**What we now owe: R10**, and it is owed to SR-WORK-QUALITY's frozen `t`, not to
...
- **Choosing this record's own abstention threshold.** Rejected on contact with
  [SR-WORK-QUALITY](0056_WORK-quality.md) decision 6, which had frozen `t` the
...
- **Calibrating the cutoff against the 50-query golden set now.** Fastest route
  ...
  [SR-RS](0133_predictions.md)'s run-classification rule: choosing the number
...
- [SR-WORK-QUALITY](0056_WORK-quality.md) decisions 5 and 6 — the frozen
...
[SR-ANSWER](0105_answer.md) · [SR-T1-ACCELERATOR](0110_accelerator.md) ·
[SR-REFER](0127_refer-plane.md) · [SR-RS](0133_predictions.md) ·
[SR-TUNE](0135_tuning.md) · [SR-MCP](0136_mcp.md) ·
[SR-RERANK](0138_rerank.md) · [SR-WORK-QUALITY](0056_WORK-quality.md)
