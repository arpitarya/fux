...
kind: process
name: SR-WORK-QUALITY
title: "SR-WORK-QUALITY (0056) — what \"good\" means: the four-gate funnel, the declared mix, and the published cost of an error"
...
> ⚠ **This was `SR-QUALITY` at `0141` until 2026-09-13** (Arpit). It is a WORK
> record now — what a quality number MEANS is how work is measured, not what
> the engine guarantees — and **the handle `SR-QUALITY` is retired and never
> reused.** Every live citation reads `SR-WORK-QUALITY`; the old handle
> survives only in `work/WORKLOG.md`, `CHANGELOG.md` and `archive/`, which are
> history and are not rewritten.

**Fux has always measured carefully and has never said what it was measuring.**
[SR-RS](0133_predictions.md) governs *how* a claim is frozen and is silent on
...
2a. **A BAR ON `recall@k` NAMES ITS `k`, OR IT IS VOID.** `recall@k` is not one
   ...
   [SR-RS](0133_predictions.md)'s moving threshold wearing an omission instead
   of an edit.
...
3. **`nDCG` and `MRR` are diagnostics, not the headline**, and the demotion is
   ...
   **reranker follows retrieval** ([SR-RERANK](0138_rerank.md)), so the
...
9. **The `answered` gate is measured as a separate `judged` series.** L3 is
   ...
   **never fuse** a judged number into the headline; **never compare** across
   ...
   adjudicate one.** It inherits SR-RS's blind/informed label and adds a third
...
12. **The RANK CONTRACT and the RELEVANCE SET are two fields, not one.**
    ...
    **Four rules, and each closes a specific way this can go wrong:**
    ...
    the system it judges ([SR-RS](0133_predictions.md) decision 13). The
...
[SR-WORK-ENVIRONMENTS](0052_WORK-environments.md) retired that corpus on 2026-09-11. The output
...
50 goldens, all valid against SR-WORK-QUALITY decision 12
...
[SR-RS](0133_predictions.md) decision 19 — never a difference of aggregates.
...
- ⚠ **`recall@k` is not computed today, and the reason CHANGED on 2026-08-28.**
  ...
  [SR-WORK-ENVIRONMENTS](0052_WORK-environments.md) closed to every agent on 2026-09-11 — so the
...
- [SR-RS](0133_predictions.md) — the freezing discipline this record supplies a
  quantity for
...
**Records** — [SR-RS](0133_predictions.md) · [SR-RERANK](0138_rerank.md)
