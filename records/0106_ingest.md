...
owns: [src/fux/ingest@48251900db7e, src/fux/ingest/priors.py@8ffcc632a4be]
laws: [L2, L3, L4]
timestamp: 2026-08-20T00:00:00Z
content_sha: 67a306f2a57cc10b1e5ae4b6d40c69ab2f776c7bd6613a356f56a91849baebff
---
...
[SR-FUXIGNORE](0144_fuxignore.md) decision 11, in two delimited blocks,
...
| a **URL** is never recorded, and prints on every networked run | `.fuxignore` matches repo-relative paths and a URL has none (SR-FUXIGNORE decision 11c). Accepted: a repo has a handful of dead URLs, not hundreds, and repeat failure is already `.fux/runtime/url-state.json`'s job |
...
there. That cost is SR-FUXIGNORE decision 11's to carry; what belongs here is
...
([SR-DOTFUX](0102_fux-directory.md)), so a fresh clone is correctly laid out
before the first byte lands.

**8. Ingest is offline.** The exceptions are the named fenced paths
([SR-URL-INGEST](0107_url-ingest.md)); a plain run never imports the fetcher.
...
⚠ **`DEFAULT_TYPES` is unchanged by this.** [SR-TYPES](0128_types-list.md)
verdict G was measured, and a measurement is replaced only by a better
measurement. A consumer opts a decodable type in through the types list (`.fux/formats.toml`),
...
source** ([SR-ARCHIVED-CONTENT](0134_archived-content.md) decision 1). It reads
...
[SR-ARCHIVED-CONTENT](0134_archived-content.md) decision 1a). Three things
...
| "run the rules over the whole file" | the frontmatter is deliberately outside the pass. It is stripped before indexing already ([SR-ENRICH](0137_enrich.md) decision 8), so nothing in it reaches a committed term, and matching a `model:` value would refuse a file for text the index never sees |
...
body holds ([SR-TUNE](0135_tuning.md) decision 13). Neither moves a document's
...
([SR-ACQUIRED](0145_acquired-plane.md) decision 8, [SR-CONFIG](0113_config.md)
...
Ruled by Arpit 2026-08-28; SR-FETCHER decision 11 had named the asymmetry.
...
- ⚠ **A FETCH FAILURE IS NOT QUEUED**, and that distinction earns a third skip
  ...
  real 404 and a real 429 stay out, and `json: nothing readable in .json` goes
  in.
...
retires nothing. [SR-ENRICH](0137_enrich.md) decisions 17-18.

**A pinned URL is a `POLICY` skip, taken before the fetch and carried forward
after it.** `update=never` ([SR-URL-LIST](0116_url-list.md) decision 14) filters
...
- ⚠ **A carried `url:` record is not re-extracted, so a pinned document's
  extraction is frozen too** — including its redaction. [SR-PII](0148_pii.md)
...
[SR-URL-LIST](0116_url-list.md) decision 14 always promised it. The walk is
...
- **`fux ingest --stop` and the runner takeover change nothing about what a run
  ...
  **never by reading the dirty list** — the list is advisory, and a run that
  ...
  [SR-MAINTENANCE](0129_hooks.md) 1d and the surface is
...
- **`run()` takes an optional `progress`, `None` meaning silent.** Four phases
  ...
  to [SR-CLI](0101_cli-surface.md); what binds here is that the phases report
...
- ⚠ **`fux ingest` now writes a committed file that is also its own input.**
  ...
  [SR-FUXIGNORE](0144_fuxignore.md) decision 11.
...
- **The graph plane can no longer be handed a dangling edge by ingest**
  (decision 10). [SR-GRAPH](0126_graph.md)'s `edges_from_records` lifts edges
...
- **Keep the already-reported set in `.fux/runtime/skipped`.** Rejected on
  ...
  [SR-FUXIGNORE](0144_fuxignore.md) decision 11 and made loud by 11a.
...
[SR-PII](0148_pii.md) decision 15, which owns redaction. **Two properties this
...
[SR-DOTFUX](0102_fux-directory.md) · [SR-ASK](0103_ask.md) ·
[SR-URL-INGEST](0107_url-ingest.md) ·
[SR-INDEX-LIFECYCLE](0108_index-lifecycle.md) ·
[SR-RECORD](0109_index-record.md) · [SR-EXTRACTED](0115_extracted-mode.md) ·
[SR-GRAPH](0126_graph.md) · [SR-TYPES](0128_types-list.md) ·
