...
kind: component
name: SR-TUNE
title: "SR-TUNE (0135) — the tunables file, and per-source priority"
...
[SR-CONFIDENCE](0141_confidence.md) decision 13 puts `separation_floor` and
...
- ⚠ **The knob is real and is not clamped.** `separation_floor = 0.0` means no
  ...
  in capitals.
- **The safeguard lives in SR-CONFIDENCE, not here:** the block **publishes**
...
[confidence]               # the BAND — decision 5d, SR-CONFIDENCE decision 13
...
|---|---|---|
| **changes the committed index** | the phrase cap · the edge grades `10`/`8`/`6` · the token cap · the fixed shard count | format decisions; moving one costs a re-ingest and a `_format` bump. The type allowlist already has a home — [SR-TYPES](0128_types-list.md) |
| **derived, and a speed knob not a ranking one** | the block size · the community sweep cap | changes `.fux/runtime/` only; **the differential law means results cannot move, only latency**. Belongs to [SR-T1-ACCELERATOR](0110_accelerator.md) |
| **operational, not retrieval** | runner timeouts · the progress threshold · the fetch cache's TTL and byte cap | the cache pair is the arguable one — it is resource policy, [SR-CACHE](0131_cache.md) owns it, and exposing it belongs in `fux.toml` beside the fetcher config |
...
worse than an error ([SR-CONFIG](0113_config.md) decision 10).
...
| `w == 0` | error, naming the `!` entry | that is **exclusion**, and [SR-DIR-LIST](0120_dir-list.md) decision 2a already owns it. **Two ways to do one thing is the rot** |
...
Node reader read no `.fux/tune.toml` at all ([SR-NODE-SEARCH](0153_node-search.md)
...
consequences are [SR-T1-ACCELERATOR](0110_accelerator.md) §The weighted bound.
...
measurement, [SR-CONFIDENCE](0141_confidence.md) decision 12's outcome.
...
[SR-EXPAND](0149_expand.md) decision 5), default **`0.2`** — Query2doc's 1:5
...
2026-09-05** (W-111, [SR-RANKING](0111_ranking.md)) — and **the tie-break
...
- **What moved.** `max_table_rows` left `fux.toml [decode]`, where
  [SR-TABULAR](0150_tabular.md) put it on 2026-09-06. `max_phrases` is new: it
  was a hard-coded `12` in `ingest/extract.py` until this ruling, and its default
  is now **32** ([SR-EXTRACTED](0115_extracted-mode.md)). `fux.toml [decode]`
  is refused by name, naming the new home ([SR-CONFIG](0113_config.md)).
...
- 🔴 **A sweep over any of the three meets a `0 broken` bar VACUOUSLY**, at every
  ...
  Caught by [SR-RS](0133_predictions.md) decision 22d — the `heading` control's
...
[SR-LAW-0](0002_LAW-0-authority.md) decision 6.
[`tests/test_sr_config_keys.py`](../tests/test_sr_config_keys.py) asserts this
block equals `tune.py`'s `_SCHEMA` **in both directions**, and that
`tune.specimen()` — what `fux setup` writes — names no key outside it.

**Sigils as in [SR-CONFIG](0113_config.md) decision 13:** `+` a key the loader
...
only on 2026-09-12 (SR-CONFIG decision 14), modelled on this file.
...
**15. ALL THREE DOCUMENT PRIORS ARE REMOVED, and a committed one is refused BY
NAME** (Arpit, 2026-09-13). `superseded_weight` first — *"there is no weight
that determines which document supersedes another in the real world, so let's
remove it"* (W-151) — then `archived_weight` and `recency_half_life_days` on the
same evidence (W-152). **`[ranking]` is `rerank_weight` and `expand_weight`,
and neither is a document prior.**

- **The evidence is decision 13's last block, read one knob at a time.**
  [VERDICT-W143](../work/regression/2026-09-12-priors-and-tables/VERDICT-W143.md)
  answered the pre-registered question with **NO** across 26 intent-split
  probes: no single global value clears `0 broken`. For this key there is a
  further, structural reason — the band of values that orders a corpus sensibly
  has its lower edge set by **an unrelated document's score**, so adding a
...
- **The FACT survives untouched** — `supersedes:` frontmatter, the `superseded`
  ...
  document above a retired one at an equal score. **The fact is not the weight**,
  and supersession now reaches ranking through the tie-break and nowhere else.
- 🔴 **A committed `superseded_weight` is an error that NAMES the removal**, via
  `tune._REMOVED_KEYS` and its Node twin — never a bare *"unknown key"*. `fux
  setup` wrote this key into `.fux/tune.toml` for every repo that ran it, so a
  generic error would send somebody hunting for a typo in a line they never
  typed. **A key leaves the schema and arrives in that table in the same
  change**, or the removal is a trap.
- ⚠ **What this does NOT decide:** whether anyone will ever write `supersedes:`.
  It has never appeared in a real corpus, so the `superseded` property may be
  `False` everywhere — which is a question about the *concept*, not the knob,
  and is Arpit's.

**15a. `archived_weight` — being retired is a fact, not a weight.** Its best
candidate (`0.75`) clears on `rung-01000` and **breaks `p20` on both other
rungs**, for a net of **+2** against decision 19's floor of **6**. ⚠ **It was
never in W-97's sweep at all** (W-73's law), so no selection evidence for it
exists outside VERDICT-W143.

- **Nothing is lost to a caller**, which is what separates it from recency: the
  fact was already public and stays public — `archived=true` in
  `.fux/sources/dirs`, the `archived` record property, `ARCHIVED_MARKER` and the
  response-level note in prose, and `archived: bool` on every JSON hit,
  `required: always`, with the output schema's own instruction: **"BRANCH ON
  THIS, never on the prose note."** An agent can filter; a human can see.
- 🔴 **The response note loses its `(demoted, weight N)` clause** in both
  readers — there is no demotion left to disclose. What
  [SR-ARCHIVED-CONTENT](0134_archived-content.md) decision 2's veto asserted is
  now **structural** rather than a property of a shipped default.

**15b. `recency_half_life_days` — the asymmetric one, and the sharpest result.**
**No candidate value exists at all.** At a half-life of a year or less,
history-seeking goes to **0 of 13**. The failure is structural: a per-document
decay is being asked to carry a per-query distinction, and *"what do we do now?"*
and *"what did we do before?"* want opposite orderings out of one corpus.
...
- **Two derived things go with it**, because their only reader was this knob:
  ...
  `stats.json`'s `newest_mtime` ([SR-RUNTIME-STATS](0125_runtime-stats.md)
  decisions 1 and 3). The bound argument that field bought is **kept in that
  record**, because it binds any future prior that reads a date.
- **`mtime` itself is untouched** — still committed by ingest, still the second
  key of [SR-RANKING](0111_ranking.md)'s declared tie-break, and now on the
  `ask` hit.

**15c. What the three removals share, and it is the reason none of them is a
ranking change.** Every one shipped as a no-op — `1.0`, `1.0`, `0.0` — so
`Weighting.of()` returns the same float before and after. **Nothing ships**,
which is also the answer to *never ship a ranking change off one synthetic
corpus*. What goes is three global multipliers no single value can set
correctly; `Weighting` is left with `[priority]`, and
...
- ⚠ **The accelerator gets slower in proportion to the spread.** Decision 12's
  ...
  [SR-RS](0133_predictions.md) with a frozen pre-registration.
...
[SR-CONFIG](0113_config.md) · [SR-URL-LIST](0116_url-list.md) ·
[SR-DIR-LIST](0120_dir-list.md) ·
[SR-RUNTIME-STATS](0125_runtime-stats.md) · [SR-GRAPH](0126_graph.md) ·
[SR-REFER](0127_refer-plane.md) · [SR-TYPES](0128_types-list.md) ·
[SR-CACHE](0131_cache.md) · [SR-RS](0133_predictions.md) ·
