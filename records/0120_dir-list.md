...
> a disclaimer — is [SR-ARCHIVED-CONTENT](0134_archived-content.md).
...
Same reasons as the URL list ([SR-URL-LIST](0116_url-list.md)): one entry per
...
flowchart LR
    ...
    R --> X["SR-ARCHIVED-CONTENT<br/>ranking · marker · disclaimer"]
```
...
on 2026-09-11 — [SR-TYPES](0128_types-list.md) decision 12). `[sources] dirs` in `fux.toml` is a
**retired key that errors with instructions**
([SR-CONFIG](0113_config.md) decision 10).

**2. The grammar is [SR-URL-LIST](0116_url-list.md)'s**, by reference and not
...
2026-09-11, when the types list left for TOML (SR-TYPES decision 12).
...
deliberate (SR-FUXIGNORE decision 2) and the warning is the only place it is
caught.
...
[SR-ENRICH](0137_enrich.md) and is the one attribute whose effect on the index
is indirect — see [SR-URL-LIST](0116_url-list.md) §The `dirs` attribute set.
...
([SR-FUXIGNORE](0144_fuxignore.md) decision 4). A `!` line there is a
...
explicitly ([SR-URL-LIST](0116_url-list.md) decision 12). This file is
...
- **`fux add --types` seeds the built-in allowlist when it creates the file**
  ([SR-TYPES](0128_types-list.md)'s *absent means the default*), because the
...
- The grammar this record reuses — [SR-URL-LIST](0116_url-list.md), and its one
  implementation,
...
- The key it retires — [SR-CONFIG](0113_config.md) decision 10.
- The layout it extends — [SR-DOTFUX](0102_fux-directory.md) decision 2.
...
# expect: sourcelist.py, plus typesfile.py (.fux/formats.toml, SR-TYPES decision 12)
#         and fuxignore.py (.fux/.fuxignore, SR-FUXIGNORE)
