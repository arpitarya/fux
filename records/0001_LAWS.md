...
owns: [src/fux/__init__.py@7c7a36e58f81, src/fux/errors.py@149588d64df5, src/fux/schema.py@eb12ad1f4562, src/fux/frontmatter.py@24eaf48601ee]
laws: [L1, L2, L3, L4, L6, L7, L8, L10, L11]
timestamp: 2026-08-18T00:00:00Z
content_sha: 59e372dd2ebe2cf8b00d5c924726296e588bf98f71d4fc0f1607592180f32716
---
...
- [`src/fux/__init__.py`](../src/fux/__init__.py) · file
- [`src/fux/errors.py`](../src/fux/errors.py) · file
- [`src/fux/frontmatter.py`](../src/fux/frontmatter.py) · file
- [`src/fux/schema.py`](../src/fux/schema.py) · file

**Describes** — reaches into, does not own:

- [`node/src/errors.mjs`](../node/src/errors.mjs) · owned by [SR-NODE-SEARCH](0153_node-search.md)
...
Fux has **ten** rules no feature may break. **Each one is stated in its own
record** — `SR-LAW-0` … `SR-LAW-11`, the table below — and
...
| ~~**L5**~~ | **RETIRED 2026-09-20** — hashed display meta was deleted outright (Arpit, W-194), and there is no residue of the law once the mechanism is gone. **The ACL-mismatch leak it closed is an ACCEPTED, DOCUMENTED EXPOSURE**, not a solved problem. **The handle is never reused.** | [SR-LAW-5](0007_LAW-5-hashed-meta.md) — kept at `status: superseded` for the argument and the reopen trigger |
...
**2. This record assigns the stable handles L0–L4, L6–L8, L10 and L11** (L0
since 2026-09-06, L10 since 2026-09-12, L11 since 2026-09-15) so a decision can
say which law binds it without quoting it.

⚠ **TWO handles are retired and neither is ever reused. Ten live laws, twelve
numbers.**

- **L9**, a handle from 2026-09-11 to 2026-09-13 — the environment rule is
  process, not a guarantee the engine makes, and it moved to
  [SR-WORK-ENVIRONMENTS](0052_WORK-environments.md).
- **L5**, a handle from 2026-08-18 to 2026-09-20 — hashed display meta was
  **deleted outright** (Arpit, W-194) and there is no residue of the law once
  the mechanism is gone. Its record stays at `status: superseded`, because the
  leak it closed is now an **accepted exposure** and a reopen is cheaper than a
  rediscovery.

**Both gaps are deliberate**: renumbering would silently change the meaning of
every citation already written, which is the one failure a stable handle exists
to prevent.
...
positioning audit
([`work/proposals/positioning-documents-not-code.md`](../archive/proposals/positioning-documents-not-code.md) §6),
...
# expect exactly the twelve records/*_LAW-*.md records -- ten live and
# SR-LAW-5, retired 2026-09-20, which KEEPS its block as history and is skipped
# by the generator on `status: superseded`. A hit anywhere else is a
