...
kind: component
name: SR-REFUSAL
title: "SR-REFUSAL (0146) — the response the server sent instead of the document"
...
1. **Refusal is checked after `_unpack` and before persist and decode.** A
   ...
   ([SR-ACQUIRED](0145_acquired-plane.md) decision 6).
...
4. **Every condition is pure over the bytes.** The six are `content_type`,
   ...
   ⚠ **This is [SR-FETCHER](0117_fetcher.md) decision 13 holding, and it was
...
([SR-URL-LIST](0116_url-list.md) decision 14) is filtered out **above**
...
- ⚠ **Write-if-missing, so no existing repo gets any of this.** A repo set up
  ...
  [SR-DOTFUX](0102_fux-directory.md) decision 6's standing problem, and
...
- **A `[refusals]` table in `fux.toml`.** Rejected for the reason
  SR-URL-LIST gives for the URL list: a growing list of multi-line entries in
...
[SR-URL-INGEST](0107_url-ingest.md) · [SR-URL-LIST](0116_url-list.md) ·
[SR-FETCHER](0117_fetcher.md) · [SR-DECODE](0139_decode.md) ·
[SR-ACQUIRED](0145_acquired-plane.md)
