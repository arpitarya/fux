...
kind: component
name: SR-ANSWER
title: SR-ANSWER (0105) — the answer verb
...
([SR-REFER](0127_refer-plane.md)), never a rewrite of them.

**4. One ANSWER is forced; top-1 is not.** There is no `--top`, and a caller
wanting alternatives still wants [`ask`](0103_ask.md) — but since W-108 the
...
[SR-REFER](0127_refer-plane.md) decision 17 carries both halves.
...
**It needs no fifth freshness label.** [SR-REFER](0127_refer-plane.md)
...
removed or repurposed ([SR-ASK](0103_ask.md) decision 11's rule).
...
[SR-EXPAND](0149_expand.md) decision 12 records the split, and `--expand` is
...
The declared tie-break ([SR-RANKING](0111_ranking.md)) runs in `rank()`, which
...
- **`--cache-ttl`** is the only way to ask for a cached copy. `cmd_answer` built
  ...
  [SR-URL-FRESHNESS](0147_url-freshness.md) carries the arithmetic and why the
  default stays `0`.
- 🔴 **`fux verify --rerun` called this verb's refer path, which fetches**,
  breaking [SR-PROVENANCE](0142_provenance.md) decision 14 — Arpit's ruling
...
- 🔴 **`--band` reports a real `separation` on this verb for the first time,
  ...
  ([SR-CONFIDENCE](0141_confidence.md) decision 14); what changed is that the
  number is now true.
...
- The ranking it projects — [SR-ASK](0103_ask.md).
...
- **The measured basis for decisions 11-13** — `answer` over the 43 graded
  ...
  ⚠ **That corpus was retired by [SR-WORK-ENVIRONMENTS](0052_WORK-environments.md) on
