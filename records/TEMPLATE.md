...
description: "Copy for each completed feature or ruled measurement. One feature, one SR. Real records use type: Standing Record, and a real kind."
status: proposed
date: 2026-08-27
...
content_sha: ""   # stamped by scripts/sr-hash.py; the template is not a record
---

# SR-<NAME>: <short decision title>

> **How to use this file.** Copy it to `records/NNNN_<short-name>.md` and give
...
> | `name` | `SR-<NAME>` — cite this everywhere, never the number |
...
> | `feature` | the one feature this record belongs to; one feature, one SR |
> | `owns` | inline list of the `src/`/`tools/` paths this record claims, each as `path@<12-hex content hash>`, `[]` when none. **Must match the ownership table in [`README.md`](README.md)** (paths only; the hash is not part of the identity). Stamped by [`scripts/sr-owns.py`](../scripts/sr-owns.py) **before** `sr-hash.py` |
> | `laws` | inline list of the SR-LAWS numbers this decision is bound by, `[]` when none. Never restate a law |
...
> [`tests/test_sr_frontmatter.py`](../tests/test_sr_frontmatter.py) checks
...
```mermaid
flowchart LR
    A[input] --> B[the thing this SR decides]
    B --> C[output]
```
...
```text
   +-------+      +-----------------------------+      +--------+
   | input | ---> | the thing this SR decides  | ---> | output |
   +-------+      +-----------------------------+      +--------+
```
...
**Records** — [SR-<NAME>](NNNN_<short-name>.md) · [SR-<NAME>](NNNN_<short-name>.md)
