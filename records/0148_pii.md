...
owns: [src/fux/ingest/pii.py@e43d90ebe560, src/fux/templates/pii.toml.txt@f6d9a00009b0, tools/pii-probe@f65b665b7472, .fux/pii.toml@f6d9a00009b0]
laws: [L2, L3, L5]
timestamp: 2026-09-01T00:00:00Z
content_sha: 553788c64289edc6688a4383be56ac1564db5d1c968194e65b083835b433b23d
---
...
- [`.fux/pii.toml`](../.fux/pii.toml) · file
...
[types-as-TOML comparison](../archive/compare/types-toml.compare.md) §1 (archived 2026-09-24; its argument is in SR-TYPES decision 12) argued
...
**2026-09-21 — the URL attribute set grew to seven, and redaction is untouched.**

`decoder=` joined the `urls` grammar ([SR-URL-LIST](0116_url-list.md) decision
17). This record owns `enrich` on that list and the three-layer resolution it
shares with `keep` and `ttl`; **`decoder` has no source-wide layer at all**, so
it adds no fourth case to that resolution.

**Redaction is unaffected, and worth checking rather than assuming:** the sha is
taken **before** redaction, and the declared decoder changes *which converter
produces the Markdown*, not *when the sha is taken* or *what `pii.toml` sees*.
A document whose line changes decoder produces different text and therefore a
different sha — which is a re-extraction, exactly as a decoder digest bump is,
and not a redaction question.

⚠ **Stated here because this record co-owns `sourcelist.py`, not because
anything it decides moved.**
