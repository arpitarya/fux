...
kind: component
name: SR-FUXIGNORE
title: "SR-FUXIGNORE (0144) — .fux/.fuxignore: one file for what is not indexed, in .gitignore's grammar"
...
[SR-TYPES](0128_types-list.md)'s measured answer to a real defect. What was
...
**subtracts** (SR-DIR-LIST decision 2b, chosen so there is no precedence order
...
⚠ **The second half overrides [SR-TYPES](0128_types-list.md), and that record
is amended rather than left to contradict this one.** SR-TYPES decision 7 read
...
Python as raw bytes**, which is the shape SR-TYPES was opened about — it costs
...
([SR-TYPES](0128_types-list.md) decisions 2a and 12): a `!` glob there is a
...
(SR-TYPES decision 3) because a present-but-empty allowlist empties the index.
...
Consistent with every other consumer-owned file (SR-DOTFUX decision 6); the
...
`fux doctor` as an undeclared entry — SR-DOTFUX veto condition 1, which has
already fired twice.
...
(SR-URL-LIST). `.fuxignore` never sees the URL plane.
...
| **which block a line sits in IS its class** | the `not indexed` / `skipped` split (SR-INGEST decision 15) survives the round trip without anything parsing the note text — the property that decision rests on |
...
- ⚠ **A committed line can now index a format with no decoder.** Raw bytes in
  the index is the exact failure SR-TYPES measured (`.json` alone carried
...
- **`SR-TYPES` decision 7 and `SR-DIR-LIST` decision 2b are narrower than
...
- ⚠ **W-88's report-once promise now covers files only.** A URL skip has
  ...
  ([SR-URL-INGEST](0107_url-ingest.md)), which counts streaks rather than
...
- **Move the type allowlist here too, making `.fuxignore` the only filter.**
  Rejected: it inverts SR-TYPES from an allowlist to a denylist, and *"a
  denylist is never finished — the next generated format nobody has heard of
  arrives indexed"* (SR-TYPES decision 1) is unanswered by anything in this
  record. The allowlist stays in the types list (`.fux/formats.toml` since
  SR-TYPES decision 12); only exclusion moves.
...
- **Make `.fuxignore` purely subtractive — no `!` at all.** Rejected: it
  preserves SR-DIR-LIST's "no precedence to remember" but makes `.fux/**`
...
**Records** — [SR-INGEST](0106_ingest.md) · [SR-DOTFUX](0102_fux-directory.md) ·
[SR-URL-LIST](0116_url-list.md) · [SR-DIR-LIST](0120_dir-list.md) ·
[SR-TYPES](0128_types-list.md) · [SR-DECODE](0139_decode.md)
