...
kind: component
name: SR-EXPAND
title: "SR-EXPAND (0149) — the caller supplies the vocabulary, and fuses its own phrasings"
...
unrepresentable. This is [SR-TUNE](0135_tuning.md) decision 6's argument for
...
is additive ([SR-ASK](0103_ask.md) decision 11's rule) and appears only when
...
caller never asked for, and [SR-CONFIDENCE](0141_confidence.md)'s docstring
states plainly that `support` is bounded by `--top`.

**12. `answer` takes `--expand` and does not take `-q`.**
[SR-ANSWER](0105_answer.md) decision 4: the verb means one answer to one
...
- ⚠ **`expand_weight` is a literature default on an unmeasured corpus** and
  joins the knobs [W-97](../archive/../archive/open/W-97-tuner-knob-sweep.md) sweeps.
...
- Cormack, Clarke & Buettcher, *Reciprocal Rank Fusion outperforms Condorcet
  ...
  https://doi.org/10.1145/1571941.1572114
- [SR-TUNE](0135_tuning.md) decision 6 — one object, not three parameters.
- [SR-ANSWER](0105_answer.md) decision 4 — one answer to one question.
- [SR-CONFIDENCE](0141_confidence.md) — the band, and `support`'s bound.
- [SR-ASK](0103_ask.md) decision 11 — the additive-key rule for `--json`.
- [SR-PORT-LIST](0114_port-list.md) rule 1 — a revival returns with a record.
