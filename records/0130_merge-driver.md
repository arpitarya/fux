...
kind: component
name: SR-MERGE-DRIVER
title: "SR-MERGE-DRIVER (0130) — the committed index merges line by line, last-writer-wins on (ver, sha), and refuses rather than guesses"
...
`sha` changes ([SR-RECORD](0109_index-record.md)), which is what makes *higher*
...
`.gitattributes` line is [SR-MAINTENANCE](0129_hooks.md)'s `fux hooks`. ⚠ **The
...
- **Both halves are fixed together**: the message names taking a side first,
  and [SR-INDEX-LIFECYCLE](0108_index-lifecycle.md) makes the reader diagnose
  markers as markers.
...
[SR-RECORD](0109_index-record.md) · [SR-POSTINGS](0112_postings.md) ·
