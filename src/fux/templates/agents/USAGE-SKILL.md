---
name: fux-usage
description: Run the Fux index correctly from any environment - resolving the `fux` command whether or not a virtualenv is active, choosing between ask, find, answer, explain, graph and path, and reading the JSON rather than the prose. Use before running any fux command, whenever `fux` is reported as not found, or when a fux command fails and you are about to fall back to grep or ripgrep.
compatibility: Requires the `fux` CLI to be resolvable by one of the four invocations below. No network access is needed for `ask` or `find`; `answer` fetches the sources it cites.
---
