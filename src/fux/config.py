...
DEFAULT_MIDDLEWARE = ".fux/middleware/cdp.py"
...
class UrlSource:
    ...
    - `config` — the `[sources.url.config]` table, passed **verbatim** to the
      middleware's optional `configure(config)` hook. Fux validates that it is
      ...
      and it is what keeps the adapter cap from leaking one middleware's
...
def _load_url_source(path: Path, raw) -> UrlSource | None:
    ...
    middleware = raw.get("middleware", DEFAULT_MIDDLEWARE)
    if not isinstance(middleware, str) or not middleware.strip():
        raise FuxError(f"{path}: [sources.url] middleware must be a path to a consumer-owned .py file")
